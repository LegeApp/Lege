...
use djvu_encoder::image::image_formats::{Pixmap, Pixel, Bitmap, GrayPixel};
use std::fs;
...
fn test_single_page_compound() {
    ...
    page = page.with_background(background).expect("Failed to add background");
    page = page.with_jb2_auto_extract(foreground).expect("Failed to add foreground");
...
fn test_multipage_compound_document() {
    ...
    let doc = DjvuBuilder::new(num_pages as usize)
        .with_dpi(300)
        .build();
    ...
    for page_num in 0..num_pages {
        ...
        let page = PageBuilder::new(page_num as usize, width, height)
            .with_background(background).expect("Failed to add background")
    ...
    for page_num in 1..=num_pages {
        ...
        if output.status.success() {
            ...
            println!("  FAILED to decode page {}: {}", page_num, String::from_utf8_lossy(&output.stderr));
...
fn test_ten_page_compound_document() {
    ...
    for page_num in [1, 5, 10] {
        ...
        if output.status.success() {
            ...
            panic!("Failed to decode page {}: {}", page_num, String::from_utf8_lossy(&output.stderr));
        }
    }
