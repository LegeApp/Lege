...
fn test_zp_encoder_basic() {
    ...
    // Create encoder
    let buffer = Vec::new();
    let mut encoder = ZEncoder::new(Cursor::new(buffer), true)
        .expect("Failed to create ZP encoder");
...
fn test_zp_encoder_larger_data() {
    ...
    println!("Last 32 bytes: {:02X?}", &data[data.len().saturating_sub(32)..]);

    assert!(!data.is_empty(), "ZP output should not be empty");
    // Rough compression ratio check - should be less than 10000/8 = 1250 bytes
    // due to compression
    println!("Compression ratio: {:.2}x", 10000.0 / 8.0 / data.len() as f64);
}
