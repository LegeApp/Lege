...
impl Encode {
    ...
    pub fn from_u8_image(img: &::image::GrayImage, data16: &mut [i16], w: usize, h: usize) {
        ...
        // Load pixels in standard order (no flip)
        for y in 0..h {
            for x in 0..w {
                let dst_idx = y * w + x;
    ...
    pub fn from_u8_image_with_stride(
        ...
        // After the 180Â° flip, edge pixels are still at the edges
        // The rightmost column of valid data is at x = w-1
        // The bottom row of valid data is at y = h-1

        // Extend right border (replicate rightmost valid column)
        for y in 0..h {
            let edge_pixel = data16[y * stride + (w - 1)];
            for x in w..stride {
                data16[y * stride + x] = edge_pixel;
            }
        }

        // Extend bottom border (replicate bottom-most valid row)
        for y in h..buffer_h {
            for x in 0..stride {
                let edge_pixel = data16[(h - 1) * stride + x.min(w - 1)];
                data16[y * stride + x] = edge_pixel;
            }
        }
    }
    ...
    pub fn from_i8_channel_with_stride(
        ...
        // NO FLIP - load pixels in standard order
        // The DjVu decoder will handle the coordinate system
        for y in 0..h {
            for x in 0..w {
                let src_idx = y * w + x;
                let dst_idx = y * stride + x;
        ...
        // After the 180Â° flip, edge pixels are still at the edges of the valid region
        // The rightmost column of valid data is at x = w-1
        // The bottom row of valid data is at y = h-1
        ...
        F: Fn(usize, usize) -> i32,
    {
        ...
        for y in 0..h {
            for x in 0..w {
                let dst_idx = y * w + x;
                data16[dst_idx] = pixel_fn(x, y) as i16;
            }
        }
...
fn filter_fv(buf: &mut [i16], w: usize, h: usize, rowsize: usize, scale: usize) {
    ...
    let mut y = 1usize;
    let mut p = s;
    let mut h_adj = if h > 0 { ((h - 1) / scale) + 1 } else { 0 };
    let hlimit = h_adj;
