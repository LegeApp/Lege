...
impl IWEncoder {
    ...
    pub fn encode_chunk(&mut self, max_slices: usize) -> Result<(Vec<u8>, bool), EncoderError> {
        ...
        while slices_encoded < max_slices && self.y_codec.curbit >= 0 {
            ...
            eprintln!("COLOR_DEBUG: total_slices={}, crcb_delay={}, cb_codec.is_some()={}, cr_codec.is_some()={}", 
                     self.total_slices, self.crcb_delay, self.cb_codec.is_some(), self.cr_codec.is_some());
            
            if let Some(ref mut cb) = self.cb_codec {
                if self.total_slices as i32 >= self.crcb_delay {
                    debug!("Encoding Cb slice {}", self.total_slices);
                    eprintln!("COLOR_DEBUG: Encoding Cb slice {}", self.total_slices);
                    let zp: &mut dyn ZpEncoderCursor = &mut zp_impl;
                    cb.code_slice(zp)?;
                } else {
                    eprintln!("COLOR_DEBUG: Skipping Cb - delay not met ({} < {})", self.total_slices, self.crcb_delay);
                }
            }
            if let Some(ref mut cr) = self.cr_codec {
                if self.total_slices as i32 >= self.crcb_delay {
                    debug!("Encoding Cr slice {}", self.total_slices);
                    eprintln!("COLOR_DEBUG: Encoding Cr slice {}", self.total_slices);
                    let zp: &mut dyn ZpEncoderCursor = &mut zp_impl;
                    cr.code_slice(zp)?;
                } else {
                    eprintln!("COLOR_DEBUG: Skipping Cr - delay not met ({} < {})", self.total_slices, self.crcb_delay);
                }
            }
        ...
        if self.serial == 0 {
            ...
            // C++ always uses major version 1, regardless of color mode
            let major = 1; // Version 1 - matches C++ behavior for both color and grayscale
            eprintln!("COLOR_DEBUG: is_color={}, major={:#x}", is_color, major);
        ...
        eprintln!(
            "[DEBUG] encode_chunk: Created chunk with serial {}. Total chunk size: {} bytes.",
            self.serial, chunk_data.len()
        );
