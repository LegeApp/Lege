...
/// Convert interleaved RGB bytes to planar YCbCr (i8), dispatched through the
/// primitive table in `crate::simd` (see `llm-docs/SIMD_AND_PARALLELISM_PLAN.md`).
/// Backend (scalar/wide/avx2) is selected once at startup via `DJVU_PRIMITIVES`.
pub fn rgb_to_ycbcr_planes(img_raw: &[u8], out_y: &mut [i8], out_cb: &mut [i8], out_cr: &mut [i8]) {
    ...
    (crate::simd::PRIMITIVES.color.rgb_to_ycbcr)(img_raw, out_y, out_cb, out_cr);
}
