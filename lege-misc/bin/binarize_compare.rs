...
//!
//! Usage:
//!   cargo run -p lege --bin binarize_compare -- <input-image> <out-dir> [mode] [target_h]
//!
//! `mode` (default `all`):
...
fn to_operating_point(rgb: &[u8], w: u32, h: u32, target_h: u32) -> (Vec<u8>, u32, u32) {
    ...
    (resize_rgb(rgb, w, h, nw, nh, FilterType::Lanczos3), nw, nh)
}
...
fn main() -> Result<()> {
    ...
    let usage =
        "usage: binarize_compare <input-image> <out-dir> [arms|filters|res|window|all] [target_h]";
