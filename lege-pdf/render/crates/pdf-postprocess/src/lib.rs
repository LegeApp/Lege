...
use pdf_render_api::{HostPage, PostprocessCapabilities, PostprocessOperations, RenderedPage};
...
pub use adaptive::{
    AdaptivePostprocess, ExecutionBackend, ExecutionStats, FallbackReason, PostprocessPreference,
    PostprocessReport,
};
pub use cpu::CpuPostprocess;
#[cfg(feature = "gpu")]
pub use gpu::WgpuPostprocess;
...
impl PostprocessGraph {
    /// Operation set required to execute this graph. Parameter and graph-shape
    /// validation remains the executor's responsibility.
    pub fn required_operations(&self) -> PostprocessOperations {
        self.ops
            .iter()
            .fold(PostprocessOperations::empty(), |required, op| {
                required
                    | match op {
                        PostprocessOp::Crop(_) => PostprocessOperations::CROP,
                        PostprocessOp::Resize(_) => PostprocessOperations::RESIZE,
                        PostprocessOp::ConvertToGray(_) => PostprocessOperations::CONVERT_TO_GRAY,
                        PostprocessOp::ApplyToneCurve(_) => PostprocessOperations::APPLY_TONE_CURVE,
                        PostprocessOp::Otsu(_) => PostprocessOperations::OTSU,
                        PostprocessOp::Sauvola(_) => PostprocessOperations::SAUVOLA,
                        PostprocessOp::FuseThresholds(_) => PostprocessOperations::FUSE_THRESHOLDS,
                        PostprocessOp::Dither(_) => PostprocessOperations::DITHER,
                        PostprocessOp::PackMonochrome => PostprocessOperations::PACK_MONOCHROME,
                    }
            })
    }
}
