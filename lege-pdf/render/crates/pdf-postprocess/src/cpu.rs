...
impl PostprocessBackend for CpuPostprocess {
    fn capabilities(&self) -> pdf_render_api::PostprocessCapabilities {
        pdf_render_api::PostprocessCapabilities::HOST_ALL
    }

    fn supports(&self, graph: &PostprocessGraph) -> bool {
        self.capabilities().supports(graph.required_operations()) && validate(graph).is_ok()
    }
...
fn dither(raster: &mut Raster, spec: &DitherSpec) -> Result<(), PostprocessError> {
    ...
    match spec {
        ...
        DitherSpec::FloydSteinberg => {
            ...
            // Only the current and next rows can carry pending error. Padding
            // both ends removes neighbour branches while reducing scratch from
            // `4 * width * height` bytes to `8 * (width + 2)` bytes.
            ...
            for y in 0..h {
                for x in 0..w {
                    let i = y * w + x;
                    let slot = x + 1;
                    let v = raster.data[i] as i32 + current[slot] / 16;
...
mod tests {
    ...
    fn convert_to_gray_rec709_and_flat() {
        ...
        let out = run(
            &page,
            vec![PostprocessOp::ConvertToGray(GraySpec {
                flat_weights: false,
            })],
        );
    ...
    fn crop_preserves_pixels_and_size() {
        ...
        let (w, h, px) = out_gray(&run(
            &page,
            vec![PostprocessOp::Crop(CropSpec {
                x: 1,
                y: 1,
                width: 2,
                height: 2,
    ...
    fn crop_out_of_bounds_is_a_typed_error() {
        ...
        let err = CpuPostprocess
            .execute(
                &page,
                &PostprocessGraph {
                    ops: vec![PostprocessOp::Crop(CropSpec {
                        x: 1,
                        y: 0,
                        width: 2,
                        height: 1,
    ...
    #[test]
    fn floyd_steinberg_scanlines_match_full_frame_error_storage() {
        fn full_frame_reference(mut data: Vec<u8>, width: usize, height: usize) -> Vec<u8> {
            let mut errors = vec![0i32; width * height];
            for y in 0..height {
                for x in 0..width {
                    let i = y * width + x;
                    let value = i32::from(data[i]) + errors[i] / 16;
                    ...
                    if y + 1 < height {
                        if x > 0 {
                            errors[i + width - 1] += error * 3;
                        }
                        errors[i + width] += error * 5;
                        if x + 1 < width {
        ...
        let source: Vec<u8> = (0..width * height)
            .map(|i| ((i * 37 + i / width * 19 + 11) & 0xff) as u8)
            .collect();
        let expected = full_frame_reference(source.clone(), width, height);
        let page = gray_page(width as u32, height as u32, &source);
        let (_, _, actual) = out_gray(&run(
            &page,
            vec![PostprocessOp::Dither(DitherSpec::FloydSteinberg)],
        ));
    ...
    #[test]
    fn cpu_capabilities_cover_the_complete_graph_vocabulary() {
        let graph = PostprocessGraph {
            ops: vec![
                ...
                PostprocessOp::ApplyToneCurve(ToneCurve::identity()),
                PostprocessOp::Otsu(OtsuSpec::default()),
                PostprocessOp::Sauvola(SauvolaSpec { window: 3, k: 0.2 }),
        ...
        let capabilities = CpuPostprocess.capabilities();
        assert_eq!(
            graph.required_operations(),
            pdf_render_api::PostprocessOperations::all()
        );
        assert!(capabilities.supports(graph.required_operations()));
        assert!(!capabilities.resident_execution);
