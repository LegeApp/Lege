...
fn compile_test_page() -> CompiledPage {
    ...
    let snapshot = DocumentSnapshot::open(source, DocumentLimits::default()).expect("open failed");
    ...
    for op in page.operations.iter() {
        match op {
            ...
            DisplayOp::DrawGlyphRun { run, .. } => {
                let _ = writeln!(
                    out,
                    "<!-- glyph-run run#{} glyphs={} -->",
                    run.0,
                    page.glyph_runs[run.index()].glyphs.len()
                );
            }
...
fn svg_path_d(path: &PathData, ctm: &Ctm) -> String {
    ...
    for verb in path.verbs.iter() {
        match verb {
            ...
            PathVerb::CurveTo => {
                let (c1, c2, p3) = (
                    ctm.map(path.points[pt]),
                    ctm.map(path.points[pt + 1]),
                    ctm.map(path.points[pt + 2]),
                );
                let _ = write!(
                    d,
                    "C{} {} {} {} {} {} ",
        ...
        match op {
            ...
            DisplayOp::FillPath {
                ...
                fill_polygon(
                    ...
                    &subpaths,
                    color,
                    matches!(rule, pdf_page_ir::FillRule::EvenOdd),
                );
            }
            _ => {}
        }
