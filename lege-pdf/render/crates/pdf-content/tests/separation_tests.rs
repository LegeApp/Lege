...
fn separation_full_tint_is_ink_not_white() {
    ...
    assert!(
        c.r < 0.02 && c.g < 0.02 && c.b < 0.02,
        "tint 1.0 must be full ink, got {c:?}"
    );
}
...
fn separation_zero_tint_is_blank() {
    ...
    assert!(
        ...
        "tint 0 leaves no colorant, got {c:?}"
    );
}
...
fn separation_initial_color_is_full_colorant() {
    ...
    assert!(
        c.r < 0.02,
        "initial colour of a Separation is full tint, got {c:?}"
    );
}
...
fn non_separation_named_spaces_are_untouched() {
    ...
    assert!(
        c.r > 0.9 && c.g < 0.1,
        "ICCBased 3-comp stays RGB, got {c:?}"
    );
}
...
fn degenerate_separation_image_ramp_is_repaired_and_flagged() {
    ...
    assert!(
        page.images[0].lowering_degraded,
        "repaired ramp must be flagged"
    );
...
fn all_colorant_runs_its_tint_transform_like_any_other() {
    ...
    assert!(
        dark.r < 0.02,
        "tint 1 through a 1->0 ramp is black, got {dark:?}"
    );
    let light = first_solid(&page_with(ramp, b"/Cs cs 0 scn 0 0 100 100 re f"));
    assert!(
        light.r > 0.98,
        "tint 0 through a 1->0 ramp is white, got {light:?}"
    );
}
...
fn all_colorant_image_lut_matches_a_named_colorant() {
    ...
    assert_eq!(
        a, n,
        "/All must not be special-cased against a named colorant"
    );
...
fn two_colorant_devicen_image_lowers_to_a_two_input_lut() {
    ...
    assert!(
        black.iter().all(|&c| c < 60),
        "corner (0,0) black: {black:?}"
    );
    assert!(red[0] > 200 && red[1] < 60, "corner (1,0) red: {red:?}");
    assert!(
        green[1] > 200 && green[0] < 60,
        "corner (0,1) green: {green:?}"
    );
    assert!(
        white.iter().all(|&c| c > 200),
        "corner (1,1) white: {white:?}"
    );
    // Crucially, the *second* input changes the result — the bug was that it
    // never reached the transform at all.
    assert_ne!(
        black, green,
        "the second colorant must affect the baked colour"
    );
    assert_ne!(
        black, red,
        "the first colorant must affect the baked colour"
    );
}
