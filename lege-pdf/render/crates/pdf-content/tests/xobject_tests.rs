...
fn image_xobject_emits_draw_image() {
    ...
    assert!(
        page.ops
            .iter()
            .any(|o| matches!(o, SemanticOp::DrawImage(_)))
    );
}
...
fn nested_forms_respect_depth_limit() {
    ...
    let mut ctx = ParseContext::new();
    let limits = pdf_content::ContentLimits {
        max_invoke_depth: 4,
        ..Default::default()
    };
    ...
    assert!(
        matches!(err, pdf_content::ContentError::RecursionDepth(_)),
        "{err:?}"
    );
}
...
fn pending_path_does_not_leak_into_or_out_of_a_form() {
    ...
    assert_eq!(
        path.verbs.len(),
        5,
        "form fill must not include the caller's path"
    );
}
...
fn a_resource_name_is_not_cached_across_resource_dictionaries() {
    ...
    assert_eq!(
        page.shadings[shadings[0].0 as usize]
            .object
            .map(|o| o.number),
        Some(7)
    );
    assert_eq!(
        page.shadings[shadings[1].0 as usize]
            .object
            .map(|o| o.number),
        Some(8)
    );
