...
fn varied_fixture(page_count: u32) -> Vec<u8> {
    ...
    for i in 0..page_count {
        ...
        c.push_str(&format!(
            "BT /F1 12 Tf 72 {} Td (Page {i}) Tj ET Q",
            700 - i * 5
        ));
...
fn probe_all(snapshot: &DocumentSnapshot, threads: usize) -> Vec<String> {
    ...
    let slots: Vec<std::sync::Mutex<Option<String>>> = (0..page_count)
    ...
    slots
        .into_iter()
        .map(|s| s.into_inner().unwrap().expect("page not probed"))
        .collect()
}
...
fn concurrent_compilation_of_the_same_page_is_stable() {
    ...
    let results: Vec<String> = std::thread::scope(|scope| {
        let handles: Vec<_> = (0..8)
            .map(|_| scope.spawn(|| probe(&snapshot, 0)))
            .collect();
