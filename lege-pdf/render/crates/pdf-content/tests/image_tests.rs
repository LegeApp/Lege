...
fn constant_black_cmyk_profile() -> Vec<u8> {
    let mut body = Vec::new();
    body.extend_from_slice(b"mft2\0\0\0\0");
    body.extend_from_slice(&[4, 3, 2, 0]); // inputs, outputs, grid, padding
    ...
    // Sixteen CMYK corners, all mapped to v2 Lab black with neutral a/b.
    for _ in 0..16 {
        for value in [0u16, 32768, 32768] {
...
fn codec_cmyk_image_carries_icc_lookup_tables_into_ir() {
    ...
    b.add_stream(
        ...
        "/Type/XObject/Subtype/Image/Width 1/Height 1/BitsPerComponent 8\
         /ColorSpace[/ICCBased 6 0 R]/Filter/DCTDecode",
        &[0xFF, 0xD8, 0xFF, 0xD9],
    );
    b.add_stream(6, "/N 4", &constant_black_cmyk_profile());
    b.finish_classic_xref("/Root 1 0 R");

    let page = compile(&open(b.into_bytes()), 0);
    let image = &page.images[0];
    assert!(image.samples.is_none(), "codec samples remain backend-lazy");
    assert!(image.codec_data.is_some());
    match &image.color_space {
        ImageColorSpace::IccCmyk { transform } => {
            assert_eq!(transform.grid, 2);
            assert_eq!(transform.clut.len(), 16);
            assert!(transform.input_tables.iter().all(|table| table.len() == 2));
            assert!(transform.output_tables.iter().all(|table| table.len() == 2));
        }
        other => panic!("expected ICC CMYK transform in image IR, got {other:?}"),
    }
}
...
fn indexed_lab_palette_converts_to_rgb() {
    ...
    match &img.color_space {
        ImageColorSpace::Indexed { base, lookup, .. } => {
            assert!(
                matches!(**base, ImageColorSpace::Rgb),
                "Lab base pre-converted to Rgb"
            );
            assert!(
            ...
            assert!(
                lookup[3] < 30 && lookup[4] < 30 && lookup[5] < 30,
                "entry 1 is black: {:?}",
                &lookup[3..6]
            );
        }
...
fn malformed_color_key_mask_is_ignored() {
    ...
    assert!(
        page.images[0].mask.is_none(),
        "wrong-length color-key ignored"
    );
}
...
fn dct_image_carries_codec_flag_and_no_samples() {
    ...
    assert!(
        page.features.contains(PageFeatures::NEEDS_DCT),
        "DCT codec flagged"
    );
    let img = &page.images[0];
...
fn separation_image_bakes_a_tint_lut() {
    ...
    let ImageColorSpace::TintLut { rgb } = &img.color_space else {
        panic!(
            "Separation image must lower to TintLut, got {:?}",
            img.color_space
        );
    };
...
fn separation_image_unresolvable_transform_falls_back_to_lut_not_blank() {
    ...
    let ImageColorSpace::TintLut { rgb } = &img.color_space else {
        panic!(
            "unresolvable transform must still fall back to a TintLut, got {:?}",
            img.color_space
        );
    };
    ...
    assert_eq!(
        &rgb[255 * 3..255 * 3 + 3],
        &[0, 0, 0],
        "tint 255 approximates to black"
    );
}
...
fn multi_colorant_devicen_image_evaluates_the_tint_transform_per_sample() {
    ...
    assert_eq!(
        &samples[0..3],
        &[0, 0, 0],
        "tints (0,0) through C0 -> black"
    );
    assert_eq!(
        &samples[3..6],
        &[255, 255, 255],
        "tints (1,1) through C1 -> white"
    );
}
...
00ff000000ff>] ID "
    ...
    assert_eq!(
        decoded.len(),
        row * h,
        "the embedded \" EI \" must not end the image"
    );
    assert!(matches!(
        img.color_space,
        ImageColorSpace::Indexed { hival: 3, .. }
    ));
}
...
fn ascii85_eod_from_a_later_image_is_not_borrowed() {
    ...
    content.extend_from_slice(
        b" EI Q 1 0 0 RG 5 w 0 0 m 8 8 l S q BI /W 8 /H 5 /BPC 8 /CS /G /F /A85 ID ",
    );
    ...
    assert_eq!(
        page.images.len(),
        2,
        "the first frame swallowed the second image"
    );
    assert!(
...
fn a_mask_stream_that_is_not_an_image_mask_has_the_opposite_polarity() {
    fn mask_decode(image_mask: bool, extra: &str) -> Option<Vec<[f32; 2]>> {
        ...
        b.add_stream(
            6,
            &format!(
                "/Type/XObject/Subtype/Image/Width 2/Height 2/BitsPerComponent 1{flag}{extra}"
            ),
