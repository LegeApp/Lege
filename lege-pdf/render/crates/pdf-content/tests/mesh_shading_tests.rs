...
fn type4_free_form_triangles_decode() {
    ...
    let t1 = &triangles[1];
    assert!(
        (t1[0].x - 100.0).abs() < 0.01 && t1[0].y == 0.0,
        "shared v1"
    );
    assert!((t1[1].x - 50.196).abs() < 0.01, "shared v2");
    assert!(
        t1[2].x == 0.0 && (t1[2].y - 100.0).abs() < 0.01,
        "new vertex"
    );
...
const PATCH_COLORS: [[u8; 3]; 4] = [[255, 0, 0], [0, 255, 0], [0, 0, 255], [255, 255, 0]];
...
fn type7_tensor_patch_reads_sixteen_points() {
    ...
    assert!(
        close(p.points[5], [33.3, 33.3]),
        "P(1,1): {:?}",
        p.points[5]
    );
    assert!(
        close(p.points[6], [33.3, 66.7]),
        "P(1,2): {:?}",
        p.points[6]
    );
    assert!(
        close(p.points[10], [66.7, 66.7]),
        "P(2,2): {:?}",
        p.points[10]
    );
    assert!(
        close(p.points[9], [66.7, 33.3]),
        "P(2,1): {:?}",
        p.points[9]
    );
}
