...
mod tests {
    ...
    fn cmyk_policy_is_the_measured_table() {
        ...
        assert_eq!(
            px(cmyk_to_rgb(0.0, 0.0, 0.0, 0.0)),
            [255, 255, 255],
            "no ink → paper"
        );
        ...
        assert_eq!(
            px(cmyk_to_rgb(0.0, 0.0, 0.0, 1.0)),
            [35, 31, 32],
            "full K is measured ink"
        );

        let cyan = px(cmyk_to_rgb(1.0, 0.0, 0.0, 0.0));
        assert!(
            cyan[0] < 60 && cyan[2] > 180,
            "cyan is an ink, not [0,255,255]: {cyan:?}"
        );
