...
pub fn adobe_cmyk_to_srgb_u8(c: u8, m: u8, y: u8, k: u8) -> [u8; 3] {
    ...
    correct(
        c1_rgb,
        (fix_c - ((c_index as i32) << 13)) * (c_index as i32 - c1_index as i32),
    );

    let m1_rgb = ADOBE_CMYK[index_from_cmyk(c_index, m1_index, y_index, k_index)];
    correct(
        m1_rgb,
        (fix_m - ((m_index as i32) << 13)) * (m_index as i32 - m1_index as i32),
    );

    let y1_rgb = ADOBE_CMYK[index_from_cmyk(c_index, m_index, y1_index, k_index)];
    correct(
        y1_rgb,
        (fix_y - ((y_index as i32) << 13)) * (y_index as i32 - y1_index as i32),
    );

    let k1_rgb = ADOBE_CMYK[index_from_cmyk(c_index, m_index, y_index, k1_index)];
    correct(
        k1_rgb,
        (fix_k - ((k_index as i32) << 13)) * (k_index as i32 - k1_index as i32),
    );
...
mod tests {
    ...
    fn the_table_is_complete_and_verbatim() {
        ...
        assert_eq!(
            ADOBE_CMYK[index_from_cmyk(0, 0, 0, 0)],
            [255, 255, 255],
            "no ink = paper"
        );
        assert_eq!(
            ADOBE_CMYK[index_from_cmyk(0, 0, 0, 8)],
            [34, 30, 31],
            "full K is not #000"
        );
    }
    ...
    fn the_bug_the_oracle_found_is_fixed() {
        ...
        assert!(
            px[1] > 80 && px[1] < 150,
            "green is muted, not 143-bright: {px:?}"
        );
