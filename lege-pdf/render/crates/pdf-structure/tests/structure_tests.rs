...
fn classic_table_loads() {
    ...
    assert_eq!(
        structure.xref.locate(ObjectId::new(0, 0)),
        ObjectLocation::Free
    );
...
fn recovered_stub_page_tree_triggers_rebuild() {
    ...
    assert!(
        o2 > xref_off,
        "object 2 should resolve to its real later definition"
    );
    assert_eq!(structure.trailer.root, ObjectId::new(1, 0));
}
...
fn sloppy_xref_entry_line_endings_tolerated() {
    ...
    assert_eq!(
        structure.xref.locate(ObjectId::new(1, 0)),
        ObjectLocation::Offset(off1)
    );
    assert_eq!(
        structure.xref.locate(ObjectId::new(2, 0)),
        ObjectLocation::Offset(off2)
    );
}
...
fn incremental_update_shadows_and_frees() {
    ...
    assert_eq!(
        structure.xref.locate(ObjectId::new(3, 0)),
        ObjectLocation::Free
    );
...
fn xref_stream_type2_entries_locate_in_object_stream() {
    ...
    assert_eq!(
        structure.xref.locate(ObjectId::new(4, 1)),
        ObjectLocation::Free
    );
}
...
fn hybrid_xrefstm_classic_entries_win() {
    ...
    assert_eq!(
        structure.xref.locate(ObjectId::new(2, 0)),
        ObjectLocation::Offset(off2_table)
    );
    // Stream supplies object 3.
    assert_eq!(
        structure.xref.locate(ObjectId::new(3, 0)),
        ObjectLocation::Offset(off3)
    );
}
...
fn prev_chain_cycle_terminates() {
    ...
    assert!(
        structure
            .recovery
            .iter()
            .any(|e| matches!(e, RecoveryEvent::Other(msg) if msg.contains("cycle")))
    );
    assert_eq!(structure.trailer.root, ObjectId::new(1, 0));
}
...
fn object_zero_never_resolvable() {
    let structure = load(basic_two_object_pdf());
    assert_eq!(
        structure.xref.locate(ObjectId::new(0, 65535)),
        ObjectLocation::Free
    );
    assert_eq!(
        structure.xref.locate(ObjectId::new(0, 0)),
...
fn a_subsection_declared_at_one_but_starting_with_the_free_head_is_corrected() {
    ...
    assert_eq!(
        at(1),
        Some(cat),
        "object 1 is the catalog, not a free entry"
    );
