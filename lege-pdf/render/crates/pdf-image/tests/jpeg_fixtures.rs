...
fn assert_close_gray(name: &str) {
    ...
    assert!(
        mean < 0.5 && max <= 2,
        "{name}: mean {mean} max {max} vs libjpeg"
    );
}

/// Color comparisons use the same fancy chroma-upsample filter as libjpeg;
/// the small remaining allowance is for IDCT rounding differences.
fn assert_close_color(name: &str, format: DecodedFormat, mean_bound: f64) {
    ...
    assert!(
        mean < mean_bound,
        "{name}: mean {mean} vs libjpeg (bound {mean_bound})"
    );
}
...
fn progressive_ycbcr_420() {
    assert_close_color("prog_rgb420", DecodedFormat::Rgb8, 0.75);
}

#[test]
fn baseline_ycbcr_422() {
    assert_close_color("base_rgb422", DecodedFormat::Rgb8, 0.75);
}
...
fn adobe_cmyk_photoshop_convention() {
    ...
    assert!(
        mean < 6.0,
        "Adobe CMYK (raw, /Decode-normalized): mean {mean}"
    );
}
...
fn adobe_cmyk_noninverted_writer_passes_through() {
    ...
    assert!(
        mean < 6.0,
        "non-inverted CMYK passes through as true DeviceCMYK: mean {mean}"
    );
}
...
fn nomad_flute_devicecmyk_ycck() {
    ...
    assert!(
        mean < 8.0,
        "Nomad Flute YCCK CMYK (raw, /Decode-normalized) vs libjpeg: mean {mean}"
    );
}
