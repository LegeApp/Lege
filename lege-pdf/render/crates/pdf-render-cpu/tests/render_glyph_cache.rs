...
fn small_glyph_renders_through_cache_and_repeats_identically() {
    ...
    assert!(
        painted(&a, 60, 35),
        "triangle interior painted via cache blit"
    );
    assert!(
        !painted(&a, 95, 70),
        "bottom-right corner outside the triangle"
    );
    assert!(
        !painted(&a, 22, 70),
        "bottom-left corner outside the triangle"
    );
    ...
    assert_eq!(
        a.pixels, b.pixels,
        "cache-hit render matches the populating render"
    );
}
