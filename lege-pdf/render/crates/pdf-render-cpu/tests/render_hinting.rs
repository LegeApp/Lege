...
use pdf_page_ir::{
    ...
    PageComplexity, PageFeatures, Paint, PlacedGlyph, Rect, ResourceKey,
};
use pdf_render_api::{
...
fn render(page: CompiledPage, hinting: HintingPolicy) -> HostPage {
    let backend = CpuBackend::new(CpuBackendOptions {
        hinting,
        ..Default::default()
    });
...
fn auto_hinting_changes_small_text_but_not_large() {
    ...
    assert!(
        !differs(&c, &d),
        "Auto must not hint above AUTO_HINT_MAX_PPEM"
    );
}
...
fn embedded_hinting_applies_at_any_size() {
    ...
    assert!(
        differs(&a, &b),
        "Embedded honours the font at large sizes too"
    );
}
