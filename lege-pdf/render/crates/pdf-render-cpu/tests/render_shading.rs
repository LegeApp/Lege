...
fn page_with(
    operations: Vec<DisplayOp>,
    paints: Vec<Paint>,
    paths: Vec<PathData>,
    shadings: Vec<ShadingResource>,
    size: f64,
) -> CompiledPage {
    CompiledPage {
...
fn axial_sh_paints_horizontal_gradient() {
    ...
    assert!(
        left < mid && mid < right,
        "monotone gradient: {left} {mid} {right}"
    );
...
fn bbox_clips_the_shading() {
    ...
    let (host, _) = CpuBackend::default()
        .render_to_host(&request(page, 20))
        .unwrap();
    assert_ne!(
        px(&host, 10, 10),
        [255, 255, 255, 255],
        "inside the bbox is painted"
    );
    assert_eq!(
        px(&host, 2, 2),
        [255, 255, 255, 255],
        "outside the bbox (corner) is clear"
    );
    assert_eq!(
        px(&host, 18, 10),
        [255, 255, 255, 255],
        "right of the bbox is clear"
    );
    assert_eq!(
        px(&host, 10, 2),
        [255, 255, 255, 255],
        "above the bbox is clear"
    );
}
...
fn axial_without_extend_leaves_background_outside_axis() {
    ...
    assert_eq!(
        px(&host, 0, 5),
        [255, 255, 255, 255],
        "left of axis untouched"
    );
    assert_eq!(
        px(&host, 9, 5),
        [255, 255, 255, 255],
        "right of axis untouched"
    );
...
fn radial_sh_is_radially_symmetric() {
    ...
    assert!(
        corner > 200,
        "corner (r>5, extended) near white, got {corner}"
    );
...
fn shading_pattern_fills_only_the_path() {
    ...
    let verbs: Arc<[PathVerb]> = Arc::from([
        PathVerb::MoveTo,
        PathVerb::LineTo,
        PathVerb::LineTo,
        PathVerb::LineTo,
...
fn axial_pattern_background_fills_outside_the_axis() {
    ...
    let (host, _) = CpuBackend::default()
        .render_to_host(&request(page, 10))
        .unwrap();
    let bg = px(&host, 2, 5);
    assert!(
        bg[1] > 200 && bg[0] < 80 && bg[2] < 80,
        "off-axis takes the green background, got {bg:?}"
    );
    let axis = px(&host, 5, 5);
    assert!(
        axis[0] == axis[1] && axis[1] == axis[2],
        "on-axis is the gray gradient, got {axis:?}"
    );
    assert_eq!(
        px(&host, 0, 0),
        [255, 255, 255, 255],
...
fn tensor_patch_fills_its_quad_with_bilinear_corners() {
    ...
    let kind = ShadingKind::MeshPatches {
        patches: Arc::from([patch]),
        background: None,
    };
    let page = page_with(
        vec![DisplayOp::DrawShading {
