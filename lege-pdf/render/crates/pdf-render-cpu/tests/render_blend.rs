...
fn multiply_gray_over_white_darkens() {
    ...
    let p = center(&blend_over(
        Background::White,
        Color::from_rgb(0.5, 0.5, 0.5),
        BlendMode::Multiply,
    ));
    assert!(
        near(p[0], 128) && near(p[1], 128) && near(p[2], 128),
        "{p:?}"
    );
}
...
fn normal_blend_unaffected() {
    ...
    assert!(
        near(p[0], 51) && near(p[1], 102) && near(p[2], 153),
        "{p:?}"
    );
}
...
fn nonseparable_is_deterministic() {
    ...
    let a = center(&blend_over(
        bg,
        Color::from_rgb(0.8, 0.1, 0.4),
        BlendMode::Saturation,
    ));
    let b = center(&blend_over(
        bg,
        Color::from_rgb(0.8, 0.1, 0.4),
        BlendMode::Saturation,
    ));
...
fn multiply_image_blends_with_backdrop() {
    ...
    let p = center(&render_page(
        page,
        Background::Solid(Color::from_rgb(0.5, 0.5, 0.5)),
    ));
    assert!(near(p[0], 128), "r={}", p[0]);
    assert!(near(p[1], 0), "g={}", p[1]);
    assert!(near(p[2], 0), "b={}", p[2]);
...
fn knockout_group_replaces_earlier_elements_in_overlap() {
    ...
    let render_group = |knockout: bool| {
        ...
        page.operations = Arc::from([
            DisplayOp::BeginTransparencyGroup {
                group: pdf_page_ir::TransparencyGroupId(0),
            },
            DisplayOp::FillPath {
                path: PathId(0),
                paint: PaintId(0),
    ...
    assert!(
        near(knocked[0], knocked[1]),
        "knockout drops the red layer: {knocked:?}"
    );
}
