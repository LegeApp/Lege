...
fn group_opacity_applies_to_the_whole_group() {
    ...
    let ops = vec![
        DisplayOp::BeginTransparencyGroup {
            group: TransparencyGroupId(0),
        },
        fill(0, 0),
    ...
    assert!(
        near(px(&host, 10, 10), [255, 128, 128, 255]),
        "{:?}",
        px(&host, 10, 10)
    );
...
fn group_opacity_does_not_compound_on_overlap() {
    ...
    assert!(
        near(overlap, solo),
        "overlap must match solo (no compounding): {overlap:?} vs {solo:?}"
    );
}
...
fn full_opacity_group_is_transparent_passthrough() {
    ...
    assert!(
        near(px(&host, 10, 10), [51, 102, 153, 255]),
        "{:?}",
        px(&host, 10, 10)
    );
}
...
fn non_isolated_multiply_group_composites_against_page_backdrop() {
    ...
    let ops = vec![
        ...
        DisplayOp::BeginTransparencyGroup {
            group: TransparencyGroupId(0),
        }, // wrapper, Normal, non-isolated
        DisplayOp::BeginTransparencyGroup {
            group: TransparencyGroupId(1),
        }, // Multiply, non-isolated
    ...
    let host = render(
        ...
        ops,
        vec![
            group_ni(1.0, BlendMode::Normal),
            group_ni(1.0, BlendMode::Multiply),
        ],
        20,
    );
    assert!(
        near(px(&host, 10, 10), [0, 64, 0, 255]),
        "gold-loss repro: {:?}",
        px(&host, 10, 10)
    );
    ...
    assert!(
        near(px(&host, 3, 3), [255, 128, 0, 255]),
        "backdrop halo: {:?}",
        px(&host, 3, 3)
    );
}
...
fn non_isolated_normal_group_is_backdrop_passthrough() {
    ...
    assert!(
        near(px(&host, 6, 10), [0, 128, 255, 255]),
        "blue area: {:?}",
        px(&host, 6, 10)
    );
    assert!(
        near(px(&host, 14, 10), [255, 128, 0, 255]),
        "orange area: {:?}",
        px(&host, 14, 10)
    );
}
...
fn nested_groups_multiply_opacity() {
    ...
    assert!(
        near(px(&host, 10, 10), [255, 191, 191, 255]),
        "{:?}",
        px(&host, 10, 10)
    );
}
...
fn group_content_starts_with_the_soft_mask_reset() {
    ...
    assert!(
        near(px(&host, 5, 10), [255, 128, 128, 255]),
        "one application of 0.5: {:?}",
        px(&host, 5, 10)
    );
    assert!(
        near(px(&host, 15, 10), [255, 255, 255, 255]),
