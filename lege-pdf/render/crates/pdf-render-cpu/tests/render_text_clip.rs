...
fn mode7_text_clip_constrains_a_following_fill_to_glyph_shape() {
    ...
    assert!(
        is_red(px(&host, 510, 260)),
        "triangle interior must be filled through the text clip"
    );
    assert!(
        is_white(px(&host, 160, 610)),
        "outside the glyph must stay unpainted (clipped out)"
    );
    assert!(
        is_white(px(&host, 850, 610)),
        "other bbox corner outside the glyph stays white"
    );
}
...
fn mode4_glyph_paints_and_clips_the_following_fill() {
    ...
    assert!(
        is_red(px(&host, 510, 260)),
        "interior filled through the clip"
    );
    assert!(
        is_white(px(&host, 160, 610)),
        "exterior clipped out and unpainted"
    );
}
...
fn empty_text_clip_clips_everything_out() {
    ...
    for (x, y) in [(510, 260), (160, 610), (500, 500), (10, 10)] {
        assert!(
            is_white(px(&host, x, y)),
            "empty text clip must let nothing paint at ({x},{y})"
        );
    }
}
