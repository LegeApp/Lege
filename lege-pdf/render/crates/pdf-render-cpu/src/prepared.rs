...
pub struct PreparedClip {
    ...
    pub has_mask: bool,
    /// The nearest path clip at or above this one — the clip whose mask a draw
    /// under this clip actually uses. Every rect descendant of a given path clip
    /// shares one `mask_source`, so the path is rasterized **once** and cached,
    /// not rebuilt per descendant. `None` when the chain is purely rectangular.
    /// A shared map-boundary clip with N rect children collapses from N large
    /// mask builds (and N cached masks) to one.
    pub mask_source: Option<u32>,
}
...
pub struct PreparedTiling {
    ...
    pub under: [u8; 4],
    pub color_policy: pdf_render_api::RenderColorPolicy,
    ...
    pub blend: pdf_page_ir::BlendMode,
    /// When set, an `/ImageMask` stencil painted with this tiling pattern: the
    /// pattern shows only through the stencil's coverage (§8.9.6.2 — an image
    /// mask is painted with the current fill colour, which may be a pattern).
    /// `None` for an ordinary path-shaped tiling fill (byte-identical there).
    pub stencil: Option<Box<crate::image::PreparedImage>>,
}
...
pub struct CpuPreparedPage {
    ...
    /// Recoloring contract for visible page paint. Nested soft-mask content
    /// always lowers with `Original` so mask luminosity remains semantic.
    pub color_policy: pdf_render_api::RenderColorPolicy,
    ...
    #[cfg(feature = "profiling")]
    pub(crate) profile: std::cell::RefCell<pdf_profiling::ProfileReport>,
...
pub fn lower(
    ...
    lower_impl(
        ...
        decode_limits,
        hinting,
        pdf_render_api::RenderColorPolicy::Original,
        None,
        None,
        None,
        None,
...
fn decode_unique_payload(
    ...
    if seen.insert(key) {
        if let Some(decoded) = decode_codec_samples(
            out, kind, data, width, height, bpc, is_mask, parms, None, None,
        ) {
            out.profile
                .borrow_mut()
...
fn lower_impl(
    ...
    let mut soft_mask_content_depth = 0_u32;
    ...
    for op in page.operations.iter() {
        ...
        match op {
            ...
            DisplayOp::FillPath {
                ...
                let paint_policy = if soft_mask_content_depth == 0 {
                    out.color_policy
                } else {
                    pdf_render_api::RenderColorPolicy::Original
                };
            ...
            DisplayOp::DrawGlyphRun {
                ...
                // shading (attached after lowering). A tiling-pattern fill
                // reuses the same glyph outlines as the fill mask for the
                // ordinary path tiler.
                let (color, run_shading, run_tiling) = match &page.paints[paint.index()] {
                    Paint::Solid(color) => (recolor(*color, paint_policy), None, None),
                    Paint::Shading { shading, matrix } => {
                        match prepare_shading(
                            &page.shadings[shading.index()],
                            matrix.then(base),
                            ...
                            Some(sh) => (pdf_page_ir::Color::BLACK, Some(Box::new(sh)), None),
                            None => continue,
                        }
                    }
                ...
                if do_fill {
                    let b = if let Some((tiling, pattern_to_device)) = run_tiling {
                        lower_tiled_glyph_run(
                            &mut out,
                            gr,
                            program.as_ref(),
                            ctm,
                            ...
                            Some(prog) => {
                                ...
                                let cached = match shared_glyphs {
                                    Some(gc) if gr.render_mode == 0 && !synth.is_active() => {
                                        let font_id =
                                            *glyph_font_ids.entry(gr.font.0).or_insert_with(|| {
                                                FontProgramKey::for_resource(
                                                    &page.fonts[gr.font.index()],
                                                )
                                            });
                ...
                if do_stroke && let (Some(prog), Some(gs)) = (&program, stroke) {
                    let (stroke_color, stroke_shading) = match &page.paints[gs.paint.index()] {
                        Paint::Solid(c) => (recolor(*c, paint_policy), None),
                        Paint::Shading { shading, matrix } => {
                            match prepare_shading(
                                &page.shadings[shading.index()],
                                matrix.then(base),
            ...
            DisplayOp::EndSoftMask => {
                soft_mask_content_depth = soft_mask_content_depth.saturating_sub(1);
            ...
            DisplayOp::DrawShading { shading, .. } => {
                ...
                let clip = clip_stack.last().copied();
                let policy = if soft_mask_content_depth == 0 {
                    out.color_policy
                } else {
                    pdf_render_api::RenderColorPolicy::Original
                };
                let b =
                    lower_shading_op(&mut out, &page.shadings[shading.index()], ctm, clip, policy);
                accumulate_scope_bounds(&mut scope_stack, b);
            }
            DisplayOp::DrawImage {
                image,
                ...
                let clip = clip_stack.last().copied();
                // An /ImageMask stencil painted with a tiling pattern (§8.9.6.2:
                // an image mask is painted with the current fill colour). Fill
                // the image's unit square with the pattern, masked by the
                // stencil's coverage — instead of the solid-colour fallback,
                // which painted the whole stencil black.
                if ir.is_stencil
                    && let Paint::Pattern { tiling, matrix } = &page.paints[paint.index()]
                {
                    let (tiling, matrix) = (*tiling, *matrix);
                    if let Some((stencil_img, _)) =
                        lower_image(&mut out, ir, [255, 255, 255], ctm, *alpha, *blend, None)
                    {
                        let unit = unit_square_path();
                        let b = lower_tiled(
                            &mut out,
                            &unit,
                            ctm,
                    ...
                    && let Paint::Shading { shading, matrix } = &page.paints[paint.index()]
                {
                    ...
                    let stencil_rgb = match &page.paints[paint.index()] {
                        Paint::Solid(c) => {
                            let c = recolor(*c, paint_policy);
                            [to_u8(c.r), to_u8(c.g), to_u8(c.b)]
                        }
                        _ => [0, 0, 0],
                    };
                    if let Some((img, b)) =
                        lower_image(&mut out, ir, stencil_rgb, ctm, *alpha, *blend, clip)
                    {
                        out.ops.push(PreparedOp::Image(img));
                        accumulate_scope_bounds(&mut scope_stack, Some(b));
                    }
                }
            }
...
/// The unit square `[0,1]²` as a closed path — the fill shape for a stencil
/// painted with a tiling pattern (the stencil's own coverage does the masking;
/// this rect just bounds the tiled region to the image placement).
fn unit_square_path() -> PathData {
    PathData {
        verbs: std::sync::Arc::from(
            [
                PathVerb::MoveTo,
                PathVerb::LineTo,
                PathVerb::LineTo,
...
/// Returns the prepared image (the caller pushes it, or attaches it as a
/// stencil to a tiling fill) and its device bounds.
#[allow(clippy::too_many_arguments)]
fn lower_image(
    ...
    clip: Option<u32>,
) -> Option<(Box<crate::image::PreparedImage>, DeviceRect)> {
    ...
    let (samples, img_w, img_h, bpc, color_space) = match ir.samples.clone() {
        ...
        None => {
            ...
            let mut d = decode_codec_image(out, ir, target_size)?;
            ...
            let cs = match &ir.color_space {
                ...
                // A CMYK codec reports only channel arity. Preserve the
                // declared press-profile transform so sampling does not fall
                // back to DeviceCMYK's frozen table.
                pdf_page_ir::ImageColorSpace::IccCmyk { .. }
                    if d.color_space == pdf_page_ir::ImageColorSpace::Cmyk =>
                {
                    ir.color_space.clone()
                }
            ...
            if let Some(opacity) = d.alpha.clone() {
                match ir.smask_in_data {
                    ...
                    2 => {
                        let Some(unassociated) =
                            unpremultiply_in_data_alpha(&d.samples, &opacity, d.format)
                        else {
                            out.diagnostics.note_degraded(
                                "JPX premultiplied in-data alpha has an invalid decoded layout, draw skipped"
                                    .into(),
                            );
                            return None;
                        };
                        d.samples = unassociated;
                        in_data_smask = Some(std::sync::Arc::new(pdf_page_ir::ImageSMask {
                            width: d.width,
                            height: d.height,
                            bits_per_component: 8,
    ...
    let prepared = Box::new(crate::image::PreparedImage {
...
fn reinterpreting_space_name(cs: &pdf_page_ir::ImageColorSpace) -> &'static str {
    match cs {
        ...
        pdf_page_ir::ImageColorSpace::IccCmyk { .. } => "IccCmyk",
        _ => "reinterpreting",
    }
}
...
/// Convert a JPX `/SMaskInData 2` base plane from associated to straight
/// samples before the alpha plane is used as an ordinary soft mask.
///
/// An alpha-zero pixel has no defined straight colour. Canonicalize it to zero
/// so interpolation cannot leak hidden premultiplied components into a
/// neighbouring partially-covered pixel.
fn unpremultiply_in_data_alpha(
    samples: &[u8],
    alpha: &[u8],
    format: pdf_image::DecodedFormat,
) -> Option<std::sync::Arc<[u8]>> {
    let channels = match format {
        pdf_image::DecodedFormat::Rgba8 => 3,
        pdf_image::DecodedFormat::GrayA8 => 1,
        _ => return None,
    };
    ...
    for (pixel, &a) in straight.chunks_exact_mut(channels).zip(alpha) {
        ...
        for component in pixel {
            let value = (u32::from(*component) * 255 + a / 2) / a;
...
fn declared_space_family(space: &pdf_page_ir::ImageColorSpace) -> pdf_color::ColorSpaceFamily {
    match space {
        ...
        pdf_page_ir::ImageColorSpace::TintLut { .. }
        | pdf_page_ir::ImageColorSpace::TintLut2 { .. } => pdf_color::ColorSpaceFamily::DeviceN,
        pdf_page_ir::ImageColorSpace::Lab { .. } => pdf_color::ColorSpaceFamily::Lab,
        pdf_page_ir::ImageColorSpace::IccRgb { .. }
        | pdf_page_ir::ImageColorSpace::IccCmyk { .. } => pdf_color::ColorSpaceFamily::IccBased,
    }
}
...
fn decode_codec_samples(
    ...
    let (color_space, comps, bpc, output_format_counter) = match img.format {
        ...
        DecodedFormat::Rgba8 => (
            pdf_page_ir::ImageColorSpace::Rgb,
            4,
            8,
            "codec.output.rgba8",
        ),
    ...
    let (samples, alpha) = match img.format {
        ...
        DecodedFormat::Rgba8 | DecodedFormat::GrayA8 => {
            let n = if matches!(img.format, DecodedFormat::Rgba8) {
                4
            } else {
                2
            };
...
fn push_clip(
    ...
    // A single-subpath axis-aligned rectangle → pure rect clip (bounds only, no
    // mask), whether or not its edges are pixel-aligned. A per-clip full-region
    // `Alpha8` mask for every fractional-edge `re W n` is what makes clip-dense
    // vector pages (music notation, maps) take seconds/page or time out.
    let is_rect = range_end - range_start == 1 && is_axis_aligned_rect(&out.points[pt_start..]);
    ...
    let mask_source = if matches!(kind, ClipKind::Path { .. }) {
        Some(cid)
    } else {
        parent.and_then(|p| out.clips[p as usize].mask_source)
    };
...
/// Finish a tiling fill whose device-space geometry has already been appended
/// to the shared point/subpath arena. Paths use this after flattening; text
/// uses it after appending glyph outlines, so both forms share identical
/// bounds, lattice enumeration, clipping, and execution.
#[allow(clippy::too_many_arguments)]
fn finish_tiled_fill(
    out: &mut CpuPreparedPage,
    range_start: u32,
    pt_start: usize,
    rule: pdf_page_ir::FillRule,
    ...
    out.ops.push(PreparedOp::TiledFill(Box::new(PreparedTiling {
        ...
        under: recolored_bytes(&tiling.under_color, color_policy),
        color_policy,
        tiles,
        blend,
        stencil,
...
fn try_cached_glyph_run(
    ...
    let hinted = uniform && !prog.is_type1() && out.hinting.should_hint(ppem as f32, axis_aligned);
    ...
    if !miss_keys.is_empty() {
        let miss_gids: Vec<u32> = miss_keys
            .iter()
            .map(|&(gi, _)| run.glyphs[gi].glyph)
            .collect();
        ...
        for (&(gi, key), outline) in miss_keys.iter().zip(&extracted) {
            ...
            let bitmap = match outline {
                ...
                Some(outline) => rasterize_glyph_bitmap(
                    ...
                    |p| {
                        [
                            (la * p[0] as f64 + fox) as f32,
                            (ld * p[1] as f64 + foy) as f32,
                        ]
                    },
                    raster,
                ),
    ...
    let rgb = [to_u8(color.r), to_u8(color.g), to_u8(color.b)];
    out.ops
        .push(PreparedOp::GlyphRun(Box::new(PreparedGlyphRun {
            origin: pdf_page_ir::PaintOrigin::default(),
            placements,
            rgb,
            alpha,
...
fn lower_glyph_outlines(
    ...
    let range_start = out.subpaths.len() as u32;
    let pt_start = out.points.len();
    append_glyph_outlines(out, run, prog, ctm, synth);
    finish_glyph_fill(out, range_start, pt_start, clip, color, op_alpha, blend)
}
...
/// varies at every destination pixel.
#[allow(clippy::too_many_arguments)]
fn lower_tiled_glyph_run(
    out: &mut CpuPreparedPage,
    run: &GlyphRun,
    program: Option<&FontProgram>,
    ctm: Matrix,
    ...
    let pt_start = out.points.len();
    match program {
        Some(prog) => append_glyph_outlines(out, run, prog, ctm, synth),
        None => append_glyph_boxes(out, run, ctm),
    }
    finish_tiled_fill(
        out,
...
/// Append real glyph outlines to the shared device-space geometry arena
/// without emitting a paint operation. Solid and shading fills finish this as
/// a normal glyph draw; tiling fills finish it as a pattern-shaped path.
...
fn lower_glyph_stroke(
    ...
    lower_stroke(
        out, &path, ctm, style, color, op_alpha, clip, blend, shading,
    )
}
...
/// Whether the four flattened points form an axis-aligned rectangle at *any*
/// (fractional) device coordinates — every corner sits on both an x- and a
/// y-extreme of their bounding box. Unlike [`is_integer_rect`] this does not
/// require pixel alignment, so a rect clip whose edges fall mid-pixel (the norm
/// at fractional scales) still takes the cheap bounds-only clip path instead of
/// a per-clip full-region `Alpha8` mask. The clip edge is then hard to the
/// conservative integer bounds (≤1 px looser than a rasterised AA mask) — a
/// trade that turns clip-dense vector pages (music, maps: thousands of `re W n`)
/// from seconds-per-page/timeout into milliseconds.
fn is_axis_aligned_rect(points: &[[f32; 2]]) -> bool {
    if points.len() != 4 {
        return false;
    }
    let (mut x0, mut y0, mut x1, mut y1) = (
    ...
    // Reject degenerate (zero-area) "rectangles" — a line/point is not a rect.
    ...
    points
        .iter()
        .all(|p| (near(p[0], x0) || near(p[0], x1)) && (near(p[1], y0) || near(p[1], y1)))
}
...
fn rgb8_linear(rgb: [u8; 3]) -> [f32; 3] {
    rgb.map(|component| srgb_to_linear(component as f32 / 255.0))
}

fn recolor(
    color: pdf_page_ir::Color,
    policy: pdf_render_api::RenderColorPolicy,
) -> pdf_page_ir::Color {
    use pdf_render_api::RenderColorPolicy;
    let source = [
    ...
    let mapped = match policy {
        RenderColorPolicy::Original => return color,
        RenderColorPolicy::WarmPaper { paper_rgb } => {
            let paper = rgb8_linear(paper_rgb);
            [
                source[0] * paper[0],
                source[1] * paper[1],
                source[2] * paper[2],
        ...
        RenderColorPolicy::Night {
            paper_rgb,
            text_rgb,
        } => {
            let paper = rgb8_linear(paper_rgb);
            let text = rgb8_linear(text_rgb);
            let luma =
                (source[0] * 0.2126 + source[1] * 0.7152 + source[2] * 0.0722).clamp(0.0, 1.0);
            [
                text[0] + (paper[0] - text[0]) * luma + (source[0] - luma) * 0.35,
                text[1] + (paper[1] - text[1]) * luma + (source[1] - luma) * 0.35,
                text[2] + (paper[2] - text[2]) * luma + (source[2] - luma) * 0.35,
            ]
        }
    };
    pdf_page_ir::Color {
...
fn recolored_bytes(
    ...
    color_to_bytes(&recolor(*color, policy))
}
...
fn prepare_shading(
    ...
    match &res.kind {
        ShadingKind::MeshTriangles {
            ...
            let bg = background_bytes(background, honor_background, color_policy);
            return prepare_mesh_layer(
                triangles.iter().map(|t| {
                    [
                        device_vertex(&t[0], to_device, color_policy),
                        device_vertex(&t[1], to_device, color_policy),
                        device_vertex(&t[2], to_device, color_policy),
                    ]
                }),
                bg,
                out_size,
        ...
        ShadingKind::MeshPatches {
            ...
            for p in patches.iter() {
                tessellate_patch(p, to_device, color_policy, &mut triangles);
            }
    ...
    match &res.kind {
        ShadingKind::FunctionGrid {
            ...
            Some(PreparedShading {
                ...
                ramp: colors
                    .iter()
                    .map(|color| recolored_bytes(color, color_policy))
                    .collect(),
                extend: [false, false],
                background: background_bytes(background, honor_background, color_policy),
                bbox: device_bbox,
            })
        }
        ShadingKind::Axial {
...
fn device_vertex(
    ...
    let color = recolor(v.color, color_policy);
    DevVertex {
        x: p.x,
        y: p.y,
        rgba: [
...
fn lower_shading_op(
    ...
    let shading = prepare_shading(res, ctm, out.size, false, color_policy)?;
...
mod font_cache_tests {
    ...
    fn device_bounds_sane_rejects_corrupt_geometry() {
        ...
        assert_eq!(
            device_bounds_sane(&[[10.0, 10.0], [50.0, 40.0]], size),
            Some((10, 10, 50, 40))
        );
        ...
        assert_eq!(
            device_bounds_sane(&[[0.0, 0.0], [f32::NAN, 5.0]], size),
            None
        );
    ...
    #[test]
    fn axis_aligned_rect_accepts_fractional_edges_rejects_rotated() {
        // A fractional-edge axis-aligned rectangle (the norm at scale != 1) —
        // the case is_integer_rect rejects, forcing a full-region mask.
        let frac = [
            [12.24, 12.06],
            [599.76, 12.06],
            [599.76, 780.5],
        ...
        assert!(
            is_axis_aligned_rect(&frac),
            "fractional rect must be a rect clip"
        );
        assert!(
            !is_integer_rect(&frac, 12.24, 12.06, 599.76, 780.5),
            "sanity: not integer"
        );
        // A rotated rectangle is not axis-aligned → stays a mask.
        let rot = [[10.0, 0.0], [20.0, 10.0], [10.0, 20.0], [0.0, 10.0]];
        assert!(
            !is_axis_aligned_rect(&rot),
            "rotated quad is not axis-aligned"
        );
        // A degenerate zero-area "rect" (a line) is not a rect.
        let line = [[0.0, 5.0], [100.0, 5.0], [100.0, 5.0], [0.0, 5.0]];
        assert!(
            !is_axis_aligned_rect(&line),
            "zero-area shape is not a rect"
        );
        // Wrong point count.
        assert!(!is_axis_aligned_rect(&[[0.0, 0.0], [1.0, 1.0]]));
    }
    ...
    fn identity_is_content_not_pointer() {
        ...
        assert_ne!(
            bytes.as_ptr(),
            independent.as_ptr(),
            "genuinely distinct allocations"
        );
        assert_eq!(
...
mod glyph_cache_tests {
    ...
    fn key_distinct_for_hinting_and_transform() {
        ...
        assert!(
            cache.get(&phased).is_none(),
            "sub-pixel phase is part of key"
        );

        // Different glyph index → distinct slot.
        let other = GlyphCacheKey {
            glyph: 6,
            ..unhinted
        };
    ...
    fn empty_outline_yields_empty_bitmap() {
        ...
        assert!(
            place_glyph(bmp, (10, 10)).is_none(),
            "empty glyph places nothing"
        );
    }
}
...
mod image_cache_tests {
    ...
    #[test]
    fn dct_nearest_hint_uses_continuous_tone_magnification_smoothing() {
        let prepared = lower_page(&dct_page(jpeg_payload(17)), None);
        let interpolation = prepared.ops.iter().find_map(|op| match op {
            PreparedOp::Image(image) => Some(image.interpolation),
            _ => None,
        });
        assert_eq!(
            interpolation,
            Some(InterpolationMode::Bilinear),
            "JPEG scans follow PDFium/MuPDF's magnification policy"
        );
    }
