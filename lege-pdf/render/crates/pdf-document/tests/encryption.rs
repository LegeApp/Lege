...
fn rc4_strings_and_streams_round_trip() {
    ...
    let obj6 = snapshot
        .objects()
        .resolve(&snapshot, ObjectId::new(6, 0), &mut ctx)
        .unwrap();
    ...
    let obj5 = snapshot
        .objects()
        .resolve(&snapshot, ObjectId::new(5, 0), &mut ctx)
        .unwrap();
...
fn encrypt_dict_is_never_decrypted() {
    ...
    let enc = snapshot
        .objects()
        .resolve(&snapshot, ObjectId::new(4, 0), &mut ctx)
        .unwrap();
...
fn objstm_members_are_not_double_decrypted() {
    ...
    let obj8 = snapshot
        .objects()
        .resolve(&snapshot, ObjectId::new(8, 0), &mut ctx)
        .unwrap();
    let title_id = snapshot.names().lookup(b"Title").expect("Title interned");
    match obj8.as_dict().unwrap().get(title_id).expect("has /Title") {
        PdfObject::String(s) => {
            assert_eq!(
                s.as_bytes(),
                b"Inside",
                "objstm member string was double-decrypted"
            );
        }
        other => panic!("expected string, got {other:?}"),
    }
