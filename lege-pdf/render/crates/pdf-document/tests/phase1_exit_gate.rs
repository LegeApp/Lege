...
fn probe_page(snapshot: &DocumentSnapshot, index: u32) -> PageProbe {
    ...
    let content_bytes = snapshot
        .decode_stream_data(stream, &mut ctx)
    ...
    let resources = match resources {
        PdfObject::Reference(id) => snapshot
            .objects()
            .resolve(snapshot, id, &mut ctx)
            .expect("resources"),
    ...
    let f1 = font_dict
        .get(names.lookup(b"F1").expect("F1 interned"))
        .expect("F1 entry");
    let font_id = f1.as_reference().expect("F1 is a reference");
    let font = snapshot
        .objects()
        .resolve(snapshot, font_id, &mut ctx)
        .expect("font resolution");
    let font_base = font
        .as_dict()
...
fn probe_all(snapshot: &DocumentSnapshot, threads: usize) -> Vec<PageProbe> {
    ...
    let slots: Vec<std::sync::Mutex<Option<PageProbe>>> = (0..page_count)
    ...
    results
        .into_iter()
        .map(|p| p.expect("page not probed"))
        .collect()
}
...
fn six_threads_resolve_six_pages_deterministically() {
    ...
    for (i, probe) in baseline.iter().enumerate() {
        assert_eq!(probe.object, Some(ObjectId::new(layout::page(i as u32), 0)));
...
fn garbage_prefixed_header_opens() {
    let bytes = builder::with_leading_garbage(builder::phase1_exit_fixture(3), &[0xAB; 512]);
...
fn supported_rc4_document_opens() {
    ...
    assert!(
        snapshot.security().is_some(),
        "an encrypted doc must carry a security context"
    );
}
...
fn max_pages_limit_enforced() {
    let source: Arc<dyn PdfSource> = Arc::new(OwnedBytesSource::new(builder::multipage_classic(5)));
...
fn decode_budget_limit_enforced() {
    ...
    let snapshot = DocumentSnapshot::open(source, DocumentLimits::default()).expect("open");
    let mut ctx = ParseContext::new();
...
fn truncated_page_tree_synthesizes_blank_placeholders_to_declared_count() {
    ...
    b2.add_object(
        2,
        "<</Type/Pages/Kids[3 0 R]/Count 9/MediaBox[0 0 200 200]>>",
    );
