...
fn assert_content_decrypts(snap: &DocumentSnapshot) {
    ...
    let obj = snap
        ...
        .expect("resolve contents");
    let pdf_object::PdfObject::Stream(stream) = &*obj else {
...
fn missing_password_fails_typed() {
    for name in [
        ...
        assert!(
            matches!(
                err,
                DocumentError::Security(SecurityError::PasswordRequired)
            ),
...
fn wrong_password_fails_typed() {
    for name in [
        ...
        assert!(
            matches!(
                err,
                DocumentError::Security(SecurityError::IncorrectPassword)
            ),
