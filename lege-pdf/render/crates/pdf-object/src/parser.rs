...
impl<'a> ObjectParser<'a> {
    ...
    fn parse_object_at_depth(&mut self, depth: usize) -> Result<PdfObject, SyntaxError> {
        ...
        match tok.value {
            ...
            Token::DictOpen => self
                ...
                .map(|d| PdfObject::Dictionary(Arc::new(d))),
            Token::Keyword(kw) => {
                ...
                if kw
                    .first()
                    .is_some_and(|&c| pdf_syntax::classify::is_numeric_char(c))
            ...
            Token::ArrayClose | Token::DictClose => Err(SyntaxError::UnexpectedToken {
                expected: "object",
                offset: tok.start,
            }),
    ...
    fn integer_or_reference(
        ...
        let ok = (|| -> Result<Option<PdfObject>, SyntaxError> {
            ...
            Ok(Some(PdfObject::Reference(ObjectId::new(
                number, generation,
            ))))
        })()?;
        match ok {
    ...
    fn read_stream_body(
        ...
        repairs.push(IndirectRepair::StreamLengthRepaired {
            declared,
...
mod tests {
    ...
    fn references_repair_a_missing_generation() {
        ...
        assert!(matches!(
            parse(&names, b"5000000000 0 R"),
            PdfObject::Integer(5000000000)
        ));
        assert!(matches!(
            parse(&names, b"12 99999 R"),
            PdfObject::Integer(12)
        ));
        // PDFium kInvalidObjNum (ported from SyntaxParserTest.GetInvalidReference).
        assert!(matches!(
            parse(&names, b"4294967295 0 R"),
            PdfObject::Integer(4294967295)
        ));
    }
    ...
    fn dictionaries() {
        ...
        assert_eq!(
            dict.get(names.known.type_).and_then(PdfObject::as_name),
            Some(names.known.page)
        );
        assert_eq!(
            dict.get(names.known.count).and_then(PdfObject::as_int),
            Some(3)
        );
        assert!(matches!(
            dict.get(names.known.kids),
    ...
    fn parse_indirect_ok(names: &NameTable, input: &[u8]) -> ParsedIndirect {
        let limits = SyntaxLimits::default();
        let mut p = ObjectParser::new(input, 0, &limits, names);
        p.parse_indirect(&mut |_| None)
            .expect("parse_indirect failed")
    }
    ...
    fn stream_with_correct_length() {
        ...
        assert!(parsed.repairs.is_empty());
        let PdfObject::Stream(s) = &parsed.object else {
            panic!("expected stream")
        };
        let StreamData::InSource { offset, len } = s.data else {
            panic!("expected InSource")
        };
        assert_eq!(
    ...
    fn stream_wrong_length_repaired_by_scan() {
        ...
        assert_eq!(
            parsed.repairs,
            vec![IndirectRepair::StreamLengthRepaired {
                declared: Some(3),
                actual: 11
            }]
        );
    ...
    fn stream_indirect_length_resolved() {
        ...
        let StreamData::InSource { len, .. } = s.data else {
            panic!("expected InSource")
        };
        assert_eq!(len, 4);
    }
    ...
    fn base_offset_flows_into_stream_data() {
        ...
        let StreamData::InSource { offset, .. } = s.data else {
            panic!("expected InSource")
        };
