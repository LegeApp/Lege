...
impl PaddleXEngine {
    pub fn new(model_path: &str, config: PaddleXConfig) -> Result<Self> {
        ...
        // Debug: Print all inputs and outputs
        debug_println!("Model inputs:");
        for (i, input) in session.inputs.iter().enumerate() {
            debug_println!("  [{}] {}: {:?}", i, input.name, input.input_type);
        }
        debug_println!("Model outputs:");
        for (i, output) in session.outputs.iter().enumerate() {
            debug_println!("  [{}] {}: {:?}", i, output.name, output.output_type);
        }
    ...
    fn build_session(model_path: &str) -> Result<(Session, String)> {
        ...
        fn get_provider_attempts() -> Vec<(
            ...
            if gpu_disabled {
                ...
                vec![
                    (vec![webgpu_execution_provider_dispatch()], vec!["WebGPU"]),
    ...
    /// Synchronously detects objects in a single image.
    /// Used by inference worker threads that can't use async.
    pub fn detect_single(&mut self, image: &RgbImage) -> Result<Vec<Detection>> {
        let mut inputs = self.preprocess(image)?;
        let outputs = self.run_inference_single(&mut inputs)?;
    ...
    async fn run_inference_single_async(
        ...
        let output_tensor = outputs[self.output_index].try_extract_tensor::<f32>()?;
    ...
    /// Synchronous (blocking) version of run_inference_single for worker threads
    fn run_inference_single(
        ...
        let run_options = RunOptions::new()?;
        // Use blocking run() instead of run_async()
    ...
    async fn run_inference_batch_with_indices_async(
        ...
        match &shape[..] {
            ...
            [nn, kk] => {
                anyhow::ensure!(*kk == 6, "Unexpected K ({}), expected 6", kk);
                // For batch processing, we need to split the [N, 6] output across batch_size items
    ...
    /// Core async inference that accepts a flat pre-built buffer and per-item metadata.
    async fn run_inference_batch_with_indices_from_flat_async(
        ...
        let expected = batch_size * 3 * (PADDLE_IMG_SIZE as usize) * (PADDLE_IMG_SIZE as usize);
        anyhow::ensure!(
            batch_image_data.len() == expected,
        ...
        let im_shapes: Vec<f32> = bytemuck::cast_vec(im_shapes_pairs);
        let scale_factors: Vec<f32> = bytemuck::cast_vec(scale_pairs);
        // Note: ndarray requires owned Vec for from_shape_vec; this is still better than N concatenations.
        let image_data = Array::from_shape_vec(
            (
                batch_size,
                3,
        ...
        let output_tensor = outputs[self.output_index].try_extract_tensor::<f32>()?;
        let (shape, data) = output_tensor;
        // Reuse existing shape handling logic
        let mut results = Vec::with_capacity(batch_size);
        match &shape[..] {
            [bb, nn, kk] => {
                anyhow::ensure!(*kk == 6, "Unexpected K ({}), expected 6", kk);
            ...
            [nn, kk] => {
                ...
                if batch_size == 1 {
                    ...
                    for page_detections in detections_per_page {
                        if page_detections.is_empty() {
                            results.push(Array::from_shape_vec((0, 6), vec![])?);
                        } else {
                            let mut flat = Vec::with_capacity(page_detections.len() * 6);
                            for d in page_detections {
                                flat.extend_from_slice(&d);
                            }
                            results.push(Array::from_shape_vec((flat.len() / 6, 6), flat)?);
                        }
                    }
                }
            }
