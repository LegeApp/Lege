...
const JP_FONTS: &[&str] = &[
    ...
    "Hiragino Sans", "Hiragino Kaku Gothic Pro", "Hiragino Mincho ProN",
    // PDFium's Linux list.
    "TakaoGothic", "VL Gothic", "IPAGothic", "Kochi Gothic",
    "TakaoPGothic", "VL PGothic", "IPAPGothic",
    "TakaoMincho", "IPAMincho", "Kochi Mincho",
    "Noto Sans CJK JP", "Noto Serif CJK JP", "Droid Sans Fallback",
];
const GB_FONTS: &[&str] = &[
    // Windows / macOS.
    "SimSun", "NSimSun", "SimHei", "Microsoft YaHei", "DengXian",
    "FangSong", "KaiTi", "PingFang SC", "STHeiti", "STSong", "Songti SC",
    // Linux.
...
const B5_FONTS: &[&str] = &[
    ...
    "Noto Sans CJK TC", "Noto Serif CJK TC", "Droid Sans Fallback",
];
const HANGUL_FONTS: &[&str] = &[
    // Windows / macOS.
    "Malgun Gothic", "Gulim", "Batang", "Dotum", "Apple SD Gothic Neo",
    // Linux.
    "UnDotum", "Noto Sans CJK KR", "Noto Serif CJK KR", "Droid Sans Fallback",
];
...
pub fn default_font_paths() -> Vec<PathBuf> {
    ...
    if let Some(local) = std::env::var_os("LOCALAPPDATA") {
        paths.push(Path::new(&local).join("Microsoft").join("Windows").join("Fonts"));
    }
    // macOS.
...
impl FolderFontProvider {
    ...
    fn pick(&self, family: &str, bold: bool, italic: bool) -> Option<SystemFont> {
        ...
        let best = faces
            ...
            .or_else(|| faces.first())?;
        Some(SystemFont { data: self.load(&best.path)?, index: best.index })
    }
...
fn index_font(path: &Path, out: &mut HashMap<String, Vec<FaceEntry>>) {
    ...
    for (face_index, font) in fonts {
        ...
        let family = name_of(NameId::TYPOGRAPHIC_FAMILY_NAME).or_else(|| name_of(NameId::FAMILY_NAME));
        ...
        out.entry(normalize(family.as_bytes()))
            .or_default()
            .push(FaceEntry { path: path.to_path_buf(), index: face_index, bold, italic });
    }
}
...
mod tests {
    ...
    fn names_normalize_across_spellings() {
        for n in ["Arial-BoldMT", "Arial,BoldMT", "arial bold mt", "ARIAL_BOLDMT"] {
    ...
    fn style_stems_are_extracted() {
        ...
        assert_eq!(style_stem(b"Cambria-BoldItalic").as_deref(), Some("Cambria"));
    ...
    fn charsets_come_from_ordering_and_names() {
        ...
        assert_eq!(Charset::from_font_name(b"DengXian-GBK-EUC-H-Identity-H"), Charset::ChineseSimplified);
        assert_eq!(Charset::from_font_name(b"CambriaMath-KSCms-UHC-H-Identity-H"), Charset::Hangul);
        assert_eq!(Charset::from_font_name(b"KozMinPro-90ms-RKSJ-H"), Charset::ShiftJis);
        assert_eq!(Charset::from_font_name(b"MSung-B5pc-H"), Charset::ChineseTraditional);
