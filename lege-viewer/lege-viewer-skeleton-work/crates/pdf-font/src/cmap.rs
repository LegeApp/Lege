...
fn synth_codespace(scheme: Scheme, lead: &[(u8, u8); 2]) -> Vec<Codespace> {
    let full = |n: u8| Codespace { n, lo: [0; 4], hi: [0xff; 4] };
    match scheme {
        ...
        Scheme::MixedTwoBytes => {
            ...
            for &(a, b) in lead {
                ...
                ranges.push(Codespace { n: 2, lo: [lo2, 0x00, 0, 0], hi: [hi2, 0xff, 0, 0] });
            }
...
pub fn parse_embedded_cmap(data: &[u8]) -> Option<CMap> {
        ...
        match tok {
            ...
            Token::Keyword(kw) => match kw.as_slice() {
                ...
                b"begincidrange" => {
                    // Triples: <lo> <hi> cid.
                    loop {
                        let Some(Token::Hex(lo)) = lex.next() else { break };
                        let Some(Token::Hex(hi)) = lex.next() else { break };
                        let Some(Token::Number(cid)) = lex.next() else { break };
                ...
                b"begincidchar" => {
                    // Pairs: <code> cid.
                    loop {
                        let Some(Token::Hex(code)) = lex.next() else { break };
                        let Some(Token::Number(cid)) = lex.next() else { break };
                        if code.len() <= 4 {
    ...
    if codespace.is_empty() {
        if let Some(base) = &usecmap {
            ...
            codespace.push(Codespace { n: 2, lo: [0; 4], hi: [0xff; 4] });
        }
    }
...
fn is_delim(b: u8) -> bool {
    is_ws(b) || matches!(b, b'<' | b'>' | b'/' | b'[' | b']' | b'{' | b'}' | b'(' | b')' | b'%')
}
...
mod tests {
    ...
    fn unknown_predefined_name_is_none() {
        ...
        assert!(predefined_cmap(b"Identity-H").is_none(), "Identity is not a table");
    }
