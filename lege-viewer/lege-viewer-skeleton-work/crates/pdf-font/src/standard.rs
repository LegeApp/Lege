...
impl StandardFont {
    ...
    pub fn family(self) -> Family {
        ...
        match self {
            ...
            F::Courier | F::CourierBold | F::CourierOblique | F::CourierBoldOblique => Family::Fixed,
...
pub fn standard_by_name(name: &[u8]) -> Option<StandardFont> {
    ...
    let normalized: Vec<u8> = lower.iter().copied().filter(|b| !b" -_,".contains(b)).collect();
    ...
    let f = match s {
        ...
        b"helveticaboldoblique" | b"helveticabolditalic" | b"arialbolditalic"
        ...
        b"courierboldoblique" | b"courierbolditalic" | b"couriernewbolditalic"
        ...
        b"timesbold" | b"timesnewromanbold" | b"timesnewromanpsboldmt"
        | b"timesnewromanpsbold" => F::TimesBold,
        b"timesitalic" | b"timesnewromanitalic" | b"timesnewromanpsitalicmt"
        | b"timesnewromanpsitalic" => F::TimesItalic,
        b"timesbolditalic" | b"timesnewromanbolditalic" | b"timesnewromanpsbolditalicmt"
...
pub fn synthesis(chosen: StandardFont, want_bold: bool, want_italic: bool) -> Synthesis {
    Synthesis {
        oblique_shear: if want_italic && !chosen.is_italic() { SYNTHETIC_OBLIQUE_TAN } else { 0.0 },
...
mod tests {
    ...
    fn standard_names_and_pdfium_aliases_resolve() {
        for (name, want) in [
            ...
            (b"TimesNewRomanPS-BoldItalicMT", StandardFont::TimesBoldItalic),
            ...
            (b"ABCDEF+Arial,BoldItalic", StandardFont::HelveticaBoldOblique),
        ] {
            assert_eq!(standard_by_name(name), Some(want), "{}", String::from_utf8_lossy(name));
        }
    ...
    fn unknown_fonts_fall_back_by_name_and_flags() {
        let by_name = |n: &[u8]| substitute(SubstitutionRequest { base_font: n, ..Default::default() });
        ...
        assert_eq!(by_name(b"SomeMono-BoldItalic"), StandardFont::CourierBoldOblique);
    ...
    fn every_bundled_face_parses_and_has_glyphs() {
        for f in [
            ...
            let prog = f.program().unwrap_or_else(|| panic!("{} must parse", f.pdf_name()));
    ...
    fn synthesis_is_only_needed_for_the_symbolic_faces() {
        ...
        assert_eq!(synthesis(StandardFont::HelveticaBoldOblique, true, true), Synthesis::default());
        assert_eq!(synthesis(StandardFont::TimesRoman, false, false), Synthesis::default());
