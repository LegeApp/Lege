...
pub fn cid_to_gid_from_cff(data: &[u8]) -> Option<CidToGid> {
    ...
    let n_glyphs = {
        let mut cr = Reader { data, pos: charstrings_off };
        cr.index()?.len()
    };
...
fn parse_charset(data: &[u8], offset: usize, n_glyphs: usize) -> Option<Vec<u16>> {
    ...
    match format {
        ...
        1 | 2 => {
            while out.len() < n_glyphs {
                let first = r.u16()?;
                let n_left = if format == 1 { r.u8()? as u32 } else { r.u16()? as u32 };
                for i in 0..=n_left {
                    if out.len() >= n_glyphs {
                        break;
                    }
...
fn build_cmap(encoding: &[(u8, u16)]) -> Vec<u8> {
    ...
    let mut segs: Vec<(u16, u16)> = encoding.iter().map(|&(c, g)| (0xF000 | c as u16, g)).collect();
...
fn read_cff_info(data: &[u8]) -> Option<CffInfo> {
    ...
    let charstrings_off = dict.iter().find(|(op, _)| *op == 17)?.1.first().copied()? as usize;
    let n_glyphs = Reader { data, pos: charstrings_off }.index()?.len();
    ...
    let names = if is_cid {
        ...
        let charset_off = dict.iter().find(|(op, _)| *op == 15).and_then(|(_, v)| v.first().copied());
        match charset_off {
            ...
            _ => (0..n_glyphs).map(|gid| sid_to_name(gid as u16, &strings)).collect(),
        }
    };
    ...
    let encoding = if is_cid {
        ...
        match dict.iter().find(|(op, _)| *op == 16).and_then(|(_, v)| v.first().copied()) {
            Some(off) if off > 1 => parse_cff_encoding(data, off as usize, n_glyphs).unwrap_or_default(),
            Some(1) => Vec::new(),
            _ => standard_encoding_pairs(&names),
        }
    };

    Some(CffInfo { n_glyphs, units_per_em, bbox, names, encoding })
}
...
fn sid_to_name(sid: u16, strings: &[&[u8]]) -> Vec<u8> {
    ...
    if sid < CFF_STANDARD_STRINGS.len() {
        ...
        strings.get(sid - CFF_STANDARD_STRINGS.len()).map(|s| s.to_vec()).unwrap_or_default()
    }
}
...
pub(crate) const CFF_STANDARD_STRINGS: [&[u8]; 391] = [
    ...
    b"percent", b"ampersand", b"quoteright", b"parenleft", b"parenright", b"asterisk",
    b"plus", b"comma", b"hyphen", b"period", b"slash", b"zero",
    b"one", b"two", b"three", b"four", b"five", b"six",
    b"seven", b"eight", b"nine", b"colon", b"semicolon", b"less",
    b"equal", b"greater", b"question", b"at", b"A", b"B",
    b"C", b"D", b"E", b"F", b"G", b"H",
    b"I", b"J", b"K", b"L", b"M", b"N",
    b"O", b"P", b"Q", b"R", b"S", b"T",
    b"U", b"V", b"W", b"X", b"Y", b"Z",
    b"bracketleft", b"backslash", b"bracketright", b"asciicircum", b"underscore", b"quoteleft",
    ...
    b"exclamdown", b"cent", b"sterling", b"fraction", b"yen", b"florin",
    b"section", b"currency", b"quotesingle", b"quotedblleft", b"guillemotleft", b"guilsinglleft",
    b"guilsinglright", b"fi", b"fl", b"endash", b"dagger", b"daggerdbl",
    b"periodcentered", b"paragraph", b"bullet", b"quotesinglbase", b"quotedblbase", b"quotedblright",
    b"guillemotright", b"ellipsis", b"perthousand", b"questiondown", b"grave", b"acute",
    b"circumflex", b"tilde", b"macron", b"breve", b"dotaccent", b"dieresis",
    b"ring", b"cedilla", b"hungarumlaut", b"ogonek", b"caron", b"emdash",
    b"AE", b"ordfeminine", b"Lslash", b"Oslash", b"OE", b"ordmasculine",
    b"ae", b"dotlessi", b"lslash", b"oslash", b"oe", b"germandbls",
    b"onesuperior", b"logicalnot", b"mu", b"trademark", b"Eth", b"onehalf",
    b"plusminus", b"Thorn", b"onequarter", b"divide", b"brokenbar", b"degree",
    b"thorn", b"threequarters", b"twosuperior", b"registered", b"minus", b"eth",
    b"multiply", b"threesuperior", b"copyright", b"Aacute", b"Acircumflex", b"Adieresis",
    ...
    b"Ydieresis", b"Zcaron", b"aacute", b"acircumflex", b"adieresis", b"agrave",
    b"aring", b"atilde", b"ccedilla", b"eacute", b"ecircumflex", b"edieresis",
    b"egrave", b"iacute", b"icircumflex", b"idieresis", b"igrave", b"ntilde",
    b"oacute", b"ocircumflex", b"odieresis", b"ograve", b"otilde", b"scaron",
    b"uacute", b"ucircumflex", b"udieresis", b"ugrave", b"yacute", b"ydieresis",
    b"zcaron", b"exclamsmall", b"Hungarumlautsmall", b"dollaroldstyle", b"dollarsuperior", b"ampersandsmall",
    b"Acutesmall", b"parenleftsuperior", b"parenrightsuperior", b"twodotenleader", b"onedotenleader", b"zerooldstyle",
    b"oneoldstyle", b"twooldstyle", b"threeoldstyle", b"fouroldstyle", b"fiveoldstyle", b"sixoldstyle",
    b"sevenoldstyle", b"eightoldstyle", b"nineoldstyle", b"commasuperior", b"threequartersemdash", b"periodsuperior",
    b"questionsmall", b"asuperior", b"bsuperior", b"centsuperior", b"dsuperior", b"esuperior",
    ...
    b"parenrightinferior", b"Circumflexsmall", b"hyphensuperior", b"Gravesmall", b"Asmall", b"Bsmall",
    b"Csmall", b"Dsmall", b"Esmall", b"Fsmall", b"Gsmall", b"Hsmall",
    b"Ismall", b"Jsmall", b"Ksmall", b"Lsmall", b"Msmall", b"Nsmall",
    b"Osmall", b"Psmall", b"Qsmall", b"Rsmall", b"Ssmall", b"Tsmall",
    b"Usmall", b"Vsmall", b"Wsmall", b"Xsmall", b"Ysmall", b"Zsmall",
    b"colonmonetary", b"onefitted", b"rupiah", b"Tildesmall", b"exclamdownsmall", b"centoldstyle",
    b"Lslashsmall", b"Scaronsmall", b"Zcaronsmall", b"Dieresissmall", b"Brevesmall", b"Caronsmall",
    b"Dotaccentsmall", b"Macronsmall", b"figuredash", b"hypheninferior", b"Ogoneksmall", b"Ringsmall",
    b"Cedillasmall", b"questiondownsmall", b"oneeighth", b"threeeighths", b"fiveeighths", b"seveneighths",
    b"onethird", b"twothirds", b"zerosuperior", b"foursuperior", b"fivesuperior", b"sixsuperior",
    b"sevensuperior", b"eightsuperior", b"ninesuperior", b"zeroinferior", b"oneinferior", b"twoinferior",
    b"threeinferior", b"fourinferior", b"fiveinferior", b"sixinferior", b"seveninferior", b"eightinferior",
    b"nineinferior", b"centinferior", b"dollarinferior", b"periodinferior", b"commainferior", b"Agravesmall",
    b"Aacutesmall", b"Acircumflexsmall", b"Atildesmall", b"Adieresissmall", b"Aringsmall", b"AEsmall",
    b"Ccedillasmall", b"Egravesmall", b"Eacutesmall", b"Ecircumflexsmall", b"Edieresissmall", b"Igravesmall",
    b"Iacutesmall", b"Icircumflexsmall", b"Idieresissmall", b"Ethsmall", b"Ntildesmall", b"Ogravesmall",
    b"Oacutesmall", b"Ocircumflexsmall", b"Otildesmall", b"Odieresissmall", b"OEsmall", b"Oslashsmall",
    b"Ugravesmall", b"Uacutesmall", b"Ucircumflexsmall", b"Udieresissmall", b"Yacutesmall", b"Thornsmall",
    b"Ydieresissmall", b"001.000", b"001.001", b"001.002", b"001.003", b"Black",
    b"Bold", b"Book", b"Light", b"Medium", b"Regular", b"Roman",
    b"Semibold",
];
