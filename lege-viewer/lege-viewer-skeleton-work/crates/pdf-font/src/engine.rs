    ...
    pub fn embolden(&mut self, strength: f32) {
        ...
        for &(s, e) in &contours {
            ...
            for i in 0..n {
                ...
                let Some(din) = norm([cur[0] - prev[0], cur[1] - prev[1]]) else { continue };
                let Some(dout) = norm([next[0] - cur[0], next[1] - cur[1]]) else { continue };
...
impl FontProgram {
    ...
    pub fn parse_indexed(data: Arc<[u8]>, index: u32) -> Option<Self> {
        ...
        if let Some((units_per_em, num_glyphs)) = sfnt.filter(|&(upem, _)| valid_units_per_em(upem))
        {
            return Some(Self {
                ...
                units_per_em,
                num_glyphs,
                kind: ProgramKind::Sfnt { index, wrapped_bare_cff: false },
            ...
            && let Some(wrapped) = crate::cff::wrap_bare_cff(&data)
        {
                ...
                info.filter(|&(upem, _)| valid_units_per_em(upem))
            {
                return Some(Self {
                    ...
                    units_per_em,
                    num_glyphs,
                    kind: ProgramKind::Sfnt { index: 0, wrapped_bare_cff: true },
                    name_index: Arc::default(),
                    gid_to_char: Arc::default(),
                });
            }
    ...
    pub fn benefits_from_parse_cache(&self) -> bool {
        matches!(
            self.kind,
            ProgramKind::Type1(_) | ProgramKind::Sfnt { wrapped_bare_cff: true, .. }
        )
    }
    ...
    pub fn outlines(&self, glyph_ids: &[u32]) -> Vec<Option<Outline>> {
        ...
        glyph_ids.iter().map(|&g| Self::outline_from(&glyphs, g)).collect()
    }
        ...
        Some(
            glyph_ids
                ...
                .map(|&g| {
                    ...
                    glyph.draw(DrawSettings::hinted(&instance, false), &mut outline).ok()?;
    ...
    pub fn char_for_gid(&self, glyph_id: u32) -> Option<char> {
        ...
        let map = self.gid_to_char.get_or_init(|| {
            let mut map: std::collections::HashMap<u32, char> =
    ...
    fn outline_from(glyphs: &skrifa::outline::OutlineGlyphCollection, glyph_id: u32) -> Option<Outline> {
        let glyph = glyphs.get(GlyphId::new(glyph_id))?;
        let mut out = Outline::default();
        glyph
            .draw(DrawSettings::unhinted(Size::unscaled(), LocationRef::default()), &mut out)
            .ok()?;
...
mod tests {
    ...
    fn embolden_grows_outer_contours_and_shrinks_holes() {
        ...
        let sq = |a: f32, b: f32, ccw: bool| -> Vec<[f32; 2]> {
            ...
            if ccw { pts } else { pts.into_iter().rev().collect() }
        };
        ...
        assert!(o.points[0][0] < 100.0 && o.points[0][1] < 100.0, "{:?}", o.points[0]);
        assert!(o.points[0][0] > 100.0 - 71.0, "bounded miter: {:?}", o.points[0]);
        ...
        for p in &o.points[4..8] {
            ...
            assert!(dx.abs() < 100.0 - 20.0 && dy.abs() < 100.0 - 20.0,
                "hole must shrink: {p:?}");
        }
    }
    ...
    fn mac_only_cmap_resolves_through_the_native_fallback() {
        ...
        let prog = FontProgram::parse(
            pdf_test_support::fonts::minimal_ttf_mac_cmap_only().into(),
        )
        .expect("parse");
        assert_eq!(prog.gid_for_code(0x41), Some(1), "raw byte through (1,0)");
        assert_eq!(prog.gid_for_char('A'), Some(1), "unicode -> MacRoman -> (1,0)");
...
mod hinting_tests {
    ...
    fn helvetica() -> FontProgram {
        StandardFont::Helvetica.program().expect("bundled face parses")
    }
    ...
    fn policy_gates_on_ppem_and_alignment() {
        ...
        for p in [HintingPolicy::None, HintingPolicy::Embedded, HintingPolicy::Auto] {
            assert!(!p.should_hint(12.0, false), "{p:?} must not hint skewed text");
    ...
    fn hinted_outlines_are_in_pixels_and_differ_from_unhinted() {
        ...
        let hinted = f.outlines_hinted(&[gid], ppem).expect("hinting instance builds")[0]
            .clone()
        ...
        let extent = |o: &Outline| {
            o.points.iter().fold(0f32, |m, p| m.max(p[0].abs()).max(p[1].abs()))
        };
        assert!(extent(&unhinted) > 100.0, "design units");
        assert!(extent(&hinted) < 30.0, "pixels at 12ppem: {}", extent(&hinted));
    ...
    fn hinting_snaps_stems_to_the_grid() {
        ...
        let off_grid = |o: &Outline| {
            ...
            o.points.iter().map(|p| (p[1] - p[1].round()).abs()).sum::<f32>() / n
        };
        let scaled = {
    ...
    fn hinting_is_deterministic() {
        let f = helvetica();
        let gids: Vec<u32> = "Hamburg".chars().filter_map(|c| f.gid_for_char(c)).collect();
