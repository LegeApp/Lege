...
fn a_bare_cff_is_recognized_and_an_sfnt_is_not() {
    ...
    assert!(!is_bare_cff(&StandardFont::Helvetica.program_data()), "an OTF is not bare CFF");
...
fn a_bare_cff_would_not_parse_unwrapped_but_does_wrapped() {
    ...
    assert!(skrifa::FontRef::new(&cff).is_err(), "bare CFF is not an SFNT");
...
fn glyph_names_survive_into_the_post_table() {
    ...
    for name in [&b"A"[..], b"space", b"zero", b"eacute"] {
        ...
        assert!(gid.is_some(), "name {} resolves", String::from_utf8_lossy(name));
    }
...
fn minimal_symbolic_cff() -> Vec<u8> {
    ...
    let header_len = 4;
    let prefix_len = header_len + name_index.len() + top_index.len() + string_index.len()
        + gsubr_index.len();
...
fn a_custom_encoding_becomes_a_working_symbol_cmap() {
    ...
    // carry that through.
    let gid = prog.gid_for_code(0x41).expect("code 0x41 resolves via the synthesized cmap");
    ...
    assert_eq!(prog.gid_for_code(0x20), None, "unencoded code is not invented");
}
