...
use pdf_page_ir::{
    ...
    InterpolationMode, LineCap, LineJoin, Matrix, Paint, PaintId, PageComplexity, PageFeatures,
    PathData, PathId, PlacedGlyph, ResourceKey, ShadingId, ShadingKind, ShadingResource,
    StrokeStyle, StrokeStyleId, TilingId, TilingPattern, TransparencyGroup, TransparencyGroupId,
};

use crate::semantic::{SemColor, SemShading, SemShadingKind, SemTiling, SemanticOp, SemanticPage, TextElement, TextRun};
...
impl<'a> Lowerer<'a> {
    ...
    pub(crate) fn lower(mut self) -> CompiledPage {
        ...
        let shadings: Vec<ShadingResource> = self.page.shadings.iter().map(convert_shading).collect();
    ...
    fn lower_op(&mut self, op: &SemanticOp) {
        match op {
            ...
            SemanticOp::ClipText { runs } => {
                ...
                let run_ids: Vec<pdf_page_ir::GlyphRunId> =
                    runs.iter().map(|r| self.intern_glyph_run(r.index())).collect();
                self.ops.push(DisplayOp::PushClipText { runs: run_ids.into() });
                self.clip_count += 1;
            }
            ...
            SemanticOp::BeginGroup { isolated, knockout, bounds, opacity, blend } => {
                ...
                self.ops.push(DisplayOp::BeginTransparencyGroup { group: id });
            ...
            SemanticOp::BeginSoftMask { kind, transfer } => {
                self.ops
                    .push(DisplayOp::BeginSoftMask { kind: *kind, transfer: transfer.clone() });
            }
            SemanticOp::EndSoftMask => self.ops.push(DisplayOp::EndSoftMask),
    ...
    fn intern_font(&mut self, semantic_index: usize) -> FontId {
        ...
        let key = match sem.object {
            Some(obj) => ResourceKey { object_number: obj.number, generation: obj.generation, variant: 0 },
            None => ResourceKey { object_number: 0, generation: 0, variant: semantic_index as u32 },
        };
    ...
    fn intern_image(&mut self, semantic_index: usize) -> ImageId {
        ...
        let ir = ImageIr {
            ...
            color_space: sem.color_space.clone().unwrap_or(pdf_page_ir::ImageColorSpace::Gray),
    ...
    fn compute_features(&self, shadings: &[ShadingResource], tilings: &[TilingPattern]) -> PageFeatures {
        ...
        if !shadings.is_empty() || self.paints.iter().any(|p| matches!(p, Paint::Shading { .. })) {
            f |= PageFeatures::SHADINGS;
        }
        if !tilings.is_empty() || self.paints.iter().any(|p| matches!(p, Paint::Pattern { .. })) {
            f |= PageFeatures::PATTERNS;
        }
        if self.stroke_styles.iter().any(|s| !s.dash_pattern.is_empty()) {
    ...
    fn compute_complexity(&self) -> PageComplexity {
        let path_segment_count =
            self.paths.iter().map(|p| p.verbs.len() as u32).sum::<u32>();
        let glyph_count = self.glyph_runs.iter().map(|r| r.glyphs.len() as u32).sum();
        let image_pixels =
            self.images.iter().map(|i| i.width as u64 * i.height as u64).sum();
...
fn placed_glyphs(run: &TextRun, metrics: &pdf_font::FontMetrics, glyph_map: &pdf_font::GlyphMap) -> Vec<PlacedGlyph> {
            ...
            TextElement::Show(bytes) => {
                ...
                for dc in metrics.decode(bytes) {
                    ...
                    if let Some(vp) = vertical.then(|| metrics.vertical(dc.cid)).flatten() {
                        ...
                        out.push(PlacedGlyph { glyph: glyph_map.gid(dc.cid), x: cursor, y: run.rise });
...
fn resolve_paint(color: &SemColor) -> Paint {
    ...
    match color {
        ...
        SemColor::Pattern { .. } => Paint::Solid(Color { r: 0.0, g: 0.0, b: 0.0, a: 0.0 }),
        SemColor::ShadingPattern { shading, matrix } => {
            Paint::Shading { shading: ShadingId(shading.0), matrix: *matrix }
        }
        SemColor::TilingPattern { tiling, matrix } => {
            Paint::Pattern { tiling: TilingId(tiling.0), matrix: *matrix }
        }
    }
}
...
fn resource_key(object: Option<pdf_object::ObjectId>, fallback: u32) -> ResourceKey {
    match object {
        Some(o) => ResourceKey { object_number: o.number, generation: o.generation, variant: 0 },
        None => ResourceKey { object_number: 0, generation: 0, variant: fallback },
    }
}
...
fn convert_shading(s: &SemShading) -> ShadingResource {
    ...
    let ramp = |r: &[[f32; 4]]| -> std::sync::Arc<[Color]> { r.iter().map(|c| color4(*c)).collect() };
    let (shading_type, kind) = match &s.kind {
        SemShadingKind::Axial { coords, domain, extend, ramp: r, background } => (
            2u8,
            ShadingKind::Axial {
                coords: [coords[0] as f32, coords[1] as f32, coords[2] as f32, coords[3] as f32],
        ...
        SemShadingKind::Radial { coords, domain, extend, ramp: r, background } => (
            3u8,
            ShadingKind::Radial {
                coords: [
                    coords[0] as f32, coords[1] as f32, coords[2] as f32,
                    coords[3] as f32, coords[4] as f32, coords[5] as f32,
                ],
                domain: [domain[0] as f32, domain[1] as f32],
                extend: *extend,
                ramp: ramp(r),
        ...
        SemShadingKind::MeshTriangles { shading_type, triangles, background } => (
            *shading_type,
            ShadingKind::MeshTriangles {
                triangles: triangles
                    .iter()
                    .map(|t| {
                        [mesh_vertex_ir(&t[0]), mesh_vertex_ir(&t[1]), mesh_vertex_ir(&t[2])]
                    })
                    .collect(),
                background: background.map(color4),
            },
        ),
        SemShadingKind::MeshPatches { shading_type, patches, background } => (
            *shading_type,
        ...
        SemShadingKind::FunctionGrid { domain, matrix, grid_w, grid_h, colors, background } => (
            1u8,
        ...
        SemShadingKind::Unsupported { background } => {
            (0u8, ShadingKind::Unsupported { background: background.map(color4) })
        }
    };
    ShadingResource { key, shading_type, kind, bbox: s.bbox }
}

fn mesh_vertex_ir(v: &crate::semantic::SemMeshVertex) -> pdf_page_ir::MeshVertex {
    pdf_page_ir::MeshVertex { x: v.x as f32, y: v.y as f32, color: color4(v.color) }
}
...
fn convert_tiling(t: &SemTiling) -> TilingPattern {
    ...
    TilingPattern {
        ...
        bbox: [t.bbox[0] as f32, t.bbox[1] as f32, t.bbox[2] as f32, t.bbox[3] as f32],
