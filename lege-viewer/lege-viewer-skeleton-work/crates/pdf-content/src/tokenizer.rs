...
pub enum Lexeme {
    ...
    InlineImage { dict: Vec<(Vec<u8>, Operand)>, data: Vec<u8> },
}
...
impl<'a> ContentLexer<'a> {
    pub fn new(input: &'a [u8], limits: &'a SyntaxLimits) -> Self {
        Self { lexer: Lexer::new(input, 0, limits), limits }
    }
    ...
    fn operand_from_token(&mut self, tok: Token, depth: usize) -> Result<Operand, ContentError> {
        ...
        match tok {
            ...
            Token::ArrayClose | Token::DictClose => {
                Err(ContentError::Malformed("stray array/dict close in operands".into()))
            }
            Token::Keyword(k) => Err(ContentError::Malformed(format!(
                "keyword {:?} where operand expected",
                String::from_utf8_lossy(&k)
            ))),
            Token::Eof => Err(ContentError::Malformed("end of stream inside operand".into())),
        }
    }
    ...
    fn read_inline_image(&mut self) -> Result<Lexeme, ContentError> {
        ...
        loop {
            ...
            match tok.value {
                ...
                _ => return Err(ContentError::Malformed("inline image key must be a name".into())),
            }
        }
        ...
        let is_mask = matches!(dict_get(&dict, &[b"IM", b"ImageMask"]), Some(Operand::Bool(true)));
        let width = dict_get(&dict, &[b"W", b"Width"]).and_then(Operand::as_int).unwrap_or(0);
        let height = dict_get(&dict, &[b"H", b"Height"]).and_then(Operand::as_int).unwrap_or(0);
        let mut bpc = dict_get(&dict, &[b"BPC", b"BitsPerComponent"]).and_then(Operand::as_int).unwrap_or(0);
        if is_mask {
            bpc = 1;
        }
        ...
        let (data_end, resync) = match framed {
            ...
            None => {
                ...
                let resync =
                    find_ws_bounded_ei(input, start).map(|ei| ei + 2).unwrap_or(input.len());
...
fn frame_by_length(input: &[u8], start: usize, n: usize) -> Option<(usize, usize)> {
    ...
    let resync = if input.get(j) == Some(&b'E') && input.get(j + 1) == Some(&b'I') {
        ...
        find_ws_bounded_ei(input, data_end).map(|ei| ei + 2).unwrap_or(data_end)
    };
...
mod tests {
    ...
    fn inline_image_length_frames_past_false_ei() {
        ...
        assert_eq!(out.last(), Some(&op(b"Q")), "cursor must resync past EI to Q");
    }
    ...
    fn inline_image_filtered_dct_frames_on_eoi() {
        ...
        assert_eq!(got, &payload, "DCT framing must span the whole payload to FF D9");
        assert_eq!(out.last(), Some(&op(b"Q")));
    }
