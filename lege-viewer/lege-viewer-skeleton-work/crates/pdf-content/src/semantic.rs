...
pub enum SemColor {
    ...
    ShadingPattern { shading: ShadingId, matrix: Matrix },
    /// A tiling pattern (PatternType 1) already resolved, with its `/Matrix`.
    TilingPattern { tiling: TilingId, matrix: Matrix },
}
...
pub enum SemShadingKind {
    ...
    Unsupported { background: Option<[f32; 4]> },
}
...
pub enum SemanticOp {
    ...
    SetMiterLimit(f64),
    SetDash { pattern: Vec<f64>, phase: f64 },
    ...
    Fill { path: PathId, rule: FillRule },
    Stroke { path: PathId },
    FillStroke { path: PathId, rule: FillRule },
    ...
    /// `W n` idiom).
    Clip { path: PathId, rule: FillRule },
    ...
    BeginGroup { isolated: bool, knockout: bool, bounds: Rect, opacity: f32, blend: BlendMode },
    EndGroup,
    ...
    BeginSoftMask { kind: MaskKind, transfer: Option<pdf_page_ir::TransferLut> },
    EndSoftMask,
    ClearSoftMask,
}
