...
fn dequant(raw: u32, bits: u32, range: [f32; 2]) -> f32 {
    let max = if bits >= 32 { u32::MAX as f64 } else { ((1u64 << bits) - 1) as f64 };
...
impl Reader<'_, '_> {
    ...
    fn read_color(&mut self) -> Option<[f32; 4]> {
        ...
        for i in 0..self.params.components {
            ...
            comps.push(dequant(raw, self.params.component_bits, self.params.comp[i]));
        }
...
pub(crate) fn read_free_triangles(
    ...
    let mut r = Reader { bits: BitReader::new(data), params, convert };
    let mut out = Vec::new();
    let mut tri: [SemMeshVertex; 3] =
        [SemMeshVertex { x: 0.0, y: 0.0, color: [0.0; 4] }; 3];
    let mut have = false;
    while r.bits.remaining() > 0 && out.len() < MAX_PRIMITIVES {
        let Some((flag, vertex)) = r.read_vertex(true) else { break };
        if flag == 0 {
            tri[0] = vertex;
            let mut ok = true;
...
pub(crate) fn read_lattice_triangles(
    ...
    let Some(mut prev) = read_row(&mut r) else { return Vec::new() };
    let mut out = Vec::new();
