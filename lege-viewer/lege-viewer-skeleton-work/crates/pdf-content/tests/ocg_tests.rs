...
fn compile(bytes: Vec<u8>) -> SemanticPage {
    ...
    PageCompiler::new().compile_semantic(&snap, PageIndex(0), &mut ctx).expect("compile")
}
...
fn oc_on_image_xobjects_is_honored() {
    let page = compile(doc("<</OFF[11 0 R]>>", b"q 10 0 0 10 0 0 cm /ImA Do /ImB Do Q"));
    // ImA (layer one, on) draws; ImB (layer two, off) is suppressed.
    assert_eq!(page.images.len(), 1);
    assert_eq!(
        page.ops.iter().filter(|o| matches!(o, SemanticOp::DrawImage(_))).count(),
        1
    );
}
...
fn ocmd_is_visible_when_any_member_is_on() {
    ...
    let page = compile(doc("<</OFF[11 0 R]>>", b"/OC /M1 BDC 1 0 0 rg 0 0 50 50 re f EMC"));
    assert_eq!(fills(&page), 1);
    // Both off → hidden.
    let page = compile(doc(
        "<</OFF[10 0 R 11 0 R]>>",
