...
fn fill_lowers_to_explicit_paint_op() {
    ...
    match &page.operations[0] {
        DisplayOp::FillPath { path, paint, rule, alpha, blend } => {
            assert_eq!(path.0, 0);
            assert_eq!(paint.0, 0);
...
fn identical_paints_and_paths_are_interned_once() {
    ...
    let page = compile(&single_page(b"1 0 0 rg 0 0 10 10 re f 1 0 0 rg 0 0 10 10 re f"));
...
fn distinct_colors_make_distinct_paints() {
    let page = compile(&single_page(b"1 0 0 rg 0 0 10 10 re f 0 1 0 rg 0 0 10 10 re f"));
...
fn clip_becomes_push_pop_bracketed_to_scope() {
    ...
    let page = compile(&single_page(b"q 0 0 100 100 re W n 0 0 5 5 re f Q 0 0 5 5 re f"));
...
fn fill_stroke_emits_two_ops_sharing_the_path() {
    ...
    let (fill_path, stroke_path) = match (&page.operations[0], &page.operations[1]) {
        (DisplayOp::FillPath { path: p1, .. }, DisplayOp::StrokePath { path: p2, style, .. }) => {
            assert_eq!(style.0, 0);
            (p1.0, p2.0)
        }
...
fn group_interior_alpha_resets_to_one() {
    ...
    assert_eq!(page.groups.len(), 1);
    assert!((page.groups[0].opacity - 0.5).abs() < 1e-6, "outer ca is the group opacity");
    ...
    for op in page.operations.iter() {
        match op {
            ...
            DisplayOp::FillPath { alpha, .. } if in_group => {
                assert_eq!(*alpha, 1.0, "group-interior alpha must reset (was applied twice)");
                checked = true;
            }
            _ => {}
        }
...
/DP <</K -1 /Columns 24 /BlackIs1 true /EncodedByteAlign true /Rows 2>> ID "
    ...
    assert_eq!(page.images.len(), 1);
    let parms = page.images[0].codec_parms.as_ref().expect("inline /DP must be read");
    assert_eq!(parms.k, -1);
...
/DecodeParms <</Columns 24>> ID "
    ...
    b2.add_object(2, "<</Type/Pages/Kids[3 0 R]/Count 1/MediaBox[0 0 200 200]>>");
    b2.add_object(3, "<</Type/Page/Parent 2 0 R/Contents 4 0 R/Resources<<>>>>");
    b2.add_stream(4, "", &src2);
    b2.finish_classic_xref("/Root 1 0 R");
    let page2 = compile(&open(b2.into_bytes()));
    let parms2 = page2.images[0].codec_parms.as_ref().expect("/DecodeParms read");
...
fn icc_and_overprint_feature_flags_are_computed() {
    ...
    assert!(page.features.contains(PageFeatures::ICC_COLOR), "{:?}", page.features);
    assert!(page.features.contains(PageFeatures::OVERPRINT), "{:?}", page.features);

    // A plain device-color page sets neither.
    let mut b2 = PdfBuilder::new();
    b2.add_object(1, "<</Type/Catalog/Pages 2 0 R>>");
    b2.add_object(2, "<</Type/Pages/Kids[3 0 R]/Count 1/MediaBox[0 0 100 100]>>");
    b2.add_object(3, "<</Type/Page/Parent 2 0 R/Contents 4 0 R/Resources<<>>>>");
...
fn symbolic_substitute_carries_synthetic_style() {
    ...
    assert!(bold.synthetic_embolden_em > 0.0, "bold Symbol wants embolden");
    assert_eq!(bold.synthetic_shear, 0.0);
    let italic = &page.fonts[1];
    assert!(italic.synthetic_shear > 0.2, "italic Symbol wants the 12 degree shear");
...
fn debug_dump_is_stable_and_schema_keyed() {
    ...
    assert!(a.starts_with("CompiledPage schema=1\n"), "{a}");
...
fn text_advances_use_pdf_widths() {
    ...
    b.add_object(1, "<</Type/Catalog/Pages 2 0 R>>");
    b.add_object(2, "<</Type/Pages/Kids[3 0 R]/Count 1/MediaBox[0 0 200 200]>>");
    b.add_object(3, "<</Type/Page/Parent 2 0 R/Contents 4 0 R/Resources<</Font<</F1 5 0 R>>>>>>");
    b.add_stream(4, "", b"BT /F1 10 Tf 0 0 Td (ABC) Tj ET");
    b.add_object(5, "<</Type/Font/Subtype/Type1/BaseFont/Custom/FirstChar 65/Widths[1000 500 250]>>");
    b.finish_classic_xref("/Root 1 0 R");
    let page = compile(&open(b.into_bytes()));
    ...
    assert!(g.iter().all(|gl| gl.glyph != 0), "every code resolved to a glyph: {g:?}");
...
fn missing_width_falls_back() {
    ...
    assert!((g[1].x - 10.0).abs() < 1e-6, "Z after A(width 1000): {}", g[1].x);
}
...
fn cid_identity_two_byte_advances() {
    ...
    b.add_object(5, "<</Type/Font/Subtype/Type0/BaseFont/C/Encoding/Identity-H/DescendantFonts[6 0 R]>>");
    b.add_object(6, "<</Type/Font/Subtype/CIDFontType2/BaseFont/C/DW 1000/W[1[500 250]]>>");
    b.finish_classic_xref("/Root 1 0 R");
    let page = compile(&open(b.into_bytes()));
...
fn type0_embedded_cmap_decodes_variable_width_codes() {
    ...
    b.add_object(5, "<</Type/Font/Subtype/Type0/BaseFont/C/Encoding 7 0 R/DescendantFonts[6 0 R]>>");
    ...
    let g = &page.glyph_runs[0].glyphs;
    assert_eq!(g.len(), 3, "three variable-width codes, not the 2-byte Identity split");
    ...
    assert_eq!(g[0].x, 0.0);
    assert!((g[1].x - 5.0).abs() < 1e-6, "CID10 width 500 @10pt → 5.0, got {}", g[1].x);
    assert!((g[2].x - 7.5).abs() < 1e-6, "CID20 width 250 @10pt → 2.5 more, got {}", g[2].x);
}
...
fn type0_predefined_cmap_name_maps_to_cids() {
    ...
    b.add_object(5, "<</Type/Font/Subtype/Type0/BaseFont/C/Encoding/GBK-EUC-H/DescendantFonts[6 0 R]>>");
    b.add_object(6, "<</Type/Font/Subtype/CIDFontType2/BaseFont/C/CIDToGIDMap/Identity/DW 1000>>");
    b.finish_classic_xref("/Root 1 0 R");
    let page = compile(&open(b.into_bytes()));
...
fn ext_gstate_smask_lowers_to_soft_mask_ops() {
    ...
    b.add_object(1, "<</Type/Catalog/Pages 2 0 R>>");
    b.add_object(2, "<</Type/Pages/Kids[3 0 R]/Count 1/MediaBox[0 0 100 100]>>");
    b.add_object(3, "<</Type/Page/Parent 2 0 R/Contents 4 0 R/Resources<</ExtGState<</GS0 5 0 R>>>>>>");
    b.add_stream(4, "", b"/GS0 gs 0 0 50 50 re f");
    b.add_object(5, "<</Type/ExtGState/SMask<</S/Luminosity/G 6 0 R>>>>");
    b.add_stream(6, "/Type/XObject/Subtype/Form/BBox[0 0 100 100]/Group<</S/Transparency>>", b"1 g 0 0 100 100 re f");
    b.finish_classic_xref("/Root 1 0 R");
    let page = compile(&open(b.into_bytes()));

    let has_begin = page.operations.iter().any(|o| matches!(o, DisplayOp::BeginSoftMask { .. }));
    let has_end = page.operations.iter().any(|o| matches!(o, DisplayOp::EndSoftMask));
    ...
    assert!(has_origin, "soft-mask content carries its attribution marker");
    // The masked fill (50×50 rect) is still present after the mask content.
    assert!(page.operations.iter().any(|o| matches!(o, DisplayOp::FillPath { .. })));
}
...
fn embedded_cidfont_program_and_gids_flow_to_the_ir() {
    ...
    assert_eq!(page.fonts.len(), 1);
    assert!(!page.fonts[0].program.is_empty(), "embedded program carried to IR");
    assert_eq!(page.glyph_runs[0].glyphs[0].glyph, 1, "CID 1 → GID 1 (Identity)");
}
...
fn simple_embedded_font_resolves_code_to_gid() {
    ...
    b.add_object(6, "<</Type/FontDescriptor/FontName/C/Flags 32/FontFile2 7 0 R>>");
    b.add_stream(7, "", &ttf);
    b.finish_classic_xref("/Root 1 0 R");
    let page = compile(&open(b.into_bytes()));

    assert!(!page.fonts[0].program.is_empty(), "simple embedded program carried");
    assert_eq!(page.glyph_runs[0].glyphs[0].glyph, 1, "'A' → GID 1 via encoding+cmap");
}
