...
fn truncated_content_stream_compiles_to_blank_not_error() {
    ...
    assert!(page.ops.is_empty(), "a truncated content stream should yield a blank page");
}
...
fn one_bad_content_stream_keeps_the_good_ones() {
    ...
    b.add_object(1, "<</Type/Catalog/Pages 2 0 R>>");
    b.add_object(2, "<</Type/Pages/Kids[3 0 R]/Count 1/MediaBox[0 0 200 200]>>");
    b.add_object(3, "<</Type/Page/Parent 2 0 R/Contents[4 0 R 5 0 R]/Resources<<>>>>");
    ...
    let page = compile(&snap, 0);
    assert_eq!(page.ops.len(), 2, "the valid stream's ops must survive the corrupt one");
}
...
fn lab_fill_converts_through_cie_math() {
    ...
    assert!(r > 0.98 && g > 0.98 && b > 0.98, "Lab white -> RGB white, got {r} {g} {b}");
    ...
    assert!(r < 0.02 && g < 0.02 && b < 0.02, "Lab L*=0 -> black, got {r} {g} {b}");
}
...
fn calgray_fill_is_pass_through_gray() {
    ...
    assert!((g - 0.5).abs() < 1e-6, "CalGray is pass-through per PDFium, got {g}");
}
...
fn fill_rectangle_with_rgb() {
    ...
    assert!(matches!(page.ops[0], SemanticOp::SetFillColor(SemColor::DeviceRgb(..))));
...
fn text_clip_mode_emits_cliptext_at_et_before_following_fill() {
    ...
    let cliptext = page.ops.iter().position(|o| matches!(o, SemanticOp::ClipText { .. }));
    let fill = page.ops.iter().position(|o| matches!(o, SemanticOp::Fill { .. }));
    ...
    assert!(cliptext < fill, "the text clip must precede the fill it constrains");
...
fn non_clip_text_mode_emits_no_cliptext() {
    ...
    let page = compile(&snap, 0);
    assert!(
        !page.ops.iter().any(|o| matches!(o, SemanticOp::ClipText { .. })),
...
fn text_show_produces_run_and_font() {
    ...
    assert_eq!(page.text_runs[0].elements, vec![TextElement::Show(b"Hello".to_vec())]);

    let text = dump(&snap, &page);
    assert!(text.contains("show-text run#0"), "{text}");
    assert!(text.contains("font#0 /F1 Type1 /Helvetica obj 5 0"), "{text}");
...
fn consecutive_show_operators_advance_the_text_matrix() {
    ...
    assert!(e[1] > 0.0, "the second run is advanced past the first (was 0 before the fix)");
    ...
    assert!((step1 - 6.67).abs() < 0.5, "advance ≈ glyph width·Tfs/1000: {step1}");
}
...
fn char_and_word_spacing_widen_the_run_advance() {
    ...
    assert!((e_spaced - e_base - 18.0).abs() < 1e-6, "base {e_base} spaced {e_spaced}");
}
...
fn unbalanced_q_is_tolerated() {
    ...
    assert!(page.ops.iter().any(|o| matches!(o, SemanticOp::Fill { .. })));
}
