...
use pdf_page_ir::{CompiledPage, DisplayOp, Paint, PageFeatures, ShadingKind};
...
fn sh_operator_builds_axial_shading_with_ramp() {
    ...
    match &page.shadings[0].kind {
        ShadingKind::Axial { coords, extend, ramp, .. } => {
    ...
    assert!(page.operations.iter().any(|op| matches!(op, DisplayOp::DrawShading { .. })));
}
...
fn sh_operator_builds_axial_shading_with_type4_function() {
    ...
    b.add_stream(6, "/FunctionType 4/Domain[0 1]/Range[0 1 0 1 0 1]", b"{ dup dup }");
    b.finish_classic_xref("/Root 1 0 R");
    let page = compile(&open(b.into_bytes()), 0);
...
fn non_unit_domain_ramp_endpoints_are_the_boundary_colors() {
    ...
    match &page.shadings[0].kind {
        ShadingKind::Axial { ramp, extend, .. } => {
            assert_eq!(*extend, [true, true]);
            assert!((ramp[0].r - 0.25).abs() < 0.01, "start boundary f(0.25): {:?}", ramp[0]);
            assert!((ramp[255].r - 0.75).abs() < 0.01, "end boundary f(0.75): {:?}", ramp[255]);
        }
        other => panic!("expected axial, got {other:?}"),
    }
}
...
fn shading_pattern_resolves_to_shading_paint() {
    ...
    let has_shading_paint = page.paints.iter().any(|p| matches!(p, Paint::Shading { .. }));
