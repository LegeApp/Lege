...
fn count_fills(page: &SemanticPage) -> usize {
    page.ops.iter().filter(|o| matches!(o, SemanticOp::Fill { .. })).count()
}
...
fn two_charprocs_draw_bracketed_fills_at_widths_positions() {
    ...
    let saves = kinds.iter().filter(|o| matches!(o, SemanticOp::Save)).count();
    let restores = kinds.iter().filter(|o| matches!(o, SemanticOp::Restore)).count();
    assert_eq!(saves, 2);
    ...
    assert!((cs[1].e - 110.0).abs() < 1e-9, "second glyph advanced by A width: {:?}", cs[1]);
...
fn d1_charproc_suppresses_inner_color() {
    ...
    assert_eq!(fill_colors.len(), 1, "inner d1 colour must be dropped: {fill_colors:?}");
    assert!(
...
fn missing_charproc_skips_glyph_but_still_advances() {
    ...
    b.add_object(6, "<</square 8 0 R>>");
    b.add_object(7, "<</Type/Encoding/Differences[65/square 66/disc 67/gap]>>");
    b.add_stream(8, "", b"1000 0 d0 0 0 100 100 re f");
    b.finish_classic_xref("/Root 1 0 R");
    let snap = open(b.into_bytes());
    let page = compile(&snap, 0);
