...
fn base_doc(b: &mut PdfBuilder, annots: &str) {
    ...
    b.add_object(2, "<</Type/Pages/Kids[3 0 R]/Count 1/MediaBox[0 0 300 300]>>");
    b.add_object(
        3,
...
fn appearance_stream_renders_with_rect_fit_matrix() {
    ...
    let plain = PageCompiler::new()
        ...
        .expect("compile failed");
    assert!(!plain.ops.iter().any(|o| matches!(o, SemanticOp::Fill { .. })));
    ...
    assert!(page.ops.iter().any(|o| matches!(o, SemanticOp::Clip { .. })));
}
...
fn hidden_and_noview_annotations_are_skipped() {
    ...
    for flags in [2, 32] {
        ...
        b.add_object(
            10,
            &format!(
                "<</Type/Annot/Subtype/Square/Rect[0 0 50 50]/F {flags}/AP<</N 11 0 R>>>>"
            ),
        );
        b.add_stream(
            11,
...
fn appearance_ignores_page_content_ending_state() {
    ...
    let saves = page.ops.iter().filter(|o| matches!(o, SemanticOp::Save)).count();
    let restores = page.ops.iter().filter(|o| matches!(o, SemanticOp::Restore)).count();
    ...
    let fit = page
        ...
        .next_back()
        .expect("fit matrix");
    assert_eq!((fit.a, fit.b, fit.c, fit.d, fit.e, fit.f), (1.0, 0.0, 0.0, 1.0, 20.0, 30.0));
}
