...
impl HuffTable {
    fn build(counts: &[u8; 16], values: Vec<u8>) -> Result<HuffTable, ImageError> {
        ...
        Ok(HuffTable { fast, max_code, min_code, val_ptr, values })
    }
}
...
impl<'a> BitReader<'a> {
    fn new(data: &'a [u8], pos: usize) -> Self {
        Self { data, pos, buf: 0, count: 0, hit_marker: false }
    }
    ...
    fn of(frame: &Frame, scan_comps: &[ScanComp]) -> ScanGeom {
        ...
        let comps: Vec<GeomComp> = scan_comps
            ...
            .map(|sc| {
                ...
                GeomComp { ci: sc.comp, h: c.h, v: c.v, bw: c.bw, bw_used: c.bw_used.max(1) }
            })
            .collect();
        ...
        ScanGeom { single, mcus, mcus_x: frame.mcus_x.max(1), row_len, comps }
    }
}
...
impl<'a> Decoder<'a> {
    ...
    fn run(&mut self) -> Result<(), ImageError> {
        ...
        let soi = self.data.windows(2).take(4096).position(|w| w == [0xFF, 0xD8]);
        ...
        loop {
            ...
            match marker {
                ...
                0xC3 | 0xC5..=0xC7 | 0xCB | 0xCD..=0xCF => {
                    return Err(err("unsupported JPEG coding process (lossless/hierarchical)"));
                }
    ...
    fn parse_sof(&mut self, progressive: bool) -> Result<(), ImageError> {
        ...
        if (scaled(width, 1) as u64) * (scaled(height, 1) as u64) > self.limits.max_pixels {
            return Err(ImageError::TooLarge { width: width as u32, height: height as u32 });
        }
    ...
    fn parse_sos_and_scan(&mut self) -> Result<(), ImageError> {
        ...
        for i in 0..ns {
            ...
            scan_comps.push(ScanComp { comp, dc_tbl, ac_tbl });
        }
        let q = p + 1 + ns * 2;
        if q + 2 >= self.data.len() {
    ...
    fn walk_scan_blocks(
        ...
        for mcu in 0..geom.mcus {
            ...
            if geom.single {
                ...
                f(reader, Event::Block { si: 0, ci: g.ci, off: (row * g.bw + col) * 64 })?;
            } else {
                ...
                for (si, g) in geom.comps.iter().enumerate() {
                    for by in 0..g.v {
                        for bx in 0..g.h {
                            ...
                            f(reader, Event::Block { si, ci: g.ci, off: (row * g.bw + col) * 64 })?;
                        }
                    }
                }
            }
    ...
    fn scan_sequential_streaming(
        ...
        // One reusable sample band per scan component: `v·8` rows × `bw·8` cols.
        ...
        let x_maps = component_x_maps(frame);
        let rgb_simd = match kind {
            RowKind::Rgb(false) => rgb_ycc_chroma_x(&x_maps, w),
            _ => None,
        };
        let stride = w * bpp;
        ...
        for my in 0..mcus_y {
                ...
                for (si, sc) in scan_comps.iter().enumerate() {
                    ...
                    for by in 0..c.v {
                        for bx in 0..c.h {
                            scratch.fill(0);
                            let any_ac =
                                decode_block_sequential(reader, dc, ac, &mut preds[si], &mut scratch);
                            ...
                            if any_ac {
                                ...
                                let byte =
                                    (scratch[0] as f32 * dq[0] + 128.5).clamp(0.0, 255.0) as u8;
                                for yy in 0..dct {
                                    band[base + yy * bstride..base + yy * bstride + dct]
                                        .fill(byte);
                                }
                            }
                        }
            ...
            // Emit every output row that falls in this MCU row.
            let y0 = my * dct * vmax;
            let y1 = ((my + 1) * dct * vmax).min(h);
            for y in y0..y1 {
                let mut rows: [&[u8]; 4] = [&[][..]; 4];
                for si in 0..ncomp {
                    let c = &frame.comps[scan_comps[si].comp];
                    let bstride = c.bw * dct;
                    let blrow = (y * c.v / vmax) - my * c.v * dct;
                    rows[si] = &bands[si][blrow * bstride..blrow * bstride + bstride];
                }
                assemble_row(kind, &mut out[y * stride..y * stride + stride], w, &rows[..ncomp], &x_maps, rgb_simd);
            }
        }
    ...
    fn scan_progressive_ac(
        ...
        let ac = self.ac_tables[sc.ac_tbl].as_ref().ok_or_else(|| err("missing AC table"))?;
...
fn component_x_maps(frame: &Frame) -> Vec<Vec<usize>> {
    frame
        .comps
        .iter()
        .map(|c| {
            (0..frame.out_w())
                .map(|x| x * c.h / frame.hmax)
                .collect()
        })
        .collect()
}
...
#[inline]
fn component_row(frame: &Frame, ci: usize, y: usize) -> usize {
    let c = &frame.comps[ci];
    (y * c.v / frame.vmax) * (c.bw * frame.dct_size)
}
...
fn output_layout(frame: &Frame, adobe: Option<u8>, saw_jfif: bool) -> (RowKind, DecodedFormat, usize) {
    match frame.comps.len() {
        ...
        _ => (RowKind::Cmyk(four_comp_is_ycck(adobe)), DecodedFormat::Cmyk8, 4),
    }
}
...
/// If a 3-component YCbCr row has SIMD-friendly horizontal addressing — luma
/// identity, both chroma maps equal and either [`ChromaX::Identity`] or
/// [`ChromaX::Half`] — report it. Computed once per image (O(w)); `None`
/// keeps the scalar path. Only meaningful for `RowKind::Rgb(false)`.
fn rgb_ycc_chroma_x(x_maps: &[Vec<usize>], w: usize) -> Option<ChromaX> {
    if x_maps.len() < 3 {
        return None;
    }
    let (xm0, xm1, xm2) = (&x_maps[0], &x_maps[1], &x_maps[2]);
    if xm1 != xm2 || !(0..w).all(|x| xm0[x] == x) {
        return None;
    }
    if (0..w).all(|x| xm1[x] == x) {
        Some(ChromaX::Identity)
    } else if (0..w).all(|x| xm1[x] == x / 2) {
        Some(ChromaX::Half)
    } else {
        None
    }
...
/// Write one output row. `rows[ci]` is component `ci`'s sample row (length
/// `≥ bw·8`); `x_maps[ci][x]` is the source sample index for output column `x`.
/// `rgb_simd` (precomputed once per image via [`rgb_ycc_chroma_x`]) selects the
/// vectorized YCbCr→RGB kernel for the eligible layouts.
#[allow(unsafe_code)]
#[inline]
fn assemble_row(
    kind: RowKind,
    ...
    rows: &[&[u8]],
    x_maps: &[Vec<usize>],
    rgb_simd: Option<ChromaX>,
) {
    match kind {
        RowKind::Gray => out_row[..w].copy_from_slice(&rows[0][..w]),
        RowKind::Multi2 => {
            let (r0, r1) = (rows[0], rows[1]);
            let (xm0, xm1) = (&x_maps[0], &x_maps[1]);
            for x in 0..w {
                let dst = x * 2;
                out_row[dst] = r0[xm0[x]];
                out_row[dst + 1] = r1[xm1[x]];
            }
        }
        RowKind::Rgb(is_rgb) => {
            ...
            #[cfg(target_arch = "x86_64")]
            if !is_rgb {
                if let Some(cx) = rgb_simd {
                    if is_x86_feature_detected!("avx2") {
                        // SAFETY: guarded by the runtime AVX2 feature probe; the
                        // kernel processes whole 8-pixel groups and returns the
                        // count it wrote, leaving the tail to the scalar loop.
                        let done = unsafe { rgb_ycc_row_avx2(cx, out_row, w, r0, r1, r2) };
                        for x in done..w {
                            store_three(out_row, x, r0[xm0[x]], r1[xm1[x]], r2[xm2[x]], false);
                        }
                        return;
                    }
                }
            ...
            for x in 0..w {
                store_three(out_row, x, r0[xm0[x]], r1[xm1[x]], r2[xm2[x]], is_rgb);
            }
        }
        RowKind::Cmyk(ycck) => {
            let (r0, r1, r2, r3) = (rows[0], rows[1], rows[2], rows[3]);
            let (xm0, xm1, xm2, xm3) = (&x_maps[0], &x_maps[1], &x_maps[2], &x_maps[3]);
            for x in 0..w {
                let px = cmyk_libjpeg_pixel(r0[xm0[x]], r1[xm1[x]], r2[xm2[x]], r3[xm3[x]], ycck);
                let dst = x * 4;
...
fn assemble(
    ...
    for y in 0..h {
        ...
        for ci in 0..n {
            let c = &frame.comps[ci];
            let start = component_row(frame, ci, y);
            rows[ci] = &planes[ci][start..start + c.bw * frame.dct_size];
        }
        assemble_row(kind, &mut out[y * stride..y * stride + stride], w, &rows[..n], &x_maps, rgb_simd);
    }
    Ok(image(w, h, format, stride, out))
}
...
fn image(w: usize, h: usize, format: DecodedFormat, stride: usize, data: Vec<u8>) -> DecodedImage {
    DecodedImage { width: w as u32, height: h as u32, format, stride, data: Arc::from(data) }
}
...
mod idct_tests {
    ...
    use super::{
        assemble_row, idct_block_scalar, output_layout, Component, DecodedFormat, Frame, RowKind,
        ZIGZAG_INV,
    };
    ...
    fn dequant_from(q: &[u16; 64]) -> [f32; 64] {
        ...
        for zig in 0..64 {
            let pos = ZIGZAG_INV[zig] as usize;
            dq[pos] =
                q[zig] as f32 * AAN_SCALE_FACTORS[pos / 8] * AAN_SCALE_FACTORS[pos % 8] / 8.0;
        }
        dq
    }
    ...
    fn avx2_matches_scalar_bit_exact() {
        ...
        for trial in 0..5000 {
            ...
            let nz = if trial % 3 == 0 { 64 } else { 1 + (next() % 12) as usize };
            for _ in 0..nz {
    ...
    fn multi2_row_interleaves_both_components() {
        ...
        let xmap: Vec<usize> = (0..4).collect();
        let x_maps = [xmap.clone(), xmap];
        let mut out = [0u8; 8];
        assemble_row(RowKind::Multi2, &mut out, 4, &[&r0, &r1], &x_maps, None);
...
mod dc_only_tests {
    ...
    use super::encoder::AAN_SCALE_FACTORS;
    use super::{idct_block, idct_block_scalar, store_band_block_scalar, ZIGZAG_INV};
    ...
    fn check(out: &[f32; 64], dc: i16, dq: &[f32; 64], q0: u16) {
        ...
        for (i, &b) in band.iter().enumerate() {
            assert_eq!(b, fast, "dc {dc} q0 {q0} sample {i}: fast byte != band-store byte");
        }
    }
...
mod rgb_simd_tests {
    ...
    use super::{rgb_ycc_row_avx2, ycc_to_rgb, ChromaX};
...
mod streaming_tests {
    ...
    fn assert_paths_identical(name: &str, data: &[u8]) {
        ...
        assert_eq!(&*s.data, &*c.data, "{name}: streaming vs coefficient bytes differ");
    }
    ...
    fn baseline_422_paths_match() {
        assert_paths_identical("base_rgb422", include_bytes!("../../tests/fixtures/base_rgb422.jpg"));
    }

    #[test]
    fn baseline_restart_gray_paths_match() {
        assert_paths_identical("restart_gray", include_bytes!("../../tests/fixtures/restart_gray.jpg"));
    }
...
mod cmyk_polarity_tests {
    ...
    fn ycck_selection_matches_libjpeg() {
        ...
        assert!(four_comp_is_ycck(Some(1)), "non-zero transform → assumed YCCK");
    }
    ...
    fn ycck_adobe_white() {
        ...
        assert_eq!(decode_1_0(raw), [0, 0, 0, 0], "true DeviceCMYK white after /Decode");
    }
    ...
    fn ycck_adobe_solid_black() {
        ...
        assert_eq!(decode_1_0(raw), [0, 0, 0, 255], "true DeviceCMYK K-black after /Decode");
    }
    ...
    fn direct_cmyk_adobe_passthrough() {
        ...
        assert_eq!(decode_1_0(raw), [255, 0, 0, 0], "true DeviceCMYK cyan after /Decode");
    }
    ...
    fn direct_cmyk_no_adobe_is_true_devicecmyk() {
        ...
        assert_eq!(raw, [0, 255, 0, 0], "no-marker CMYK passes through as true DeviceCMYK");
    }
}
...
mod assemble_tests {
    ...
    fn two_component_keeps_both_channels_as_multi2() {
        ...
        assert_eq!(img.format, DecodedFormat::Multi2, "2-comp decodes to Multi2");
