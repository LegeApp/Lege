...
use jbig2enc_rust::decode::{decode_embedded, DecodeError, DecodeOptions, DecodeStrictness};
...
mod tests {
    ...
    const JBIG2_L_12X5: &[u8] = &[
        ...
        0x00, 0x00, 0x0c, 0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x01, 0x2c, 0x00,
        0x00, 0x01, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x26, 0x00,
        0x01, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x0c, 0x00, 0x00, 0x00,
        0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03,
        0xff, 0xfd, 0xff, 0x02, 0xfe, 0xfe, 0xfe, 0xd2, 0x47, 0xc5, 0xc3, 0x1b,
        0xff, 0xff, 0xac,
    ];
    ...
    const EXPECT_L: &[u8] = &[
        0x7f, 0xff, 0x7f, 0xff, 0x7f, 0xff, 0x7f, 0xff, 0x03, 0xff,
    ];
    ...
    fn pixel_limit_is_enforced() {
        ...
        let tight = DecodeLimits { max_pixels: 4, ..DecodeLimits::default() };
        let err = Jbig2Codec.decode(
            JBIG2_L_12X5,
            &descriptor(),
            &DecodeParameters::default(),
        ...
        assert!(matches!(err, Err(ImageError::Decode(_)) | Err(ImageError::TooLarge { .. })));
    }
    ...
    fn bilevel_budget_admits_a_page_over_the_raw_pixel_cap() {
        let limits = DecodeLimits { max_pixels: 8, ..DecodeLimits::default() };
        let out = Jbig2Codec
            .decode(JBIG2_L_12X5, &descriptor(), &DecodeParameters::default(), &limits)
