...
mod limit_tests {
    ...
    fn dense_codecs_never_lose_headroom() {
        ...
        assert_eq!(l.max_pixels_at_bpp(0), l.max_pixels * 8, "0 treated as 1 bpp");
    }
}
...
pub fn registry_features(registry: &CodecRegistry) -> pdf_page_ir::PageFeatures {
    registry
        .supported_filters()
        .fold(pdf_page_ir::PageFeatures::empty(), |acc, f| acc | codec_feature(f))
}
...
impl CodecRegistry {
    ...
    pub fn get(&self, filter: StreamFilter) -> Option<&dyn ImageCodec> {
        self.codecs.iter().find(|c| c.filter() == filter).map(|c| c.as_ref())
    }
