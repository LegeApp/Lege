...
fn truncated_stream_is_tolerated() {
    ...
    let sos = jpeg.windows(2).position(|w| w == [0xFF, 0xDA]).expect("has SOS");
...
fn garbage_and_limits_are_typed_errors() {
    ...
    let tiny = DecodeLimits { max_pixels: 16, ..DecodeLimits::default() };
...
fn dnl_height_placeholder_falls_back_to_the_dictionary() {
    ...
    let sof = jpeg.windows(2).position(|p| p == [0xFF, 0xC0]).expect("has SOF0");
    jpeg[sof + 5] = 0xFF;
    jpeg[sof + 6] = 0xFF;
        ...
        for y in 0..th as usize {
            for x in 0..tw as usize {
                for c in 0..3 {
                    ...
                    expect[(y * tw as usize + x) * 3 + c] =
                        (acc / (denom * denom) as u32) as u8;
                }
            }
        }
        let got: Vec<u8> = (0..th as usize)
            .flat_map(|y| {
                small.data[y * small.stride..y * small.stride + tw as usize * 3].to_vec()
            })
            .collect();
...
fn an_oversized_image_is_reduced_rather_than_rejected() {
    ...
    let tight = DecodeLimits { max_pixels: 4096, ..DecodeLimits::default() };
    ...
    let hopeless = DecodeLimits { max_pixels: 4, ..DecodeLimits::default() };
...
fn a_sof_height_over_the_dictionary_is_clamped() {
    ...
    assert!(mean < 3.0, "clamped rows should still be the real image: mean {mean}");
    ...
    .expect("decode");
    assert_eq!(smaller.height, 192, "a larger dictionary height must not win");
}
