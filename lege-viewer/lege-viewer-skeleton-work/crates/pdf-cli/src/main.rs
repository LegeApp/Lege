...
//!   render <file.pdf> <page> <out.ppm>  render one page via the CPU backend
...
fn main() -> Result<()> {
    ...
    let scale = take_flag_value(&mut args, "--scale")
        .map(|value| value.parse::<f64>().context("scale must be a positive number"))
        .transpose()?;
...
fn dump(path: &str, page_number: u32, password: Option<&str>) -> Result<()> {
    ...
    let page = compiler
        ...
        .with_context(|| format!("compiling page {page_number}"))?;
    print!("{}", pdf_content::dump::dump_semantic(&page, snapshot.names()));
    Ok(())
}
...
fn render(
    ...
    let (width, height) = match page.bounds.rotate {
        90 | 270 => ((crop.height() * scale).ceil() as u32, (crop.width() * scale).ceil() as u32),
        _ => ((crop.width() * scale).ceil() as u32, (crop.height() * scale).ceil() as u32),
    };
    ...
    let device = match page.bounds.rotate {
        90 => Matrix { a: 0.0, b: s, c: s, d: 0.0, e: -crop.y0 * s, f: -crop.x0 * s },
        180 => Matrix { a: -s, b: 0.0, c: 0.0, d: s, e: crop.x1 * s, f: -crop.y0 * s },
        270 => Matrix { a: 0.0, b: -s, c: -s, d: 0.0, e: crop.y1 * s, f: crop.x1 * s },
        _ => Matrix { a: s, b: 0.0, c: 0.0, d: -s, e: -crop.x0 * s, f: crop.y1 * s },
    };
    ...
    // Use render_to_host (not render_blocking) so the per-page RenderStats —
    // including B3's degraded-draw counter — is available to report.
    ...
    if stats.degraded_draws > 0 {
        ...
        let tag = if stats.is_silent_blank() { "SILENT BLANK" } else { "degraded" };
        eprintln!(
...
fn request_for_page(
    ...
    let device = match page.bounds.rotate {
        90 => Matrix { a: 0.0, b: scale, c: scale, d: 0.0, e: -crop.y0 * scale, f: -crop.x0 * scale },
        180 => Matrix { a: -scale, b: 0.0, c: 0.0, d: scale, e: crop.x1 * scale, f: -crop.y0 * scale },
        270 => Matrix { a: 0.0, b: -scale, c: -scale, d: 0.0, e: crop.y1 * scale, f: crop.x1 * scale },
        _ => Matrix { a: scale, b: 0.0, c: 0.0, d: -scale, e: -crop.x0 * scale, f: crop.y1 * scale },
    };
    RenderRequest {
        page,
        transform: PageTransform { matrix: device },
