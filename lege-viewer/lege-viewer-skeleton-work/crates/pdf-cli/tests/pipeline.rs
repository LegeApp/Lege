...
fn render(bytes: Vec<u8>, dim: u32) -> HostPage {
    ...
    let page = PageCompiler::new().compile(&snap, PageIndex(0), &mut ctx).unwrap();

    let scale = dim as f64 / 100.0;
    // Flip y: device_y = (100 - user_y) * scale.
    let matrix = Matrix { a: scale, b: 0.0, c: 0.0, d: -scale, e: 0.0, f: dim as f64 };
...
fn axial_shading_pdf_renders_a_gradient() {
    ...
    assert!(left < 40 && right > 210 && left < right, "gradient L={left} R={right}");
}
...
fn type4_mesh_shading_pdf_renders_a_gouraud_triangle() {
    ...
    assert_eq!(px(&host, 1, 1), [255, 255, 255, 255], "outside the triangle stays white");
}
...
fn type6_coons_patch_pdf_renders_corner_colors() {
    ...
    assert!(p[0] > 150 && p[1] > 150 && p[2] < 110, "C(1,0) yellow: {p:?}");
}
...
fn dct_jpeg_image_pdf_renders_through_the_codec_registry() {
    ...
    let jpeg = pdf_image::jpeg::encoder::write_jpeg(
        &[255, 0, 0, 0, 0, 255],
        2,
        1,
        true,
...
fn jpx_image_pdf_renders_through_the_codec_registry() {
    ...
    let jp2 = jp2lam::encode(&image, &jp2lam::EncodeOptions { quality: 100, ..Default::default() })
        .expect("encode");
...
fn every_standard_family_renders() {
    for base in ["Helvetica", "Times-Roman", "Courier", "Times-BoldItalic", "Courier-Bold"] {
...
fn symbol_resolves_every_greek_letter_in_its_alphabet() {
    ...
    for (code, name) in [(b'a', "alpha"), (b'm', "mu"), (b'b', "beta"), (b'w', "omega")] {
        assert!(solo(code) > 0.004, "Symbol '{}' ({name}) drew nothing", code as char);
    }
}
...
fn embedded_type1_fontfile_renders_through_the_native_engine() {
    ...
    b.add_stream(7, &format!("/Length1 {}/Length2 0/Length3 0", t1.len()), &t1);
    b.finish_classic_xref("/Root 1 0 R");
