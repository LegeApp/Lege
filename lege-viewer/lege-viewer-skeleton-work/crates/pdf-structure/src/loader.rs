...
pub fn load_structure(
    ...
    let degenerate =
        used_recovery && trailer.as_ref().is_some_and(|t| loader.page_tree_looks_empty(t));
...
fn parse_version(file: &[u8], recovery: &mut Vec<RecoveryEvent>) -> PdfVersion {
    ...
    let parsed = (|| {
        ...
        let major = rest.first().filter(|b| b.is_ascii_digit()).map(|b| b - b'0')?;
        let rest = rest.get(1..)?.strip_prefix(b".")?;
        let minor = rest.first().filter(|b| b.is_ascii_digit()).map(|b| b - b'0')?;
        Some(PdfVersion { major, minor })
    })();
    parsed.unwrap_or_else(|| {
        recovery.push(RecoveryEvent::Other("malformed %PDF version; assuming 1.7".into()));
...
fn find_last_keyword(haystack: &[u8], kw: &[u8]) -> Option<usize> {
    ...
    while let Some(i) = find_last_subslice(&haystack[..end], kw) {
        let left_ok =
            i == 0 || !pdf_syntax::classify::is_regular(haystack[i - 1]);
...
impl<'a> Loader<'a> {
    ...
    fn follow_chain(&mut self, start: u64) -> Result<(), StructureError> {
        ...
        while let Some(offset) = next {
            ...
            if !self.visited.insert(offset) {
                ...
                self.recovery
                    .push(RecoveryEvent::Other(format!("xref chain cycle at offset {offset}")));
                break;
            }
    ...
    fn parse_classic_table(
            ...
            match tok.value {
                Token::Integer(start) => {
                    ...
                    if start == 1 && count > 0 {
                        ...
                        if let (Ok(Token::Integer(0)), Ok(Token::Integer(65535)), Ok(Token::Keyword(k))) =
                            head
                            && k.as_slice() == b"f"
                        {
                            start = 0;
        ...
        self.revisions.push(Revision { xref_offset: offset, trailer: Arc::clone(&trailer_dict) });
    ...
    fn parse_xref_stream(&mut self, offset: u64) -> Result<Option<u64>, StructureError> {
        ...
        let raw: &[u8] = match &stream.data {
            StreamData::InSource { offset, len } => {
                ...
                self.file.get(start..end).ok_or(StructureError::NoUsableXref)?
            }
        ...
        for (start_num, count) in indices {
            ...
            let Some(seg_len) = count_usize.checked_mul(stride) else { continue };
            let Some(seg_end) = seg_start.checked_add(seg_len) else { continue };
            if seg_end > data.len() {
        ...
        self.revisions.push(Revision { xref_offset: offset, trailer: dict_arc });
    ...
    fn record_stream_entry(&mut self, entry: &[u8], widths: &[u64], number: u32) {
        fn field(entry: &[u8], widths: &[u64], idx: usize) -> u64 {
            ...
            entry[start..start + len].iter().fold(0u64, |acc, &b| (acc << 8) | u64::from(b))
        }
        // W[0] = 0 → default entry type 1 (ISO 32000-1 Table 17).
        let entry_type = if widths[0] == 0 { 1 } else { field(entry, widths, 0) };
        ...
        match entry_type {
            ...
            2 => {
                if let (Ok(container), Ok(index)) = (u32::try_from(f1), u32::try_from(f2)) {
                    ...
                    self.record(number, 0, ObjectLocation::InObjectStream { container, index });
                }
            }
    ...
    fn resolve_trailer(&self) -> Option<Trailer> {
        ...
        for rev in &self.revisions {
            let d = &rev.trailer;
            if root.is_none() {
                root = d.get(self.names.known.root).and_then(PdfObject::as_reference);
            }
            if info.is_none() {
                info = d.get(self.names.known.info).and_then(PdfObject::as_reference);
            }
            if encrypt.is_none() {
                encrypt = d.get(self.names.known.encrypt).and_then(PdfObject::as_reference);
            }
            if file_id.is_none()
        ...
        Some(Trailer { dict, root, info, encrypt, file_id })
    }
    ...
    fn rebuild(&mut self) {
        ...
        while i + 3 <= file.len() {
            ...
            {
                let mut parser =
                    ObjectParser::new(&file[i + 7..], (i + 7) as u64, &self.limits.syntax, self.names);
        ...
        for (container, offset) in containers {
            ...
            for (index, member) in members.into_iter().enumerate() {
                ...
                if !overridden_by_later {
                    best.insert(
                        member,
                        (offset, 0, ObjectLocation::InObjectStream { container, index: index as u32 }),
                    );
                }
            }
        }
        ...
        for dict in trailer_dicts.into_iter().rev() {
            self.revisions.push(Revision { xref_offset: 0, trailer: Arc::new(dict) });
        }
        ...
        if !root_known && let Some((id, _)) = self.find_catalog() {
            let dict = Dictionary::from_pairs([(
                self.names.known.root,
                PdfObject::Reference(id),
            )]);
            self.revisions.insert(0, Revision { xref_offset: 0, trailer: Arc::new(dict) });
        }
    }
    ...
    fn find_catalog(&mut self) -> Option<(ObjectId, Arc<Dictionary>)> {
        ...
        for n in numbers {
            let (generation, ObjectLocation::Offset(off)) = self.entries[&n] else { continue };
            let window = self.file.get(off as usize..)?;
            let mut parser = ObjectParser::new(window, off, &self.limits.syntax, self.names);
            let Ok(parsed) = parser.parse_indirect(&mut |_| None) else { continue };
            if let Some(dict) = parsed.object.as_dict()
                && dict.get(self.names.known.type_).and_then(PdfObject::as_name) == Some(catalog)
            {
    ...
    fn read_objstm_members(&mut self, offset: u64) -> Option<Vec<u32>> {
        ...
        if stream.dict.get(names.known.type_).and_then(PdfObject::as_name)
        ...
        let n = stream.dict.get(names.known.n).and_then(PdfObject::as_int).unwrap_or(0);
        let first = stream.dict.get(names.known.first).and_then(PdfObject::as_int).unwrap_or(0);
    ...
    fn build_xref_map(&mut self, trailer: &Trailer) -> XrefMap {
        ...
        if let Some(declared) =
            trailer.dict.get(self.names.known.size).and_then(PdfObject::as_int)
            && declared != size as i64
        {
            self.recovery
                .push(RecoveryEvent::SizeRepaired { declared, actual: size as u32 });
        }
