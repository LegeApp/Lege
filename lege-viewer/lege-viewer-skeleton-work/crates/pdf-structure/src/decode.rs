...
fn decode_stream_inner(
    ...
    for (i, (f, p)) in filters.iter().zip(parms_list).enumerate() {
        ...
        data = match name.as_ref() {
            ...
            codec @ (b"DCTDecode" | b"DCT" | b"JPXDecode" | b"JBIG2Decode"
...
fn inflate_with(
    ...
    loop {
        ...
        let input = &data[usize::try_from(before_in).unwrap_or(usize::MAX).min(data.len())..];
...
mod tests {
    ...
    fn no_filter_is_identity_but_still_budgeted() {
        ...
        let mut b = budget();
        assert_eq!(decode_stream(b"plain", &dict, &names, &mut b).unwrap(), b"plain");
        assert_eq!(b.used(), 5);
    ...
    fn flate_with_png_predictor() {
        ...
        let dict = Dictionary::from_pairs([
            (names.known.filter, PdfObject::Name(names.intern(b"FlateDecode"))),
            (names.known.decode_parms, PdfObject::Dictionary(parms.into())),
        ]);
        assert_eq!(
            decode_stream(&raw, &dict, &names, &mut budget()).unwrap(),
            vec![1, 2, 3, 4, 5, 6]
    ...
    fn run_length() {
        ...
        assert_eq!(
            decode_stream(&[1, b'A', b'B', 253, b'C', 128], &dict, &names, &mut budget()).unwrap(),
            b"ABCCCC"
        );
    }
    ...
    fn filter_chain_applies_in_order() {
        ...
        let hex: Vec<u8> = deflated.iter().flat_map(|b| format!("{b:02X}").into_bytes()).collect();
        let dict = Dictionary::from_pairs([(
            names.known.filter,
            PdfObject::Array(
                vec![
    ...
    fn truncated_flate_salvages_its_inflated_prefix() {
        ...
        let names = NameTable::new();
        let dict = dict_with_filter(&names, b"FlateDecode");
        let original: Vec<u8> = (0..8192u32).map(|i| b"the quick brown fox "[(i % 20) as usize]).collect();
        ...
        assert!(out.len() < original.len(), "a truncated stream cannot yield the whole payload");
        assert_eq!(&out[..], &original[..out.len()], "salvaged bytes must be a correct prefix");
    }
    ...
    fn lzw_round_trip(data: &[u8]) {
        for early in [true, false] {
            ...
            assert_eq!(decoded, data, "LZW round trip failed (early_change={early})");
        }
    }
    ...
    fn lzw_through_decode_stream_with_early_change_param() {
        ...
        let dict0 = Dictionary::from_pairs([
            (names.known.filter, PdfObject::Name(names.intern(b"LZWDecode"))),
            (
