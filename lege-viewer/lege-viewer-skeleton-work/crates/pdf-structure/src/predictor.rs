...
fn geometry(parms: &PredictorParms) -> Result<(usize, usize), PredictorError> {
    ...
    let bits_per_pixel = colors.checked_mul(bpc).ok_or(PredictorError::InvalidParameters)?;
    let row_bits =
        bits_per_pixel.checked_mul(columns).ok_or(PredictorError::InvalidParameters)?;
...
fn apply_tiff(mut data: Vec<u8>, parms: &PredictorParms) -> Result<Vec<u8>, PredictorError> {
    ...
    let mask: u32 = if bpc >= 32 { u32::MAX } else { (1u32 << bpc) - 1 };
...
mod tests {
    ...
    fn png_parms(columns: i64) -> PredictorParms {
        PredictorParms { predictor: 12, colors: 1, bits_per_component: 8, columns }
    }

    #[test]
    fn identity_predictor_passthrough() {
        let parms = PredictorParms { predictor: 1, ..Default::default() };
        assert_eq!(apply_predictor(vec![1, 2, 3], &parms).unwrap(), vec![1, 2, 3]);
    }
    ...
    fn png_up_filter() {
        ...
        assert_eq!(apply_predictor(data, &png_parms(3)).unwrap(), vec![5, 5, 5, 6, 7, 8]);
    }
    ...
    fn png_average_filter() {
        ...
        assert_eq!(apply_predictor(data, &png_parms(2)).unwrap(), vec![2, 4, 4, 7]);
    }
    ...
    fn png_paeth_filter_known_vector() {
        ...
        assert_eq!(apply_predictor(data, &png_parms(2)).unwrap(), vec![10, 20, 11, 21]);
    }
    ...
    fn tiff_predictor_8bit() {
        let parms = PredictorParms { predictor: 2, colors: 1, bits_per_component: 8, columns: 4 };
        // Row deltas 1,1,1,1 → cumulative 1,2,3,4.
        assert_eq!(apply_predictor(vec![1, 1, 1, 1], &parms).unwrap(), vec![1, 2, 3, 4]);
    }
    ...
    fn tiff_predictor_1bit() {
        ...
        let parms = PredictorParms { predictor: 2, colors: 1, bits_per_component: 1, columns: 8 };
    ...
    fn tiff_predictor_4bit_two_colors() {
        ...
        let parms = PredictorParms { predictor: 2, colors: 2, bits_per_component: 4, columns: 2 };
        let decoded =
            apply_predictor(vec![byte(deltas[0], deltas[1]), byte(deltas[2], deltas[3])], &parms)
                .unwrap();
    ...
    fn multi_byte_pixels_use_pixel_stride() {
        ...
        let parms = PredictorParms { predictor: 12, colors: 3, bits_per_component: 8, columns: 2 };
    ...
    fn invalid_parameters_rejected() {
        let parms = PredictorParms { predictor: 12, colors: 0, bits_per_component: 8, columns: 5 };
        assert!(apply_predictor(vec![0; 12], &parms).is_err());
        let parms = PredictorParms { predictor: 7, ..Default::default() };
        assert!(matches!(
    ...
    fn xref_style_up_predictor_roundtrip() {
        ...
        let parms = PredictorParms { predictor: 12, colors: 1, bits_per_component: 8, columns: 4 };
