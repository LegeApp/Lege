...
pub fn parse_object_stream_layout(
    ...
    let (Ok(n), Ok(first)) = (u32::try_from(n), u64::try_from(first)) else {
        return Err(StructureError::LimitExceeded("object stream /N or /First is negative"));
    };
    // The pair table lives before /First; lex only that region so member
    // data can never be misread as table entries.
    let table_end = usize::try_from(first).unwrap_or(decoded.len()).min(decoded.len());
    let mut lexer = Lexer::new(&decoded[..table_end], 0, limits);
    let mut members = Vec::with_capacity(n.min(65536) as usize);
    for _ in 0..n {
        let Ok(num_tok) = lexer.next_token() else { break };
        let Token::Integer(number) = num_tok.value else { break };
        let Ok(off_tok) = lexer.next_token() else { break };
        let Token::Integer(offset) = off_tok.value else { break };
        ...
        members.push(ObjStmMember { number, offset: first + offset });
    }
...
mod tests {
    ...
    fn parses_pair_table() {
        ...
        assert_eq!(
            layout.members,
            vec![
                ObjStmMember { number: 11, offset: 12 },
                ObjStmMember { number: 12, offset: 17 },
            ]
        );
        assert_eq!(layout.member(1), Some(ObjStmMember { number: 12, offset: 17 }));
