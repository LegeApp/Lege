...
impl<'a> Lexer<'a> {
    pub fn new(input: &'a [u8], base_offset: Offset, limits: &'a SyntaxLimits) -> Self {
        Self { input, pos: 0, base: base_offset, limits }
    }
    ...
    pub fn next_token(&mut self) -> Result<Spanned<Token>, SyntaxError> {
        ...
        let Some(b) = self.peek() else {
            return Ok(Spanned { value: Token::Eof, start, end: start });
        };
        ...
        let value = match b {
            ...
            b'>' => {
                ...
                if self.peek() == Some(b'>') {
                    ...
                    return Err(SyntaxError::UnexpectedByte { byte: b'>', offset: start });
                }
            }
            b'(' => {
                self.pos += 1;
        ...
        Ok(Spanned { value, start, end: self.offset() })
    }
    ...
    fn parse_number(run: &[u8], start: Offset) -> Result<Token, SyntaxError> {
        ...
        if int_digits == 0 && frac_digits == 0 {
            // "-", "+", ".", "-." … PDFium yields zero.
            return Ok(if has_dot { Token::Real(0.0) } else { Token::Integer(0) });
        }

        // The prefix is pure ASCII by construction.
        let text = std::str::from_utf8(prefix).map_err(|_| SyntaxError::MalformedNumber { offset: start })?;
        if !has_dot {
        ...
        text.parse::<f64>().map(Token::Real).map_err(|_| SyntaxError::MalformedNumber { offset: start })
    }
...
mod tests {
    ...
    fn integer_overflow_degrades_to_real() {
        ...
        assert_eq!(lex_one(b"9223372036854775808"), Token::Real(9223372036854775808.0));
        assert_eq!(
    ...
    fn hex_strings_pdfium_ported_cases() {
        ...
        assert_eq!(lex_one(b"<1A2zasdf>"), Token::String(vec![0x1A, 0x2A, 0xDF]));
    ...
    fn token_length_limit_enforced() {
        let limits = SyntaxLimits { max_token_bytes: 4, max_nesting_depth: 8 };
        let mut lexer = Lexer::new(b"(abcdefgh)", 0, &limits);
        assert!(matches!(lexer.next_token(), Err(SyntaxError::TokenTooLong { .. })));
        let mut lexer = Lexer::new(b"/abcdefgh", 0, &limits);
        assert!(matches!(lexer.next_token(), Err(SyntaxError::TokenTooLong { .. })));
        let mut lexer = Lexer::new(b"abcdefgh", 0, &limits);
        assert!(matches!(lexer.next_token(), Err(SyntaxError::TokenTooLong { .. })));
