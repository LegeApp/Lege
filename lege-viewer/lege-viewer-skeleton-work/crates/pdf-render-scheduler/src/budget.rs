...
impl MemoryBudget {
    pub fn new(limit: u64) -> Self {
        Self { inner: Arc::new(Inner { limit, state: Mutex::new(State { in_use: 0 }), freed: Condvar::new() }) }
    }
    ...
    pub fn acquire(&self, bytes: u64) -> Result<MemoryPermit, BudgetExceeded> {
        if bytes > self.inner.limit {
            return Err(BudgetExceeded { requested: bytes, limit: self.inner.limit });
        }
        ...
        state.in_use += bytes;
        Ok(MemoryPermit { budget: self.clone(), bytes })
    }
    ...
    pub fn try_acquire(&self, bytes: u64) -> Result<Option<MemoryPermit>, BudgetExceeded> {
        ...
        state.in_use += bytes;
        Ok(Some(MemoryPermit { budget: self.clone(), bytes }))
    }
