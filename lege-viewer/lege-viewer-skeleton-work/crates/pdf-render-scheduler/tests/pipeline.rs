...
use pdf_render_api::{
    AnnotationMode, Background, BackendCapabilities, BackendId, CancellationToken, HostPage,
    OutputFormat, OutputResidency, PageTransform, RenderBackend, RenderError, RenderLimits,
    RenderQuality, RenderRequest, RenderTicket, RenderedPage, SubmitError, SupportLevel,
};
...
impl RenderBackend for MockBackend {
    ...
    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            ...
            max_surface: DeviceSize { width: 1 << 16, height: 1 << 16 },
...
fn snapshot(pages: u32) -> DocumentSnapshot {
    let source: Arc<dyn PdfSource> = Arc::new(OwnedBytesSource::new(builder::multipage_classic(pages)));
...
fn make_requester(
    ...
    move |snapshot, page| {
        ...
        Ok(RenderRequest {
            ...
            crop: None,
            output_size: DeviceSize { width: 4 + page.0, height: 1 },
...
fn collect(scheduler: &RenderScheduler, snap: &DocumentSnapshot, pages: u32, make: &(dyn Fn(&DocumentSnapshot, PageIndex) -> Result<RenderRequest, RenderError> + Sync), cancel: Option<&CancellationToken>) -> Vec<(u32, Result<u32, String>)> {
    let mut out = Vec::new();
...
fn six_pages_compile_and_render_concurrently_in_order() {
    let snap = snapshot(6);
    let backend = Arc::new(MockBackend { sleep: Duration::from_millis(15), ..Default::default() });
    let compiles = Arc::new(AtomicUsize::new(0));
    let compile_peak = Arc::new(AtomicUsize::new(0));
    let make = make_requester(compiles, compile_peak.clone(), None);
    let opts = SchedulerOptions { compile_workers: 6, render_workers: 6, compiled_queue_depth: 6, memory_limit_bytes: 1 << 30 };
    ...
    assert!(backend.peak.load(Ordering::SeqCst) >= 2, "renders overlapped");
    assert!(compile_peak.load(Ordering::SeqCst) >= 2, "compiles overlapped");
}
...
fn output_is_deterministic_across_worker_counts() {
    ...
    let run = |cw: usize, rw: usize| {
        let backend = Arc::new(MockBackend { sleep: Duration::from_millis(1), ..Default::default() });
        let make = make_requester(Arc::new(AtomicUsize::new(0)), Arc::new(AtomicUsize::new(0)), None);
        let opts = SchedulerOptions { compile_workers: cw, render_workers: rw, compiled_queue_depth: 4, memory_limit_bytes: 1 << 30 };
...
fn one_page_failure_does_not_affect_others() {
    ...
    let make = make_requester(Arc::new(AtomicUsize::new(0)), Arc::new(AtomicUsize::new(0)), Some(2));
    let opts = SchedulerOptions { compile_workers: 4, render_workers: 4, compiled_queue_depth: 4, memory_limit_bytes: 1 << 30 };
...
fn backend_panic_on_one_page_is_isolated() {
    ...
    let backend = Arc::new(MockBackend { sleep: Duration::from_millis(1), panic_width: Some(5), ..Default::default() });
    let make = make_requester(Arc::new(AtomicUsize::new(0)), Arc::new(AtomicUsize::new(0)), None);
    let opts = SchedulerOptions { compile_workers: 4, render_workers: 4, compiled_queue_depth: 4, memory_limit_bytes: 1 << 30 };
    ...
    let err = out[1].1.as_ref().expect_err("page 1 render panicked → isolated error");
    assert!(
...
fn pipeline_token_is_injected_into_backend_requests() {
    ...
    let backend = Arc::new(MockBackend { sleep: Duration::from_millis(1), ..Default::default() });
    let make = make_requester(Arc::new(AtomicUsize::new(0)), Arc::new(AtomicUsize::new(0)), None);
    let opts = SchedulerOptions { compile_workers: 2, render_workers: 2, compiled_queue_depth: 2, memory_limit_bytes: 1 << 30 };
    let token = CancellationToken::new(); // live — never cancelled

    let out = collect(&RenderScheduler::new(backend.clone(), opts), &snap, 4, &make, Some(&token));

    assert_eq!(out.len(), 4);
    for (p, r) in &out {
        assert_eq!(*r, Ok(4 + *p));
    }
    assert_eq!(backend.saw_token.load(Ordering::SeqCst), 4, "every request carried the pipeline token");
}
...
fn cancellation_stops_the_pipeline() {
    ...
    let out = collect(&RenderScheduler::new(backend, opts), &snap, 6, &make, Some(&token));

    assert_eq!(out.len(), 6, "every page still produces a (cancelled) result in order");
    for (_, r) in &out {
        assert!(matches!(r, Err(e) if e.contains("cancel")), "cancelled: {r:?}");
    }
}
