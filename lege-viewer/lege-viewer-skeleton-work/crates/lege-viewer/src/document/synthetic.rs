...
fn compile_synthetic_page(
    ...
    cancellation: &CancellationFlag,
        ...
        Ok(Arc::new(CompiledArtifacts {
            page,
            geometry,
            semantic: SemanticArtifact::Synthetic(Arc::new(SyntheticSemanticPage {
                page,
                line_count,
            })),
...
fn raster_synthetic_tile(
    ...
    cancellation: &CancellationFlag,
        ...
        for line in artifacts.text.lines().lines.iter() {
            ...
            let end_x = ((artifacts.geometry.display_width() - 54.0) * scale).round() as i32
                - origin_x;
        ...
        if pass == RasterPass::Final {
            let block_x0 = (artifacts.geometry.display_width() * 0.56 * scale).round() as i32
                - origin_x;
            let block_y0 = (artifacts.geometry.display_height() * 0.18 * scale).round() as i32
                - origin_y;
