...
fn clean_document_reports_ok() {
    ...
    assert!(matches!(report.open, OpenOutcome::Ok), "open: {:?}", report.open);
    ...
    for page in &report.pages {
        assert!(page.media_box_present, "page {} inherits /MediaBox", page.index);
        assert!(
...
fn rebuilt_document_reports_rebuild() {
    ...
    assert!(matches!(report.open, OpenOutcome::Recovered { .. }), "open: {:?}", report.open);
}
...
fn rc4_encrypted_document_reports_scheme() {
    ...
    assert!(enc.user_password_empty, "opened via the empty user password");
}
...
fn aes256_fixture_fails_open_but_still_reports_encryption() {
    ...
    assert!(matches!(report.open, OpenOutcome::Failed { .. }), "open: {:?}", report.open);
...
fn wrong_stream_length_surfaces_as_degraded_or_recovered() {
    ...
    let page_degraded =
        matches!(&report.pages[0].compile, CompileStatus::Degraded { .. });
