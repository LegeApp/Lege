...
fn examine_open(snapshot: &DocumentSnapshot) -> DocumentReport {
    ...
    let open = if recovery.is_empty() {
        ...
        OpenOutcome::Recovered { how: recovery.iter().map(describe_recovery).collect() }
    };
    let xref = XrefHealth {
        recovery_used: !recovery.is_empty(),
        rebuilt: recovery.iter().any(|e| matches!(e, RecoveryEvent::XrefRebuilt)),
    ...
    DocumentReport { open, encryption, xref, pages, annotations, features }
}
...
fn examine_failed(
    ...
    report.xref = XrefHealth {
        ...
        rebuilt: structure.recovery.iter().any(|e| matches!(e, RecoveryEvent::XrefRebuilt)),
...
fn describe_recovery(event: &RecoveryEvent) -> String {
    match event {
        ...
        RecoveryEvent::ObjectOffsetRepaired { id, declared, actual } => {
            format!("object {id} xref offset {declared} wrong; found at {actual}")
        }
        RecoveryEvent::StreamLengthRepaired { id, declared, actual } => {
...
fn read_encryption(snapshot: &DocumentSnapshot, ctx: &mut ParseContext) -> Option<EncryptionInfo> {
    ...
    Some(EncryptionInfo { version, revision, method, user_password_empty })
}
...
fn crypt_filter_is_aes(
    ...
    let names = snapshot.names();
    let Some(cf_key) = names.lookup(b"CF") else { return false };
    let Some(cf) = dict_get_resolved(snapshot, encrypt, cf_key, ctx) else { return false };
    let Some(cf) = cf.as_dict() else { return false };
    let Some(stdcf_key) = names.lookup(b"StdCF") else { return false };
    let Some(stdcf) = dict_get_resolved(snapshot, cf, stdcf_key, ctx) else { return false };
    let Some(stdcf) = stdcf.as_dict() else { return false };
    let Some(cfm_key) = names.lookup(b"CFM") else { return false };
    let Some(cfm) = stdcf.get(cfm_key).and_then(PdfObject::as_name) else { return false };
...
fn examine_pages(snapshot: &DocumentSnapshot) -> Vec<PageStatus> {
    ...
    for index in 0..snapshot.page_count() {
        ...
        let compile = match compiler.compile_semantic(snapshot, PageIndex(index), &mut ctx) {
            ...
            Err(e) => CompileStatus::Failed { error: e.to_string() },
        };
        out.push(PageStatus { index, media_box_present, compile });
    }
    out
}
...
fn page_has_media_box(snapshot: &DocumentSnapshot, index: u32, ctx: &mut ParseContext) -> bool {
    let Ok(page_ref) = snapshot.page(PageIndex(index)) else { return false };
    let names = snapshot.names();
    let media_box = names.known.media_box;
    let parent = names.known.parent;
    let mut current = resolve(snapshot, page_ref.object, ctx);
    for _ in 0..MAX_PARENT_HOPS {
        let Some(obj) = current else { return false };
        let Some(dict) = obj.as_dict() else { return false };
        if dict.contains_key(media_box) {
            return true;
        }
        current = dict.get(parent).and_then(|p| resolve_value(snapshot, p, ctx));
    }
    false
}
...
fn inventory_annotations(
    ...
    for index in 0..snapshot.page_count() {
        let Ok(page_ref) = snapshot.page(PageIndex(index)) else { continue };
        let Some(page_obj) = resolve(snapshot, page_ref.object, ctx) else { continue };
        let Some(page_dict) = page_obj.as_dict() else { continue };
        let Some(annots) = dict_get_resolved(snapshot, page_dict, annots_key, ctx) else {
            continue;
        };
        let PdfObject::Array(entries) = annots.as_ref() else { continue };
        for entry in entries.iter() {
            let Some(annot) = resolve_value(snapshot, entry, ctx) else { continue };
...
fn read_features(snapshot: &DocumentSnapshot, ctx: &mut ParseContext) -> FeatureFlags {
    ...
    let Some(catalog) = resolve(snapshot, root, ctx) else { return flags };
    let Some(catalog) = catalog.as_dict() else { return flags };
    ...
    let has = |snapshot: &DocumentSnapshot,
     ...
     -> Option<Arc<PdfObject>> {
        ...
        if matches!(v.as_ref(), PdfObject::Null) { None } else { Some(v) }
    };
    ...
    pub fn summary(&self) -> String {
        ...
        match &self.open {
            ...
            OpenOutcome::Recovered { how } => {
                writeln_ok(&mut s, &format!("open: recovered ({} repair(s))", how.len()));
                for h in how {
        ...
        if self.annotations.count_per_subtype.is_empty() {
            ...
            writeln_ok(&mut s, &format!("annotations: {} total", self.annotations.total()));
