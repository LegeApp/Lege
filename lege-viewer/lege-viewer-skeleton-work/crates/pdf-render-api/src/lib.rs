...
impl Default for RenderLimits {
    fn default() -> Self {
        Self { max_page_bytes: 2 << 30, max_group_depth: 64, cancellation: None }
    }
}
...
impl RenderTicket {
    pub fn new(job_id: u64) -> (Self, mpsc::Sender<Result<RenderedPage, RenderError>>) {
        ...
        (Self { job_id, receiver: rx }, tx)
    }

    /// Block until the job completes.
    pub fn wait(self) -> Result<RenderedPage, RenderError> {
        self.receiver.recv().unwrap_or(Err(RenderError::Disconnected))
    }
...
pub fn submit_caught(
    ...
    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| backend.submit(request))) {
        ...
        Err(payload) => {
            ...
            let _ = tx.send(Err(RenderError::Panic { message: panic_message(payload) }));
            Ok(ticket)
        }
    }
}
...
pub fn render_blocking(
    ...
    match outcome {
        Ok(result) => result,
        Err(payload) => Err(RenderError::Panic { message: panic_message(payload) }),
    }
}
...
mod tests {
    ...
    impl RenderBackend for PanickingBackend {
        ...
        fn capabilities(&self) -> BackendCapabilities {
            BackendCapabilities {
                ...
                max_surface: DeviceSize { width: 1, height: 1 },
    ...
    fn dummy_request() -> RenderRequest {
        let page = CompiledPage::empty(pdf_page_ir::PageBounds {
            crop: pdf_page_ir::Rect { x0: 0.0, y0: 0.0, x1: 10.0, y1: 10.0 },
            rotate: 0,
        });
        RenderRequest {
            page: Arc::new(page),
            ...
            crop: None,
            output_size: DeviceSize { width: 4, height: 4 },
    ...
    fn injected_panic_becomes_typed_error_in_render_blocking() {
        ...
        match result {
            Err(RenderError::Panic { message }) => {
                assert!(message.contains("injected test panic"), "message: {message}");
            }
