...
impl PdfBuilder {
    ...
    pub fn into_bytes(self) -> Vec<u8> {
        assert!(self.revision.is_empty(), "unfinished revision: call a finish_* method");
        self.buf
    }
    ...
    pub fn add_object_stream(&mut self, number: u32, members: &[(u32, &str)]) -> u64 {
        ...
        for (index, (member_number, _)) in members.iter().enumerate() {
            self.note(*member_number, Entry::InStream { container: number, index: index as u32 });
        }
        self.add_stream(
            number,
    ...
    pub fn finish_xref_stream(&mut self, number: u32, trailer_extra: &str) {
        ...
        for (&object_number, entry) in &self.revision {
            let row = match *entry {
                Entry::Offset(offset, generation) => {
                    let o = u32::try_from(offset).expect("fixture too large").to_be_bytes();
                    let g = generation.to_be_bytes();
                    [1, o[0], o[1], o[2], o[3], g[0], g[1]]
                }
                Entry::InStream { container, index } => {
                    let c = container.to_be_bytes();
                    let i = u16::try_from(index).expect("objstm too large").to_be_bytes();
        ...
        let numbers: Vec<(u32, u64, u16)> =
            entries.iter().map(|&(n, _)| (n, 0, 0)).collect();
...
fn add_document_body(builder: &mut PdfBuilder, page_count: u32) {
    ...
    for i in 0..page_count {
        ...
        let extras = if i == 1 { "/Rotate 90/CropBox[10 10 400 500]" } else { "" };
        builder.add_object(
            page(i),
            &format!(
...
pub fn wrong_length_fixture() -> Vec<u8> {
    ...
    builder.add_object(
        PAGES,
        &format!("<</Type/Pages/Kids[{} 0 R]/Count 1/MediaBox[0 0 612 792]>>", page(0)),
    );
    builder.add_object(
        page(0),
        &format!("<</Type/Page/Parent {PAGES} 0 R/Contents {} 0 R>>", content(0)),
    );
...
pub fn encrypted_fixture() -> Vec<u8> {
    ...
    let mut builder = PdfBuilder::new();
    builder.add_object(CATALOG, &format!("<</Type/Catalog/Pages {PAGES} 0 R>>"));
    builder.add_object(PAGES, &format!("<</Type/Pages/Kids[{} 0 R]/Count 1>>", page(0)));
...
pub fn hybrid_fixture(page_count: u32) -> Vec<u8> {
    ...
    let sx = bytes.windows(9).rposition(|w| w == b"startxref").expect("startxref");
    let stream_offset: u64 = {
        let tail = &bytes[sx..];
        let text = std::str::from_utf8(tail).expect("ascii tail");
        text.lines().nth(1).expect("offset line").trim().parse().expect("offset")
    };
    bytes.truncate(sx);
