//! DeviceCMYK → sRGB via Adobe's measured table (PDFium's
//! `core/fxge/dib/cfx_cmyk_to_srgb.cpp`).
//!
//! CMYK is ink, not light: there is no formula that turns it into RGB. The
//! naive `1 − min(1, x + k)` this engine used before is arithmetic, not
//! colour — it renders `DeviceCMYK(1, 0.44, 0, 0)` as a searing
//! `rgb(0, 143, 255)` where the measured answer is a muted blue. Every CMYK
//! page in every document was affected, and the differential oracle caught it
//! on its first run against PDFium.
//!
//! So this ports PDFium's approach wholesale: a 9×9×9×9 grid of *measured*
//! Adobe values, quadrilinearly interpolated in 8.8 fixed point. The table is
//! empirical data accumulated upstream — exactly the kind of thing that
//! cannot be derived, only copied.
//!
//! # The table is verbatim
//! [`ADOBE_CMYK`] below is PDFium's `kCMYK` **byte for byte**, still in C++
//! brace-initialiser syntax. [`cmyk_table!`] re-templates it into a Rust
//! array at compile time, so 6561 hand-transcribed entries are impossible to
//! get subtly wrong and re-syncing with upstream is a copy-paste and a diff.
//! Resist the urge to "tidy" it into Rust literals.

/// Re-templater: ingest C++ `{r, g, b},` brace initialisers and emit a Rust
/// array. Token-for-token, at compile time — the table never has to be
/// touched by hand.
macro_rules! cmyk_table {
    ($({$r:literal, $g:literal, $b:literal}),* $(,)?) => {
        [$([$r, $g, $b]),*]
    };
}

/// Adobe's measured CMYK→sRGB grid: 9 samples per channel, C-major.
/// Verbatim from PDFium's `kCMYK`.
#[rustfmt::skip]
static ADOBE_CMYK: [[u8; 3]; 9 * 9 * 9 * 9] = cmyk_table![
...
/// Index into [`ADOBE_CMYK`]; each axis is a 0..=8 grid coordinate.
/// PDFium's `IndexFromCMYK`.
#[inline]
const fn index_from_cmyk(c: usize, m: usize, y: usize, k: usize) -> usize {
    9 * 9 * 9 * c + 9 * 9 * m + 9 * y + k
}

/// Convert one 8-bit CMYK quadruple to sRGB — PDFium's `AdobeCMYK_to_sRGB1`.
///
/// Works in 8.8 fixed point. For each axis it takes the nearest grid point
/// (`*_index`, rounded) and its neighbour on the side the true value lies
/// (`*1_index`), then corrects the base colour toward that neighbour in
/// proportion to the distance. The `* (index - i1_index)` term carries the
/// sign, so a value below its grid point corrects the other way; a value
/// exactly on a grid point picks an arbitrary neighbour but weights it zero.
///
/// The arithmetic is reproduced exactly, including the truncating divides —
/// this must agree with PDFium bit for bit, not merely closely.
pub fn adobe_cmyk_to_srgb_u8(c: u8, m: u8, y: u8, k: u8) -> [u8; 3] {
    let fix_c = (c as i32) << 8;
    let fix_m = (m as i32) << 8;
    let fix_y = (y as i32) << 8;
    let fix_k = (k as i32) << 8;

    // Nearest grid point on each axis (+4096 rounds; >>13 divides by 8192).
    let c_index = ((fix_c + 4096) >> 13) as usize;
    let m_index = ((fix_m + 4096) >> 13) as usize;
    let y_index = ((fix_y + 4096) >> 13) as usize;
    let k_index = ((fix_k + 4096) >> 13) as usize;

    let start_rgb = ADOBE_CMYK[index_from_cmyk(c_index, m_index, y_index, k_index)];
    let mut fix_r = (start_rgb[0] as i32) << 8;
    let mut fix_g = (start_rgb[1] as i32) << 8;
    let mut fix_b = (start_rgb[2] as i32) << 8;

    // The neighbouring grid point on each axis. Landing exactly on the base
    // point leaves nothing to interpolate toward, so step to an adjacent one
    // (inward at the top of the range); the rate below is then zero anyway.
    let neighbour = |fix: i32, index: usize| -> usize {
        let i1 = (fix >> 13) as usize;
        if i1 == index {
            if i1 == 8 { i1 - 1 } else { i1 + 1 }
        } else {
            i1
        }
    };
    let c1_index = neighbour(fix_c, c_index);
    let m1_index = neighbour(fix_m, m_index);
    let y1_index = neighbour(fix_y, y_index);
    let k1_index = neighbour(fix_k, k_index);

    let mut correct = |rgb1: [u8; 3], rate: i32| {
        fix_r += (start_rgb[0] as i32 - rgb1[0] as i32) * rate / 32;
        fix_g += (start_rgb[1] as i32 - rgb1[1] as i32) * rate / 32;
        fix_b += (start_rgb[2] as i32 - rgb1[2] as i32) * rate / 32;
    };

    let c1_rgb = ADOBE_CMYK[index_from_cmyk(c1_index, m_index, y_index, k_index)];
    correct(c1_rgb, (fix_c - ((c_index as i32) << 13)) * (c_index as i32 - c1_index as i32));

    let m1_rgb = ADOBE_CMYK[index_from_cmyk(c_index, m1_index, y_index, k_index)];
    correct(m1_rgb, (fix_m - ((m_index as i32) << 13)) * (m_index as i32 - m1_index as i32));

    let y1_rgb = ADOBE_CMYK[index_from_cmyk(c_index, m_index, y1_index, k_index)];
    correct(y1_rgb, (fix_y - ((y_index as i32) << 13)) * (y_index as i32 - y1_index as i32));

    let k1_rgb = ADOBE_CMYK[index_from_cmyk(c_index, m_index, y_index, k1_index)];
    correct(k1_rgb, (fix_k - ((k_index as i32) << 13)) * (k_index as i32 - k1_index as i32));

    [
        (fix_r.max(0) >> 8).min(255) as u8,
        (fix_g.max(0) >> 8).min(255) as u8,
        (fix_b.max(0) >> 8).min(255) as u8,
    ]
}

/// Convert normalized CMYK to sRGB — PDFium's `AdobeCMYK_to_sRGB`.
///
/// The rounding offset is PDFium's: the float immediately below 0.5, chosen
/// upstream because it reproduces `round()` exactly across all inputs while
/// `0.5` disagrees on one value (0.0019607842). Kept so our quantization
/// matches theirs.
pub fn adobe_cmyk_to_srgb(c: f32, m: f32, y: f32, k: f32) -> [f32; 3] {
    const ROUNDING_OFFSET: f32 = 0.499_999_97;
    let q = |v: f32| (v.clamp(0.0, 1.0) * 255.0 + ROUNDING_OFFSET) as u8;
    let rgb = adobe_cmyk_to_srgb_u8(q(c), q(m), q(y), q(k));
...
mod tests {
    ...
    /// Bit-exactness with PDFium is not asserted here — it is verified
    /// end-to-end by `tools/pdfium-diff` against the real library: a 225-swatch
    /// CMYK grid renders **identically**, mean max-channel difference 0.000.
    /// These tests guard the port's structure and intent.
    #[test]
    fn the_table_is_complete_and_verbatim() {
        // 9 samples per channel, four channels. If the re-templater ever
        // silently drops a row this catches it.
        assert_eq!(ADOBE_CMYK.len(), 6561);
        // Corners of the measured grid, straight from PDFium's kCMYK.
        assert_eq!(ADOBE_CMYK[index_from_cmyk(0, 0, 0, 0)], [255, 255, 255], "no ink = paper");
        assert_eq!(ADOBE_CMYK[index_from_cmyk(0, 0, 0, 8)], [34, 30, 31], "full K is not #000");
    }

    #[test]
    fn grid_points_reproduce_the_table_exactly() {
        // A CMYK value landing on a grid point must return that measured
        // entry untouched: the interpolation weights collapse to zero.
        for (c, m, y, k) in [(0usize, 0usize, 0usize, 0usize), (8, 0, 0, 0), (0, 8, 8, 0), (4, 4, 4, 4)] {
            let q = |i: usize| (i as f32 * 255.0 / 8.0).round() as u8;
            let got = adobe_cmyk_to_srgb_u8(q(c), q(m), q(y), q(k));
            let want = ADOBE_CMYK[index_from_cmyk(c, m, y, k)];
            let close = got.iter().zip(&want).all(|(a, b)| a.abs_diff(*b) <= 1);
            assert!(close, "grid ({c},{m},{y},{k}): got {got:?} want {want:?}");
        }
    }

    #[test]
    fn the_bug_the_oracle_found_is_fixed() {
        // A real book cover paints DeviceCMYK(1, 0.44, 0, 0). The naive
        // formula gave a searing rgb(0, 143, 255); the measured answer is a
        // muted blue, which is what PDFium draws.
        let rgb = adobe_cmyk_to_srgb(1.0, 0.44, 0.0, 0.0);
        let px = rgb.map(|v| (v * 255.0).round() as i32);
        assert!(px[0] < 60, "cyan ink is not zero-red but is dark: {px:?}");
        assert!(px[1] > 80 && px[1] < 150, "green is muted, not 143-bright: {px:?}");
        assert!(px[2] > 120 && px[2] < 210, "blue is muted, not 255: {px:?}");
        // The naive formula's answer must NOT be what we return.
        assert_ne!(px, [0, 143, 255], "still the naive formula");
    }

    #[test]
    fn ink_darkens_monotonically() {
        // More black must never lighten the result.
        let mut last = 255i32;
        for i in 0..=8 {
            let k = i as f32 / 8.0;
            let rgb = adobe_cmyk_to_srgb(0.0, 0.0, 0.0, k);
            let lum = (rgb[0] * 255.0) as i32;
            assert!(lum <= last, "K={k} lightened: {lum} > {last}");
            last = lum;
        }
    }
    ...
    fn out_of_range_input_is_clamped_not_wrapped() {
        // Malformed content can hand us anything; it must not panic or wrap.
        let lo = adobe_cmyk_to_srgb(-1.0, -5.0, 0.0, 0.0);
        assert_eq!(lo, adobe_cmyk_to_srgb(0.0, 0.0, 0.0, 0.0));
        let hi = adobe_cmyk_to_srgb(2.0, 9.0, 1.0, 1.0);
        assert_eq!(hi, adobe_cmyk_to_srgb(1.0, 1.0, 1.0, 1.0));
    }
}
