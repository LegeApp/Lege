...
fn check_user_password_r234(enc: &EncryptDict, password: &[u8]) -> Option<Vec<u8>> {
    ...
    if enc.u[..significant] == check[..significant] { Some(key) } else { None }
}
...
impl StandardHandler {
    ...
    pub fn open(enc: &EncryptDict) -> Option<Self> {
        if enc.r == 5 || enc.r == 6 || enc.v == 5 {
            ...
            return Some(Self { key, cipher: Cipher::Aes256 });
        }
        if enc.r > 6 || enc.v > 5 {
            return None; // Unknown future revision.
        }
        let key = derive_file_key(enc, b"");
        Some(Self { key, cipher: enc.cipher })
    }
        ...
        for pwd in candidate_encodings(password) {
            if v5 {
                if let Some(key) = derive_file_key_v5(enc, &pwd) {
                    return Some((Self { key, cipher: Cipher::Aes256 }, PasswordRole::User));
                }
                if let Some(key) = derive_file_key_v5_owner(enc, &pwd) {
                    return Some((Self { key, cipher: Cipher::Aes256 }, PasswordRole::Owner));
                }
            } else {
                if let Some(key) = check_user_password_r234(enc, &pwd) {
                    return Some((Self { key, cipher: enc.cipher }, PasswordRole::User));
                }
                if let Some(key) = check_owner_password_r234(enc, &pwd) {
                    return Some((Self { key, cipher: enc.cipher }, PasswordRole::Owner));
                }
            }
        }
        None
...
mod tests {
    ...
    fn v5_user_password_validation_accepts_and_rejects() {
        ...
        assert!(derive_file_key_v5(&enc, b"").is_some(), "empty user password validates");
        let mut wrong = enc.clone();
        wrong.u[0] ^= 0xFF; // corrupt the stored hash
        assert!(derive_file_key_v5(&wrong, b"").is_none(), "corrupted /U must reject");
        assert!(
    ...
    fn r2_user_and_owner_passwords_validate() {
        let enc = pdfium_r2();
        let (h, role) = StandardHandler::with_password(&enc, HOTEL_UTF8)
            .expect("user password opens R2");
        ...
        assert!(StandardHandler::verify_password(&enc, "").is_none(), "R2 needs a password");
    }
    ...
    fn r3_user_and_owner_passwords_validate() {
        ...
        assert!(StandardHandler::with_password(&enc, "wrong").is_none());
        assert!(StandardHandler::verify_password(&enc, "").is_none(), "R3 needs a password");
    }
    ...
    fn r6_user_and_owner_passwords_validate() {
        ...
        assert!(StandardHandler::with_password(&enc, "wrong").is_none());
        assert!(StandardHandler::verify_password(&enc, "").is_none(), "R6 needs a password");
    }
    ...
    fn empty_user_password_documents_verify_as_user() {
        ...
        enc.u = u;
        enc.ue = vec![0u8; 32];
        assert_eq!(StandardHandler::verify_password(&enc, ""), Some(PasswordRole::User));
    }
...
mod perms_tests {
    ...
    fn handler(key: [u8; 32]) -> StandardHandler {
        StandardHandler { key: key.to_vec(), cipher: Cipher::Aes256 }
    }
