...
mod tests {
    ...
    fn cbc_with_iv_and_pkcs7_round_trips() {
        ...
        let key128: [u8; 16] = unhex("2b7e151628aed2a6abf7158809cf4f3c").try_into().unwrap();
    ...
    fn fips197_aes256_block() {
        ...
        let key: [u8; 32] =
            unhex("000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f")
                .try_into()
                .unwrap();
        let plain: [u8; 16] = unhex("00112233445566778899aabbccddeeff").try_into().unwrap();
        let cipher: [u8; 16] = unhex("8ea2b7ca516745bfeafc49904b496089").try_into().unwrap();
