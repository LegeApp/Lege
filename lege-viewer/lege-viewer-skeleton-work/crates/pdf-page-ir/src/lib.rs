...
pub enum Paint {
    ...
    Pattern { tiling: TilingId, matrix: Matrix },
    ...
    /// (the pattern `/Matrix`).
    Shading { shading: ShadingId, matrix: Matrix },
}
...
pub enum ImageColorSpace {
    ...
    Indexed { base: Box<ImageColorSpace>, hival: u32, lookup: Arc<[u8]> },
    ...
    IccRgb { trc: Arc<[f32]>, matrix: [f32; 9] },
...
pub enum DisplayOp {
    ...
    PushClip { path: PathId, rule: FillRule },
    ...
    PushClipText { runs: Box<[GlyphRunId]> },
    PopClip,
    FillPath { path: PathId, paint: PaintId, rule: FillRule, alpha: f32, blend: BlendMode },
    StrokePath { path: PathId, paint: PaintId, style: StrokeStyleId, alpha: f32, blend: BlendMode },
    ...
    DrawImage { image: ImageId, paint: PaintId, transform: Matrix, alpha: f32, blend: BlendMode },
    BeginTransparencyGroup { group: TransparencyGroupId },
    EndTransparencyGroup,
    ApplySoftMask { mask: Option<MaskId> },
    DrawShading { shading: ShadingId, transform: Matrix },
    ...
    BeginSoftMask { kind: MaskKind, transfer: Option<TransferLut> },
    EndSoftMask,
...
impl CompiledPage {
    ...
    pub fn debug_dump(&self) -> String {
        ...
        let _ = writeln!(
            o,
            "bounds [{} {} {} {}] rotate {}",
            g(b.x0), g(b.y0), g(b.x1), g(b.y1), self.bounds.rotate
        );

        let flags: Vec<&str> = self.features.iter_names().map(|(n, _)| n).collect();
        let _ = writeln!(o, "features: {}", if flags.is_empty() { "none".into() } else { flags.join("|") });
        ...
        dump_table(&mut o, "stroke_styles", &self.stroke_styles, |s| {
            format!(
                ...
                s.join,
                g(s.miter_limit),
                s.dash_pattern.iter().map(|d| g(*d)).collect::<Vec<_>>().join(" "),
                g(s.dash_phase)
            )
        });
        dump_table(&mut o, "glyph_runs", &self.glyph_runs, |r| {
            format!("font#{} size {} mode {} glyphs {}", r.font.0, g(r.font_size), r.render_mode, r.glyphs.len())
        });
        dump_table(&mut o, "fonts", &self.fonts, |f| {
            format!("key {}:{}:{} program_bytes={}", f.key.object_number, f.key.generation, f.key.variant, f.program.len())
        });
        dump_table(&mut o, "images", &self.images, |i| {
            format!(
                "{}x{} bpc={} stencil={} interp={:?} decoded={} smask={}",
                i.width, i.height, i.bits_per_component, i.is_stencil, i.interpolation,
                i.samples.is_some(), i.smask.is_some()
            )
        });
        dump_table(&mut o, "groups", &self.groups, |gr| {
            format!(
                "isolated={} knockout={} opacity={} blend={:?} bounds [{} {} {} {}]",
                gr.isolated, gr.knockout, g(gr.opacity as f64), gr.blend, g(gr.bounds.x0), g(gr.bounds.y0), g(gr.bounds.x1), g(gr.bounds.y1)
            )
        });
        dump_table(&mut o, "shadings", &self.shadings, |s| match &s.kind {
            ShadingKind::Axial { extend, ramp, .. } => {
                format!("type{} axial extend=[{} {}] ramp={}", s.shading_type, extend[0], extend[1], ramp.len())
            }
            ShadingKind::Radial { extend, ramp, .. } => {
                format!("type{} radial extend=[{} {}] ramp={}", s.shading_type, extend[0], extend[1], ramp.len())
            }
        ...
        dump_table(&mut o, "tilings", &self.tilings, |t| {
            format!("uncolored={} step=[{} {}] ops={}", t.uncolored, g(t.x_step as f64), g(t.y_step as f64), t.cell.operations.len())
        });
        o
    }
...
fn fmt_matrix(m: &Matrix) -> String {
    format!("[{} {} {} {} {} {}]", g(m.a), g(m.b), g(m.c), g(m.d), g(m.e), g(m.f))
}

fn fmt_paint(p: &Paint) -> String {
    match p {
        Paint::Solid(c) => format!("solid rgba({}, {}, {}, {})", g(c.r as f64), g(c.g as f64), g(c.b as f64), g(c.a as f64)),
...
fn fmt_op(op: &DisplayOp) -> String {
    match op {
        ...
        DisplayOp::FillPath { path, paint, rule, alpha, blend } => format!(
            "fill path#{} paint#{} {:?} alpha {} {:?}",
            path.0, paint.0, rule, g(*alpha as f64), blend
        ),
        DisplayOp::StrokePath { path, paint, style, alpha, blend } => format!(
            "stroke path#{} paint#{} style#{} alpha {} {:?}",
            path.0, paint.0, style.0, g(*alpha as f64), blend
        ),
        DisplayOp::DrawGlyphRun { run, paint, alpha, blend, stroke } => {
            let s = match stroke {
                Some(gs) => format!(" stroke(paint#{} style#{} alpha {})", gs.paint.0, gs.style.0, g(gs.alpha as f64)),
                None => String::new(),
            };
            format!("draw-glyph-run run#{} paint#{} alpha {} {:?}{s}", run.0, paint.0, g(*alpha as f64), blend)
        }
        DisplayOp::DrawImage { image, paint, transform, alpha, blend } => format!(
            "draw-image image#{} paint#{} {} alpha {} {:?}",
            image.0, paint.0, fmt_matrix(transform), g(*alpha as f64), blend
        ),
        ...
        DisplayOp::DrawShading { shading, transform } => {
            format!("draw-shading shading#{} {}", shading.0, fmt_matrix(transform))
        }
...
mod tests {
    ...
    fn empty_page_is_valid() {
        let page = CompiledPage::empty(PageBounds {
            crop: Rect { x0: 0.0, y0: 0.0, x1: 612.0, y1: 792.0 },
            rotate: 0,
        });
