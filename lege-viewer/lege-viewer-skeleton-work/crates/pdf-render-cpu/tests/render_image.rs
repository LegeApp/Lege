...
use pdf_page_ir::{
    Color, CompiledPage, DeviceSize, DisplayOp, ImageColorSpace, ImageIr, ImageSMask,
...
fn image_ir(width: u32, height: u32, bpc: u8, cs: ImageColorSpace, samples: Vec<u8>, is_stencil: bool, smask: Option<ImageSMask>) -> ImageIr {
    ImageIr {
...
fn page(image: ImageIr, stencil_color: Color, scale: f64) -> CompiledPage {
    CompiledPage {
        ...
        bounds: PageBounds { crop: Rect { x0: 0.0, y0: 0.0, x1: 8.0, y1: 8.0 }, rotate: 0 },
        operations: Arc::from([
...
fn rgb_image_samples_left_red_right_blue() {
    ...
    let img = image_ir(2, 1, 8, ImageColorSpace::Rgb, vec![255, 0, 0, 0, 0, 255], false, None);
    let backend = CpuBackend::default();
    let (host, _) = backend.render_to_host(&request(page(img, Color::BLACK, 8.0), 8)).unwrap();
...
fn image_mask_paints_fill_color_where_sample_is_zero() {
    ...
    let (host, _) = backend
        ...
        .unwrap();
    assert_eq!(px(&host, 1, 4), [0, 255, 0, 255], "sample 0 painted with fill (green)");
    assert_eq!(px(&host, 6, 4), [255, 255, 255, 255], "sample 1 left as background");
}
...
fn undecodable_codec_image_is_a_recorded_silent_blank() {
    ...
    let img = ImageIr {
        key: ResourceKey { object_number: 1, generation: 0, variant: 0 },
        width: 4,
        height: 4,
        is_stencil: false,
        interpolation: InterpolationMode::Nearest,
    ...
    assert_eq!(stats.degraded_draws, 1, "the dropped JPX draw must be counted");
    assert!(stats.is_silent_blank(), "a blank page from a dropped image is a silent blank");
    assert!(!stats.recovery_notes.is_empty(), "a human-readable reason is recorded");
    assert_eq!(px(&host, 4, 4), [255, 255, 255, 255], "surface really is blank");
}
...
fn indexed_decode_array_remaps_sample_to_index() {
    ...
    let img = ImageIr {
        key: ResourceKey { object_number: 2, generation: 0, variant: 0 },
        width: 2,
        height: 1,
        is_stencil: false,
        interpolation: InterpolationMode::Nearest,
    ...
    let backend = CpuBackend::default();
    let (host, _) = backend.render_to_host(&request(page(img, Color::BLACK, 8.0), 8)).unwrap();
    assert_eq!(px(&host, 1, 4), [255, 255, 255, 255], "sample 0 → index 0 (white)");
    assert_eq!(px(&host, 6, 4), [0, 0, 0, 255], "sample 1 → index 255 via /Decode (black), not index 1 (red)");
}
...
fn tint_lut_separation_image_routes_samples_through_lut() {
    ...
    let cs = pdf_page_ir::ImageColorSpace::TintLut { rgb: Arc::from(lut) };
    let img = image_ir(2, 1, 8, cs, vec![0, 255], false, None);
    let backend = CpuBackend::default();
    let (host, _) = backend.render_to_host(&request(page(img, Color::BLACK, 8.0), 8)).unwrap();
    assert_eq!(px(&host, 1, 4), [10, 20, 30, 255], "sample 0 → LUT[0], not DeviceGray black");
    assert_eq!(px(&host, 6, 4), [200, 210, 220, 255], "sample 255 → LUT[255]");
}
...
fn zero_dimension_image_paints_nothing() {
    ...
    let backend = CpuBackend::default();
    let (host, _) = backend.render_to_host(&request(page(img, Color::BLACK, 8.0), 8)).unwrap();
    assert_eq!(px(&host, 4, 4), [255, 255, 255, 255], "background untouched");
}
...
fn soft_mask_scales_alpha() {
    ...
    let img = image_ir(1, 1, 8, ImageColorSpace::Rgb, vec![255, 0, 0], false, Some(smask));
    let backend = CpuBackend::default();
    let (host, _) = backend.render_to_host(&request(page(img, Color::BLACK, 8.0), 8)).unwrap();
    assert_eq!(px(&host, 4, 4), [255, 255, 255, 255], "smask alpha 0 → fully transparent");
}
...
fn minified_images_are_area_averaged_not_point_sampled() {
    ...
    let img = image_ir(64, 64, 1, ImageColorSpace::Gray, checkerboard(64, 64), false, None);
    let backend = CpuBackend::default();
    let (host, _) = backend.render_to_host(&request(page(img, Color::BLACK, 8.0), 8)).unwrap();
...
fn minified_stencils_get_soft_edges_from_coverage() {
    ...
    let img = image_ir(64, 64, 1, ImageColorSpace::Gray, checkerboard(64, 64), true, None);
    let backend = CpuBackend::default();
    let (host, _) = backend
    ...
    assert!((90..=170).contains(&(p[0] as i32)), "stencil coverage averaged: {p:?}");
}
...
fn sub_byte_and_byte_aligned_samples_agree() {
    ...
    let eight = image_ir(2, 1, 8, ImageColorSpace::Gray, vec![0x00, 0xFF], false, None);
    let backend = CpuBackend::default();
    let a = backend.render_to_host(&request(page(four, Color::BLACK, 8.0), 8)).unwrap().0;
    let b = backend.render_to_host(&request(page(eight, Color::BLACK, 8.0), 8)).unwrap().0;
...
fn sixteen_bit_samples_read_big_endian() {
    ...
    let img = image_ir(2, 1, 16, ImageColorSpace::Gray, vec![0x00, 0x00, 0xFF, 0xFF], false, None);
    let backend = CpuBackend::default();
    let (host, _) = backend.render_to_host(&request(page(img, Color::BLACK, 8.0), 8)).unwrap();
...
impl pdf_image::ImageCodec for StubJpxCodec {
    ...
    fn decode(
        ...
        Ok(pdf_image::DecodedImage {
            ...
            format: self.format,
            stride,
            data: Arc::from(vec![255u8; stride * h as usize]),
        })
    }
}
...
fn backend_with_stub_jpx(format: pdf_image::DecodedFormat, comps: usize) -> CpuBackend {
    let codecs = pdf_image::CodecRegistry::new([
        Arc::new(StubJpxCodec { format, comps }) as Arc<dyn pdf_image::ImageCodec>,
    ]);
    CpuBackend::new(pdf_render_cpu::CpuBackendOptions { codecs, ..Default::default() })
}
...
fn successful_image_draw_counts_coverage() {
    ...
    let (_host, stats) = backend
        .render_to_host(&request(page(jpx_image(ImageColorSpace::Rgb), Color::BLACK, 8.0), 8))
        .unwrap();
    assert!(stats.covered_pixels > 0, "a successful image draw marks coverage");
    assert!(!stats.is_silent_blank(), "a page that painted an image is not a silent blank");
}
...
fn image_only_page_with_one_drop_and_one_paint_is_not_silent_blank() {
    ...
    assert!(stats.covered_pixels > 0, "the successful image marked coverage");
    assert!(!stats.is_silent_blank(), "a page that painted content is not a silent blank");
}
...
fn image_only_page_with_only_a_drop_is_silent_blank() {
    ...
    assert!(stats.is_silent_blank(), "a page blanked by a dropped draw is a silent blank");
}
...
fn indexed_over_multicomponent_codec_is_dropped_and_tracked() {
    ...
    assert_eq!(stats.degraded_draws, 1, "the mismatched draw must be counted, not silent");
    assert!(
        stats.recovery_notes.iter().any(|n| n.contains("Indexed") && n.contains("multi-component")),
        "a human-readable reason names the mismatch: {:?}",
        stats.recovery_notes
    );
    assert_eq!(px(&host, 4, 4), [255, 255, 255, 255], "no white overlay was painted");
}
...
fn indexed_over_single_component_codec_still_reinterprets() {
    ...
    assert_eq!(stats.degraded_draws, 0, "a single-component codec honors the palette, no drop");
    assert_eq!(px(&host, 4, 4), [0, 0, 255, 255], "palette reinterpreted, draw painted");
}
...
fn fractional_image_edge_paints_partial_coverage() {
    ...
    let page = {
        ...
        p.operations = Arc::from([
            DisplayOp::ConcatTransform(Matrix::scale(11.5, 16.0).then(Matrix::translate(2.25, 0.0))),
            DisplayOp::DrawImage {
                image: pdf_page_ir::ImageId(0),
                paint: pdf_page_ir::PaintId(0),
                transform: Matrix::IDENTITY,
    ...
    let p = px(&host, 13, 8);
    assert!((p[0] as i32 - 64).abs() <= 3, "right edge at ~75% coverage: {p:?}");
}
...
fn rotated_image_edges_are_antialiased() {
    ...
    let rot = Matrix { a: s, b: s, c: -s, d: s, e: 8.0, f: 0.0 };
    ...
    assert!(intermediate >= 8, "diagonal edges antialiased: {intermediate} intermediate pixels");
