...
fn rect_page(x0: f64, y0: f64, x1: f64, y1: f64, color: Color, alpha: f32) -> CompiledPage {
    CompiledPage {
        ...
        bounds: PageBounds { crop: Rect { x0: 0.0, y0: 0.0, x1: 4.0, y1: 4.0 }, rotate: 0 },
...
fn request(page: CompiledPage, dim: u32, format: OutputFormat, background: Background) -> RenderRequest {
    RenderRequest {
        page: Arc::new(page),
...
fn render(page: CompiledPage, dim: u32, format: OutputFormat, bg: Background) -> HostPage {
    CpuBackend::default().render_to_host(&request(page, dim, format, bg)).unwrap().0
}

// §2/§4: background application + premultiplied output.
#[test]
fn backgrounds_are_applied_before_painting() {
    let white = render(empty_page(), 2, OutputFormat::Rgba8PremultipliedSrgb, Background::White);
    assert!(white.pixels.iter().all(|&b| b == 0xFF), "white is opaque 0xFF");

    let clear = render(empty_page(), 2, OutputFormat::Rgba8PremultipliedSrgb, Background::Transparent);
    assert!(clear.pixels.iter().all(|&b| b == 0x00), "transparent is all zero");
    ...
    assert_eq!(px(&solid, 0, 0), [255, 0, 0, 255], "solid red, premultiplied opaque");
}
...
fn output_is_premultiplied() {
    ...
    let host = render(page, 4, OutputFormat::Rgba8PremultipliedSrgb, Background::Transparent);
    let p = px(&host, 2, 2);
...
fn half_covered_edge_is_exact() {
    ...
    let host = render(page, 4, OutputFormat::Rgba8PremultipliedSrgb, Background::Transparent);
    let c0 = px(&host, 0, 1);
    assert!((c0[0] as i32 - 128).abs() <= 1 && (c0[3] as i32 - 128).abs() <= 1, "col0 ~50%: {c0:?}");
...
fn rendering_is_deterministic() {
    ...
    let a = render(make(), 4, OutputFormat::Rgba8PremultipliedSrgb, Background::White);
    let b = render(make(), 4, OutputFormat::Rgba8PremultipliedSrgb, Background::White);
