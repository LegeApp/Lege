...
use pdf_page_ir::{
    ...
    PageFeatures, Paint, PathData, PathVerb, Point, Rect, ResourceKey, TilingId, TilingPattern,
};
use pdf_render_api::{
...
fn page_with_tiling(tiling: TilingPattern) -> CompiledPage {
    CompiledPage {
        ...
        paints: Arc::from([Paint::Pattern { tiling: TilingId(0), matrix: Matrix::IDENTITY }]),
        stroke_styles: Arc::from([]),
        glyph_runs: Arc::from([]),
        fonts: Arc::from([]),
        images: Arc::from([]),
...
fn colored_tiling_replicates_cell_across_fill() {
    ...
    let (host, _) = backend.render_to_host(&request(page_with_tiling(tiling), 12)).unwrap();
    ...
    assert_eq!(px(&host, 3, 3), [255, 255, 255, 255], "cell gap stays white");
    // Outside the fill shape (0,0) stays white.
    assert_eq!(px(&host, 0, 0), [255, 255, 255, 255], "outside fill untouched");
}
...
fn uncolored_tiling_uses_under_color() {
    ...
    let backend = CpuBackend::default();
    let (host, _) = backend.render_to_host(&request(page_with_tiling(tiling), 12)).unwrap();
    assert_eq!(px(&host, 1, 1), [0, 255, 0, 255], "stencil painted with under-color");
