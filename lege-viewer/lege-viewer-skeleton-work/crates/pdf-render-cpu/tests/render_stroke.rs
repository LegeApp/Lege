...
fn line(pts: &[(f64, f64)]) -> PathData {
    ...
    PathData {
        verbs: verbs.into(),
        points: pts.iter().map(|&(x, y)| Point { x, y }).collect::<Vec<_>>().into(),
    }
}
...
fn horizontal_line_becomes_a_band() {
    ...
    let host = render(page(20.0, line(&[(2.0, 10.0), (18.0, 10.0)]), style(6.0, LineCap::Butt, LineJoin::Miter, &[])), 20);
...
fn square_cap_extends_past_the_end() {
    ...
    let host = render(page(20.0, line(&[(5.0, 10.0), (15.0, 10.0)]), style(4.0, LineCap::Square, LineJoin::Miter, &[])), 20);
    assert!(painted(&host, 16, 10), "square cap extends past the endpoint");
...
fn miter_join_fills_the_corner() {
    ...
    let host = render(
        page(24.0, line(&[(4.0, 4.0), (14.0, 4.0), (14.0, 14.0)]), style(4.0, LineCap::Butt, LineJoin::Miter, &[])),
        24,
    );
...
fn dashes_produce_gaps() {
    ...
    let host = render(page(40.0, line(&[(2.0, 20.0), (38.0, 20.0)]), style(4.0, LineCap::Butt, LineJoin::Miter, &[4.0, 4.0])), 40);
    let mut on = 0;
    let mut off = 0;
    for x in 2..38 {
...
fn thin_line_stays_visible() {
    ...
    let host = render(page(20.0, line(&[(2.0, 10.0), (18.0, 10.0)]), style(0.0, LineCap::Butt, LineJoin::Miter, &[])), 20);
...
fn anisotropic_ctm_stretches_the_pen_into_an_ellipse() {
    ...
    let v = page_with_ctm(64.0, ctm, line(&[(4.0, 8.0), (4.0, 56.0)]), style(1.0, LineCap::Butt, LineJoin::Miter, &[]));
    let hv = render(v, 64);
    let w = row_run(&hv, 32, 64);
    assert!((7..=10).contains(&w), "vertical line under scale(8,1) should be ~8px wide, got {w}");

    // Horizontal line at user y=32 -> device thickness 1*1 = 1px.
    let h = page_with_ctm(64.0, ctm, line(&[(1.0, 32.0), (7.0, 32.0)]), style(1.0, LineCap::Butt, LineJoin::Miter, &[]));
    let hh = render(h, 64);
    let t = col_run(&hh, 32, 64);
    assert!((1..=3).contains(&t), "horizontal line under scale(8,1) should be ~1px thick, got {t}");
...
fn painted_count(host: &HostPage, dim: usize) -> usize {
    (0..dim).map(|y| (0..dim).filter(|&x| painted(host, x, y)).count()).sum()
}
...
fn absurd_width_does_not_flood_the_page() {
    ...
    let dim = 64;
    let host = render(
        page(64.0, line(&[(20.0, 30.0), (44.0, 30.0)]), style(11_016_766.0, LineCap::Butt, LineJoin::Miter, &[])),
        dim,
    );
    let n = painted_count(&host, dim as usize);
    assert!(n < (dim * dim / 4) as usize, "absurd width flooded the page: {n}/{} px", dim * dim);
    assert!(painted(&host, 32, 30), "the line still renders as a hairline");
}
...
fn garbage_anisotropic_ctm_does_not_flood() {
    ...
    let ctm = Matrix { a: 50_288_544.0, b: 0.0, c: 0.0, d: 0.028_854_4, e: 0.0, f: 0.0 };
    ...
    let dim = 64;
    let host = render(page_with_ctm(64.0, ctm, path, style(24.552, LineCap::Butt, LineJoin::Miter, &[])), dim);
    let n = painted_count(&host, dim as usize);
    assert!(n < (dim * dim / 4) as usize, "garbage CTM flooded the page: {n}/{} px", dim * dim);
}
...
fn off_viewport_coordinates_are_dropped() {
    ...
    let dim = 64;
    let host = render(
        page(64.0, line(&[(10.0, 10.0), (1.0e9, 1.0e9)]), style(2.0, LineCap::Butt, LineJoin::Miter, &[])),
        dim,
    );
    let n = painted_count(&host, dim as usize);
    assert!(n < (dim * dim / 4) as usize, "off-viewport garbage flooded the page: {n}/{} px", dim * dim);
}
...
fn legitimate_thick_stroke_is_not_clamped() {
    ...
    let dim = 64;
    let host = render(
        page(64.0, line(&[(32.0, 8.0), (32.0, 56.0)]), style(12.0, LineCap::Butt, LineJoin::Miter, &[])),
        dim,
    );
    let w = row_run(&host, 32, dim as usize);
    assert!((10..=14).contains(&w), "12px stroke should stay ~12px wide, got {w}");
}
...
fn uniform_ctm_keeps_a_circular_pen() {
    ...
    let v = page_with_ctm(64.0, ctm, line(&[(4.0, 2.0), (4.0, 14.0)]), style(1.0, LineCap::Butt, LineJoin::Miter, &[]));
    let hv = render(v, 64);
    let w = row_run(&hv, 32, 64);
    let h = page_with_ctm(64.0, ctm, line(&[(2.0, 8.0), (14.0, 8.0)]), style(1.0, LineCap::Butt, LineJoin::Miter, &[]));
    let hh = render(h, 64);
    let t = col_run(&hh, 32, 64);
    assert_eq!(w, t, "uniform scale: horizontal and vertical thickness must match");
    assert!((3..=6).contains(&w), "width 1 under scale(4) should be ~4px, got {w}");
}
