...
fn render_run(glyphs: Vec<PlacedGlyph>, font_size: f64, render_mode: u8, dim: u32) -> HostPage {
    ...
    let page = CompiledPage {
        ...
        fonts: Arc::from([FontResource { key: ResourceKey { object_number: 0, generation: 0, variant: 0 }, program: Vec::new().into(), face_index: 0 , synthetic_shear: 0.0, synthetic_embolden_em: 0.0 }]),
        images: Arc::from([]),
        masks: Arc::from([]),
        groups: Arc::from([]),
        shadings: Arc::from([]),
...
fn glyphs_paint_boxes_with_gaps() {
    ...
    let host = render_run(vec![glyph(65, 0.0), glyph(66, 24.0), glyph(67, 48.0)], 20.0, 0, 80);
    ...
    assert!(!column_painted(&host, 24, 4, 16), "gap between glyphs is empty");
}
...
fn spaces_leave_gaps() {
    ...
    let host = render_run(vec![glyph(65, 0.0), glyph(32, 24.0), glyph(67, 48.0)], 20.0, 0, 80);
    // Column over the space glyph position (user x≈28) stays blank.
    assert!(!column_painted(&host, 28, 4, 16), "space glyph draws no box");
