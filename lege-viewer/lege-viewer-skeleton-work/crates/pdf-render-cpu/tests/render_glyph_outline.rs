...
fn render_synth(program: Arc<[u8]>, dim: u32, shear: f32, embolden_em: f32) -> HostPage {
    ...
    let page = CompiledPage {
        ...
        operations: Arc::from([DisplayOp::DrawGlyphRun { run: GlyphRunId(0), paint: PaintId(0), alpha: 1.0, blend: pdf_page_ir::BlendMode::Normal, stroke: None }]),
        paths: Arc::from([]),
        paints: Arc::from([Paint::Solid(Color::from_rgb(0.0, 0.0, 0.0))]),
        stroke_styles: Arc::from([]),
        glyph_runs: Arc::from([run]),
        fonts: Arc::from([FontResource { key: ResourceKey { object_number: 0, generation: 0, variant: 0 }, program, face_index: 0, synthetic_shear: shear, synthetic_embolden_em: embolden_em }]),
        images: Arc::from([]),
        masks: Arc::from([]),
        groups: Arc::from([]),
        shadings: Arc::from([]),
...
fn embedded_outline_renders_the_glyph_shape() {
    ...
    assert!(!painted(&host, 160, 610), "outside the triangle (bbox corner)");
    assert!(!painted(&host, 850, 610), "outside the triangle (other corner)");
}
...
fn render_stroke(width: f64, dim: u32) -> HostPage {
    ...
    let page = CompiledPage {
        ...
        operations: Arc::from([DisplayOp::DrawGlyphRun {
            ...
            stroke: Some(pdf_page_ir::GlyphStroke { paint: PaintId(0), style: pdf_page_ir::StrokeStyleId(0), alpha: 1.0 }),
        }]),
        paths: Arc::from([]),
        paints: Arc::from([Paint::Solid(Color::from_rgb(0.0, 0.0, 0.0))]),
        stroke_styles: Arc::from([style]),
        glyph_runs: Arc::from([run]),
        fonts: Arc::from([FontResource { key: ResourceKey { object_number: 0, generation: 0, variant: 0 }, program: minimal_ttf().into(), face_index: 0, synthetic_shear: 0.0, synthetic_embolden_em: 0.0 }]),
        images: Arc::from([]),
        masks: Arc::from([]),
        groups: Arc::from([]),
        shadings: Arc::from([]),
...
fn stroke_render_mode_outlines_the_glyph() {
    ...
    assert!(!painted(&stroke, 510, 200), "stroke leaves the interior white");
    // The stroke still marks the outline: the bottom edge near a vertex.
    assert!(painted(&stroke, 200, 68) || painted(&stroke, 150, 68), "stroke marks the base edge");
...
fn no_program_falls_back_to_boxes() {
    ...
    let any = (0..1000).step_by(20).any(|y| (0..1000).step_by(20).any(|x| painted(&host, x, y)));
...
fn synthetic_oblique_shears_toward_the_ascender() {
    ...
    assert!(!painted(&base, x, y), "upright glyph does not reach ({x},{y})");
    ...
    assert!(!painted(&slanted, 510, 640), "sheared glyph vacates the upright apex");
}
