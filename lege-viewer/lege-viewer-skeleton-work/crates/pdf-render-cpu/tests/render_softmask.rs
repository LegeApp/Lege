...
fn fill(path: u32, paint: u32) -> DisplayOp {
    DisplayOp::FillPath { path: PathId(path), paint: PaintId(paint), rule: FillRule::NonZero, alpha: 1.0, blend: BlendMode::Normal }
}
...
fn luminosity_mask_gates_painting() {
    ...
    let paints = vec![Paint::Solid(Color::from_rgb(1.0, 1.0, 1.0)), Paint::Solid(Color::from_rgb(1.0, 0.0, 0.0))];
    let host = render(paths, paints, ops, 20);

    assert!(red(px(&host, 5, 10)), "left half painted (mask bright): {:?}", px(&host, 5, 10));
    assert!(white(px(&host, 15, 10)), "right half masked out: {:?}", px(&host, 15, 10));
}
...
fn dark_luminosity_mask_blocks_painting() {
    ...
    assert!(white(px(&host, 10, 10)), "dark mask blocks the fill: {:?}", px(&host, 10, 10));
}
...
fn clear_soft_mask_restores_unmasked_painting() {
    ...
    assert!(red(px(&host, 10, 10)), "cleared mask → unmasked fill: {:?}", px(&host, 10, 10));
}
...
fn bc_white_backdrop_unmasks_outside_mask_content() {
    ...
    let ops = vec![
        DisplayOp::BeginSoftMask {
            kind: MaskKind::LuminosityBc { backdrop: [255, 255, 255] },
            transfer: None,
        },
        fill(0, 0), // white, left half — the mask content
        DisplayOp::EndSoftMask,
    ...
    let host = render(paths, paints, ops, 20);
    assert!(red(px(&host, 5, 10)), "inside mask content: {:?}", px(&host, 5, 10));
    assert!(red(px(&host, 15, 10)), "outside content takes /BC luminosity: {:?}", px(&host, 15, 10));
}
...
fn bc_backdrop_composites_translucent_mask_content_against_it() {
    ...
    assert!((p[1] as i32 - 128).abs() <= 4 && (p[2] as i32 - 128).abs() <= 4,
...
fn transfer_lut_inverts_a_luminosity_mask() {
    ...
    let ops = vec![
        DisplayOp::BeginSoftMask { kind: MaskKind::Luminosity, transfer: Some(invert) },
        fill(0, 0), // white, left half — the mask content
        DisplayOp::EndSoftMask,
    ...
    assert!(white(px(&host, 5, 10)), "left half now blocked: {:?}", px(&host, 5, 10));
    assert!(red(px(&host, 15, 10)), "right half now painted: {:?}", px(&host, 15, 10));
}
