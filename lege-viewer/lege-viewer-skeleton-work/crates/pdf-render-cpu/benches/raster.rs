...
use pdf_page_ir::{
    ...
    PageFeatures, Paint, PathData, PathId, PaintId, PathVerb, Point, Rect,
};
use pdf_render_api::{
...
fn synthetic_page(size: f64, n: usize, tris: usize) -> CompiledPage {
    ...
    let push_path = |verbs: Vec<PathVerb>, points: Vec<Point>, paths: &mut Vec<PathData>| -> PathId {
        let id = PathId(paths.len() as u32);
        paths.push(PathData { verbs: verbs.into(), points: points.into() });
        id
    };
    ...
    for _ in 0..n {
        ...
        verbs.clear();
        points.clear();
        verbs.extend_from_slice(&[PathVerb::MoveTo, PathVerb::LineTo, PathVerb::LineTo, PathVerb::LineTo, PathVerb::Close]);
        ...
        paints.push(Paint::Solid(Color { r: rng.next_f32(), g: rng.next_f32(), b: rng.next_f32(), a: 1.0 }));
        ops.push(DisplayOp::FillPath { path: pid, paint, rule: FillRule::NonZero, alpha: 0.6 + rng.next_f32() * 0.4, blend: pdf_page_ir::BlendMode::Normal });
    }
    ...
    for _ in 0..tris {
        ...
        verbs.clear();
        points.clear();
        verbs.extend_from_slice(&[PathVerb::MoveTo, PathVerb::LineTo, PathVerb::LineTo, PathVerb::Close]);
        points.extend_from_slice(&[
            Point { x, y },
            Point { x: x + r, y: y + r * 0.5 },
            Point { x: x - r * 0.3, y: y + r },
        ]);
        let pid = push_path(verbs.clone(), points.clone(), &mut paths);
        let paint = PaintId(paints.len() as u32);
        paints.push(Paint::Solid(Color { r: rng.next_f32(), g: rng.next_f32(), b: rng.next_f32(), a: 1.0 }));
        ops.push(DisplayOp::FillPath { path: pid, paint, rule: FillRule::NonZero, alpha: 0.8, blend: pdf_page_ir::BlendMode::Normal });
    }
...
fn main() {
    ...
    bench_case("500-rect page", 512, small.clone(), RenderQuality::Normal, 200);
    bench_case("500-rect page (draft)", 512, small, RenderQuality::Draft, 200);
    bench_case("2000-rect page", 1024, big.clone(), RenderQuality::Normal, 50);
    bench_case("2000-rect page (draft)", 1024, big, RenderQuality::Draft, 50);
}
