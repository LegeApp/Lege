...
use pdf_render_api::{
    BackendCapabilities, BackendId, HostPage, OutputFormat, RenderBackend, RenderError,
...
impl CpuBackend {
    ...
    pub fn render_with(
        ...
        let out_bytes = out_stride
            .checked_mul(height as usize)
            .ok_or(RenderError::LimitExceeded("output size overflow"))?;
        // The internal surface is always RGBA8; bound against that.
        let surface_bytes = (width as usize)
            .checked_mul(height as usize)
...
mod tests {
    ...
    fn respects_memory_limit() {
        ...
        assert!(matches!(
            render_blocking(&backend, request),
            Err(RenderError::LimitExceeded("max_page_bytes"))
        ));
    }
