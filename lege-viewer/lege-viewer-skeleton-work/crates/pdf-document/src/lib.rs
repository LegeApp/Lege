...
impl ParseContext {
    ...
    /// Reset per-job limits, recursion state, diagnostics, and profiling
    /// counters while retaining the worker-local parsed-object and object-
    /// stream caches. Long-lived viewer compile workers call this before each
    /// page so cache warmth does not make safety budgets accumulate forever.
...
impl DocumentSnapshot {
    ...
    pub fn open_with_password(
        ...
        let mut structure =
            pdf_structure::load_structure(source.as_ref(), &names, &structure_limits)?;

        // Open-time resolution shares one context; its repairs are folded
        // into the snapshot's recovery log below (post-open, repairs go to
        // each worker's own context instead — the snapshot is frozen).
        let mut ctx = ParseContext::new();

        // Establish the security context *before* building the page tree: page
        // objects' strings and content streams are encrypted and must be
        // decrypted as they resolve. The `/Encrypt` dictionary is read through
        // a resolver with no handler, since it is never itself encrypted.
        let security = {
            let plain = Resolver {
            ...
            build_security(&plain, &structure, &names, &mut ctx, password)?
        };
        ...
        let (pages, ocg) = {
            ...
            let pages = pages::build_page_tree(&resolver, &mut ctx)?;
            let ocg = ocg::parse_ocproperties(&resolver, &mut ctx);
            (pages, ocg)
        };
...
fn build_security(
    ...
    {
        return Ok(Some(SecurityContext::standard(scheme, Permissions(enc.p as u32), handler)));
    }
