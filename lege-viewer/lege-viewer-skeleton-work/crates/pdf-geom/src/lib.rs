...
impl Matrix {
    ...
    pub fn is_stably_invertible(&self) -> bool {
        ...
        let scale = self.a.abs().max(self.b.abs()).max(self.c.abs()).max(self.d.abs());
...
mod tests {
    ...
    fn matrix_invert_roundtrips() {
        let m = Matrix { a: 2.0, b: 0.5, c: -1.0, d: 3.0, e: 10.0, f: -4.0 };
        ...
        assert!(Matrix { a: 1.0, b: 2.0, c: 2.0, d: 4.0, e: 0.0, f: 0.0 }.invert().is_none());
    }
    ...
    fn small_but_valid_matrices_stay_invertible() {
        ...
        assert!(m.is_stably_invertible(), "a pure scale is well-conditioned at any size");
    }
    ...
    fn stability_is_scale_relative_not_absolute() {
        ...
        let ill = Matrix { a: 1e8, b: 1e8, c: 1e8, d: 1e8 * (1.0 + 1e-15), e: 0.0, f: 0.0 };
        assert!(ill.determinant().abs() > 1e-12, "sails past a fixed epsilon");
        assert!(!ill.is_stably_invertible(), "but must be rejected as ill-conditioned");
        ...
        let big = Matrix { a: 1e8, b: 0.0, c: 0.0, d: 1e8, e: 0.0, f: 0.0 };
        assert!(big.is_stably_invertible(), "a big pure scale is well-conditioned");
    }
    ...
    fn non_finite_matrices_are_rejected() {
        ...
        for bad in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
            let m = Matrix { a: bad, ..Matrix::IDENTITY };
            assert!(!m.is_finite());
        ...
        // left open.
        let nan_det = Matrix { a: f64::NAN, d: f64::NAN, ..Matrix::IDENTITY };
        let old_guard_would_fire = nan_det.determinant().abs() < 1e-12;
        assert!(!old_guard_would_fire, "every NaN comparison is false, so the old guard passed it");
        assert!(nan_det.invert().is_none(), "the finiteness check catches what magnitude cannot");
    }
    ...
    #[test]
    fn rect_ops() {
        let r = Rect { x0: 10.0, y0: 10.0, x1: 0.0, y1: 0.0 }.normalized();
        assert_eq!(r.width(), 10.0);
        let other = Rect { x0: 5.0, y0: 5.0, x1: 20.0, y1: 20.0 };
        assert_eq!(
            r.intersect(other),
            Some(Rect { x0: 5.0, y0: 5.0, x1: 10.0, y1: 10.0 })
        );
        assert!(r.intersect(Rect { x0: 11.0, y0: 11.0, x1: 12.0, y1: 12.0 }).is_none());
    }
}
