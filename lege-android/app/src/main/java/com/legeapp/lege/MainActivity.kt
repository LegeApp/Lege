...
class MainActivity : android.app.Activity() {
    ...
    private lateinit var outputFormat: SegmentedChoice; private lateinit var imageHandling: SegmentedChoice; private lateinit var ocrMode: SegmentedChoice; private lateinit var marginMode: SegmentedChoice; private lateinit var binarizationMode: SegmentedChoice; private lateinit var binarizationOptions: LinearLayout
    ...
    private fun toggle(name: String, hint: String?, initial: Boolean): SquareToggle {
        val box = SquareToggle(initial); val row = row(name, hint, box).apply { setOnClickListener { box.flip() } }; formBody.addView(row, lp(match, wrap)); box.row = row; return box
    }
    ...
    private fun start(input: File, outputFolder: Uri?) {
        ...
        // `image/vnd.djvu` is the IANA-registered type; the Storage Access
        // Framework uses it to pick the extension when a provider renames.
        val extension = if (outputFormat.value == 1) "djvu" else "pdf"
        val mimeType = if (extension == "djvu") "image/vnd.djvu" else "application/pdf"
        val output = runCatching { createOutputDocument(outputFolder, "$stem.$extension", mimeType) }.getOrElse { notice("Could not create the ${extension.uppercase()} output."); return }
    ...
    private fun params(epubOut: File?) = LegeParams().apply { val height = this@MainActivity.targetHeight.text.toString().toIntOrNull()?.takeIf { it > 0 } ?: 1200; this.targetHeight = height; getSharedPreferences("lege", MODE_PRIVATE).edit().putInt("target_height", height).apply(); enableLayoutDetection = layout.checked; enableOcr = ocrMode.value >= 0 || epub.checked; highQualityOutput = highQuality.checked; invertInput = inverted.checked; this.pageRange = this@MainActivity.pageRange.text.toString().ifBlank { null }; marginMode = when (this@MainActivity.marginMode.value) { 0 -> "center"; 1 -> "crop"; 2 -> "reflow"; else -> "none" }; slowOcr = ocrMode.value == 1 || epub.checked; outputFormat = if (this@MainActivity.outputFormat.value == 1) "djvu" else "pdf"; jpegCompat = this@MainActivity.jpegCompat.checked; ditherImages = this@MainActivity.imageHandling.value == 1; binarizationMode = when (this@MainActivity.binarizationMode.value) { 0 -> "adaptive"; 1 -> "threshold"; 2 -> "sauvola"; else -> "default" }; if (this@MainActivity.binarizationMode.value == 0) sauvolaK = this@MainActivity.sauvolaK.text.toString().toFloatOrNull()?.coerceIn(0f, 1f) ?: 0.05f; if (this@MainActivity.binarizationMode.value == 1) fixedThreshold = this@MainActivity.fixedThreshold.text.toString().toIntOrNull()?.coerceIn(0, 255) ?: 180; epubSidecarPath = epubOut?.absolutePath }
    ...
    private inner class SquareToggle(initial: Boolean) : TextView(this@MainActivity) { var checked = initial; private set; var row: View? = null; var onChanged: ((Boolean) -> Unit)? = null; init { gravity = Gravity.CENTER; textSize = 16f; render() }; fun flip() = setChecked(!checked); fun setChecked(value: Boolean) { checked = value; render(); onChanged?.invoke(value) }; private fun render() { text = if (checked) "✓" else ""; setTextColor(c(LIGHT)); background = bg(if (checked) ACCENT else FIELD, 2) } }
