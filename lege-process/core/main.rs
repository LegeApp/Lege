...
struct CliOptions {
    ...
    grayscale: bool, // --grayscale: MRC grayscale-clean text (JBIG2 mask + JP2 gray bg)
    music_sheet: bool, // --music-sheet: sheet-music preset (no layout, OCR off, adaptive binarization default)
    // Undocumented MRC tuning knobs for development/testing.
    mrc_bg_quality: Option<u8>,    // --mrc-bg-quality <1-100>
    mrc_bg_subsample: Option<u8>,  // --mrc-subsample <1-12>
    mrc_ink_threshold: Option<u8>, // --mrc-ink-threshold <0-255>
    mrc_adaptive_mask: bool,       // --mrc-adaptive-mask: Sauvola mask over cleaned-gray bg
...
fn extract_cli_options(args: Vec<String>) -> Result<(Vec<String>, CliOptions)> {
    ...
    while i < args.len() {
        ...
        match arg.as_str() {
            ...
            "--music-sheet" => {
                opts.music_sheet = true;
                i += 1;
            }
            "--mrc-bg-quality" => {
                let val = args
                    .get(i + 1)
                    .ok_or_else(|| anyhow!("Missing value after --mrc-bg-quality"))?;
                opts.mrc_bg_quality = Some(val.parse()?);
                i += 2;
            }
            "--mrc-subsample" => {
                let val = args
                    .get(i + 1)
                    .ok_or_else(|| anyhow!("Missing value after --mrc-subsample"))?;
                opts.mrc_bg_subsample = Some(val.parse()?);
                i += 2;
            }
            "--mrc-ink-threshold" => {
                let val = args
                    .get(i + 1)
                    .ok_or_else(|| anyhow!("Missing value after --mrc-ink-threshold"))?;
                opts.mrc_ink_threshold = Some(val.parse()?);
                i += 2;
            }
            "--mrc-adaptive-mask" => {
                opts.mrc_adaptive_mask = true;
                i += 1;
            }
            "--fast-resize" => {
...
/// Normalize `CliOptions` for the sheet-music preset:
/// - Layout detection is disabled because DocLayout-YOLO misreads staff
///   systems as figures and pastes un-cleaned raster crops over the
///   normalized background (the "gray splotch" artifact).
/// - OCR is off by default (staves aren't text; explicit `--ocr` still wins).
/// - Binarization defaults to `adaptive`, the customer-validated mode, but
///   every `--binarization` mode (adaptive/fixed/heavy) stays usable and an
///   explicit choice wins. A bare `--threshold`/`--sauvola-k` (no method) is
///   left to the standalone handling in `cli_binarization_config`, and an
///   explicit `--grayscale` keeps working as plain grayscale.
///
/// Incompatible flags are dropped with a warning on stderr so `--gui-worker`
...
fn apply_music_sheet_preset(opts: &mut CliOptions) {
    ...
    drop("--reflow", &mut opts.reflow, &mut dropped);
    drop("--invert", &mut opts.invert, &mut dropped);
    drop("--halftone", &mut opts.halftone, &mut dropped);
    drop("--dither", &mut opts.dither, &mut dropped);
    drop("--gray-jp2", &mut opts.gray_jp2, &mut dropped);
    drop("--center-margins", &mut opts.center_margins, &mut dropped);
    drop("--crop-margins", &mut opts.crop_margins, &mut dropped);
    drop("--force-crop", &mut opts.force_crop, &mut dropped);
    drop(
        "--crop-free-aspect",
        &mut opts.crop_free_aspect,
        &mut dropped,
    );
    drop("--image-only", &mut opts.image_only, &mut dropped);
    if opts.text_format.as_deref() == Some("epub") {
        opts.text_format = None;
        dropped.push("--text-format epub");
    }
    if opts.layout_exclusion_pages.take().is_some() {
        dropped.push("--exclude-layout");
    }
    if !dropped.is_empty() {
        eprintln!(
            "Warning: --music-sheet ignores incompatible option(s): {}",
            dropped.join(", ")
        );
    }

    opts.no_layout = true;
    // Default to adaptive binarization — the customer-validated mode — unless
    // the user picked a binarization mode, a standalone threshold/k (handled
    // by `cli_binarization_config`), or explicit grayscale.
    if opts.binarization.is_none()
        && !opts.grayscale
        && opts.threshold.is_none()
        && opts.sauvola_k.is_none()
    {
        opts.binarization = Some("adaptive".to_string());
    }
    if opts.ocr.is_none() {
        opts.ocr = Some(false);
        opts.ocr_mode = None;
...
fn cli_binarization_config(cli_opts: &CliOptions) -> Option<lege::color::BinarizationConfig> {
    ...
    Some(lege::CliConfigBuilder::parse_binarization_method(
        &method_str,
    ))
}
...
fn handle_simple_processing(
    ...
    for raw in input_paths {
        ...
        let job = prepare_simple_input(&cleaned)?;
    ...
    // Grayscale-clean / MRC mode: text becomes a JBIG2 mask over a JP2 gray
    // background. Overrides bilevel binarization. Image regions keep the
    // cover-format default (Jpeg variant → JP2 for regions, or JPEG under
    // --jpeg-compat) — jpeg-compat affects image regions only, never the
    // grayscale text background.
    if cli_opts.grayscale {
        pipeline_config.set_page_mode(lege::pipeline::PageMode::Grayscale);
        pipeline_config.set_keep_original_images(true);
    }
    if let Some(q) = cli_opts.mrc_bg_quality {
        pipeline_config.set_mrc_bg_quality(q);
    }
    if let Some(s) = cli_opts.mrc_bg_subsample {
        pipeline_config.set_mrc_bg_subsample(s);
    }
    if let Some(t) = cli_opts.mrc_ink_threshold {
        pipeline_config.set_mrc_ink_threshold(t);
    }
    if cli_opts.mrc_adaptive_mask {
        pipeline_config.set_mrc_adaptive_mask(true);
        // The mask binarizer must be local-adaptive Sauvola; without an
        // explicit --binarization the config default is fixed-threshold,
        // which on the raw (un-normalized) render is wrong for this path.
        if cli_opts.binarization.is_none() {
            pipeline_config
                .set_binarization(lege::CliConfigBuilder::parse_binarization_method("1"));
        }
    }
...
fn handle_cli_mode(config: AppConfig, pause_on_exit: bool) -> Result<()> {
    match run_cli()? {
        Some((file_path, pipeline_config)) => {
            ...
            let input_job = prepare_simple_input(
                file_path
                    .to_str()
...
fn run_cli() -> Result<Option<(PathBuf, PipelineConfig)>> {
    ...
    let binarization_method = if layout_detection_enabled {
        ...
        println!(
            "{}  g = Grayscale/MRC — keeps text antialiasing (JBIG2 mask over JP2 gray; images JP2){}",
            cli_color(COLORS.highlight),
            cli_reset()
        );
        print!(
    ...
    // Rendering mode: grayscale/MRC vs bilevel binarization.
    if binarization_method == "grayscale" {
        // Text becomes a JBIG2/JB2 ink mask over a JP2/IW44 grayscale
        // background. Overrides bilevel binarization; symbol mode is not used
        // for the mask. Image regions keep the selected cover format (JP2 for
        // regions by default; jpeg-compat affects image regions only).
        config.set_page_mode(lege::pipeline::PageMode::Grayscale);
        config.set_keep_original_images(true);
    } else if !no_binarization {
...
fn process_input_job(
    ...
    match input {
        ...
        SimpleInput::Zip(path) => {
            run_zip_mode_with_config(path, Some(output_path), pipeline_config, None)
        }
    }
}
...
fn process_input_job_json(
    ...
    let mut log_options = history_log::from_pipeline_config(&pipeline_config);
    ...
    let terminal = match input {
        ...
        SimpleInput::Zip(path) => {
            ...
            let worker_pipeline_config = pipeline_config.clone();
            std::thread::spawn(move || {
                match run_zip_mode_with_config(
                    path,
                    Some(worker_output_path),
                    worker_pipeline_config,
                    Some(tracker_for_thread.clone()),
                ) {
                    Ok(()) => tracker_for_thread.finish("ZIP image processing completed"),
                    Err(e) => tracker_for_thread.finish_with_error(e),
                }
            });
            worker_json::run_json_worker_with_terminal_hook(&receiver, task_id, |update| {
...
fn prepare_simple_input(path: &str) -> Result<SimpleInput> {
    ...
    if path_buf.is_dir() {
        return Ok(SimpleInput::ImageFolder {
    ...
    match ext.as_str() {
        "pdf" => {
            ...
            Ok(SimpleInput::Pdf(path_buf))
        }
        "zip" => Ok(SimpleInput::Zip(path_buf)),
...
fn parse_format_selection_with_options(
    ...
    if input.is_empty() {
        // Default: Original images with JBIG2 text, JPEG cover, layout detection ENABLED.
        // JBIG2 has broad enough decoder support to be the default; CCITT4 (option 1)
        // remains the maximum-compatibility choice. Dithering is OFF by default.
        return Ok((
            "jbig2".to_string(),
            CoverFormat::Jpeg,
...
mod cli_parser_tests {
    ...
    #[test]
    fn cli_music_sheet_preset_defaults() {
        let (_remaining, opts) = extract_cli_options(vec![
            "lege".to_string(),
            "--music-sheet".to_string(),
            "songbook.pdf".to_string(),
        ])
        .expect("music-sheet args should parse");

        assert!(opts.music_sheet);
        assert_eq!(
            opts.binarization.as_deref(),
            Some("adaptive"),
            "preset defaults to adaptive binarization"
        );
        assert!(!opts.grayscale, "preset no longer forces grayscale");
        assert!(
            !opts.mrc_adaptive_mask,
            "preset no longer enables the grayscale adaptive mask"
        );
        assert!(opts.no_layout, "preset disables layout detection");
        assert_eq!(opts.ocr, Some(false), "preset disables OCR");
        assert!(
    ...
    #[test]
    fn cli_music_sheet_drops_incompatible_flags() {
        let (_remaining, opts) = extract_cli_options(vec![
            "lege".to_string(),
            "--music-sheet".to_string(),
            "--reflow".to_string(),
        ...
        assert_eq!(opts.binarization.as_deref(), Some("adaptive"));
        assert!(opts.no_layout);
    }

    #[test]
    fn cli_music_sheet_explicit_flags_win() {
        let (_remaining, opts) = extract_cli_options(vec![
            "lege".to_string(),
            "--music-sheet".to_string(),
        ...
        assert!(
            !opts.grayscale,
            "explicit --binarization keeps bilevel mode"
        );
        assert!(
            !opts.mrc_adaptive_mask,
            "adaptive mask only applies to the grayscale preset path"
        );
        assert_eq!(opts.binarization.as_deref(), Some("adaptive"));
        assert_eq!(opts.ocr, Some(true), "explicit --ocr wins over preset");
        assert_eq!(opts.mrc_ink_threshold, Some(170));
        assert!(opts.no_layout, "layout stays off regardless");
    }

    #[test]
    fn cli_music_sheet_binarization_mode_wins() {
        let (_remaining, opts) = extract_cli_options(vec![
            "lege".to_string(),
            "--music-sheet".to_string(),
            "--binarization".to_string(),
        ...
        assert_eq!(
            opts.binarization.as_deref(),
            Some("heavy"),
            "explicit --binarization heavy is kept as-is"
        );
        assert!(!opts.grayscale, "heavy mode does not enable grayscale");
        assert!(opts.no_layout);
    }

    #[test]
    fn cli_music_sheet_standalone_threshold_not_forced_adaptive() {
        let (_remaining, opts) = extract_cli_options(vec![
            "lege".to_string(),
            "--music-sheet".to_string(),
            "--threshold".to_string(),
        ...
        assert!(
            ...
            "a standalone --threshold stays on the fixed-threshold path"
        );
        assert_eq!(opts.threshold, Some(150));
        assert!(!opts.grayscale);
        assert!(opts.no_layout);
    }
}
...
fn parse_binarization_method(input: &str) -> Result<String> {
    ...
    let choice = match parts[0].to_lowercase().as_str() {
        ...
        // Grayscale/MRC: a distinct rendering mode rather than a binarizer.
        // Returned verbatim so the caller can switch on it.
...
fn generate_output_path(
    ...
    // For PDF: stem + timestamp only (no jbig2/ccitt4 format suffix)
    let output_filename = format!("{}_processed_{}.pdf", input_stem, timestamp);
...
fn process_single_file(
    ...
    let original_size = fs::metadata(&input_path).map(|m| m.len()).unwrap_or(0);
    let mut log_options = history_log::from_pipeline_config(&log_config);
    log_options.input_path = Some(input_path.clone());
    log_options.output_path = Some(output_path.clone());
    if let Err(err) = history_log::add_started_log_entry(
        input_path.clone(),
