...
// Bind outcome kept as a Result so a load failure is a reportable error, not a
// panic: release builds use panic = "abort", so an .expect here would kill a
// --gui-worker child before it could emit the JSON error event the GUI needs.
static PDFIUM_BIND: Lazy<Result<Pdfium>> = Lazy::new(bind_pdfium_once);

// Global Pdfium instance - MUST be accessed through proper synchronization.
// Callers should have run `ensure_pdfium_binds` (via `ensure_pdfium_available`)
// during preflight so a bind failure surfaces as a clean error there instead of
// this panic.
pub static PDFIUM: Lazy<&'static Pdfium> = Lazy::new(|| {
    PDFIUM_BIND
        .as_ref()
        .unwrap_or_else(|e| panic!("Failed to initialize Pdfium: {e:#}"))
});

/// Eagerly bind the global Pdfium library, returning the underlying error
/// instead of panicking. The dylib can exist yet still fail to load — e.g.
/// macOS library validation rejecting its code signature, or an architecture
/// mismatch — so a file-existence preflight alone is not sufficient.
pub fn ensure_pdfium_binds() -> Result<()> {
    match &*PDFIUM_BIND {
        Ok(_) => Ok(()),
        Err(e) => Err(anyhow!("{e:#}")),
    }
...
pub fn count_pdf_pages_from_bytes(pdf_bytes: &[u8]) -> Result<u16> {
    ...
    u16::try_from(document.pages().len()).map_err(|_| anyhow!("PDF has more than 65,535 pages"))
}
...
fn bind_pdfium_once() -> Result<Pdfium> {
    ...
    let bindings = Pdfium::bind_to_library(&path).map_err(|e| {
        let hint = if cfg!(target_os = "macos") {
            "\nThe library file exists but could not be loaded. On macOS this usually \
             means its code signature was rejected (e.g. hardened-runtime library \
             validation with an ad-hoc signature) or its architecture does not match \
             this binary (x86_64 vs arm64)."
        } else {
            ""
        };
        anyhow!(
            "Failed to load PDFium library at {}: {:?}{hint}",
            path.display(),
            e
        )
    })?;
