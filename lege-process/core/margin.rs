...
impl ContentBounds {
    ...
    pub fn union(&self, other: &ContentBounds) -> ContentBounds {
        ContentBounds {
    ...
    pub fn contains(&self, other: &ContentBounds) -> bool {
...
pub fn analyze_document_margins(
    ...
    for page_input in all_page_data {
        let (page_margin_data, baseline_page_data) =
            analyze_single_page_margins(page_input, config);

        // Prefer pages after the front matter, but keep an early-page fallback for
        // short documents or selected page ranges.
        if should_use_page_for_baseline(&baseline_page_data) {
            if page_input.page_index >= 5 {
                baseline_candidates.push(baseline_page_data.clone());
            }
            fallback_baseline_candidates.push(baseline_page_data);
        }
...
fn analyze_single_page_margins(
    ...
    crate::pipeline::page_analysis::maybe_apply_full_page_detection(
        &mut detections,
        page.page_width,
        page.page_height,
        config,
    ...
    let baseline_content_bounds = classification.content_bounds;
    // In layout mode `pixel_bounds` contains only substantial residual ink not
    // covered by a detection. In no-layout mode it is the complete ink bound.
    // Keep this per-page safety union out of the document baseline so a rare
    // illustration or dirty page cannot enlarge every page's crop.
    let content_bounds = match (baseline_content_bounds, page.pixel_bounds) {
        (Some(layout), Some(pixel_guard)) if !page.detections.is_empty() => {
            Some(layout.union(&pixel_guard))
        }
    ...
    let make_data = |content_bounds: Option<ContentBounds>| {
    ...
    let data = make_data(content_bounds);
    let baseline_data = make_data(baseline_content_bounds);
...
pub fn apply_margin_analysis_to_page(
    ...
    let scaled_crop_bounds = analysis.crop_bounds.scale_to_resolution(
        analysis.analysis_width.max(1),
        analysis.analysis_height.max(1),
        image_width,
        image_height,
    );
    let scaled_page_bounds = page_data.content_bounds.map(|bounds| {
    ...
    let (effective_bounds, effective_setting) =
        if analysis.effective_margin_setting == MarginSettings::CropAndResize {
            ...
            } else if scaled_page_bounds.is_some_and(|bounds| {
                bounds.width() > scaled_crop_bounds.width()
                    || bounds.height() > scaled_crop_bounds.height()
            }) {
                (
                    scaled_page_bounds.unwrap_or(scaled_crop_bounds),
                    MarginSettings::StandardizeAndCenter,
                )
            } else {
                (
                    fit_crop_window_to_content(
                        &scaled_page_bounds.unwrap_or(scaled_crop_bounds),
                        scaled_crop_bounds.width().max(1),
                        scaled_crop_bounds.height().max(1),
                        image_width,
                        image_height,
                    ),
                    MarginSettings::CropAndResize,
        ...
        } else if let Some(bounds) = scaled_page_bounds {
            (bounds, analysis.effective_margin_setting)
        } else {
            // No content detected, use baseline.
            (
                analysis.baseline_bounds.scale_to_resolution(
                    analysis.analysis_width.max(1),
                    analysis.analysis_height.max(1),
                    image_width,
                    image_height,
    ...
    let crop_standard_dims = StandardPageDimensions {
        width: scaled_crop_bounds.width().max(1),
        height: scaled_crop_bounds.height().max(1),
    };
...
mod tests {
    ...
    fn table_footnote_switches_crop_to_center_when_not_forced() {
        let pages = vec![PageMarginInput {
            ...
            page_width: 1000,
            page_height: 1400,
            detections: vec![detection("footnote", [100.0, 1200.0, 900.0, 1300.0])],
            pixel_bounds: None,
        }];
    ...
    fn early_pages_can_establish_nonzero_crop_baseline() {
        let pages = vec![
            PageMarginInput {
                ...
                page_width: 640,
                page_height: 900,
                detections: vec![detection("text", [90.0, 110.0, 520.0, 760.0])],
                pixel_bounds: None,
            },
            PageMarginInput {
                page_index: 1,
                page_width: 640,
                page_height: 900,
                detections: vec![detection("text", [95.0, 115.0, 515.0, 755.0])],
                pixel_bounds: None,
            },
        ];
    ...
    fn crop_bounds_are_uniform_for_alternating_facing_page_offsets() {
        let pages = vec![
            PageMarginInput {
                ...
                page_width: 640,
                page_height: 900,
                detections: vec![detection("text", [80.0, 100.0, 500.0, 760.0])],
                pixel_bounds: None,
            },
            PageMarginInput {
                page_index: 1,
                page_width: 640,
                page_height: 900,
                detections: vec![detection("text", [140.0, 100.0, 560.0, 760.0])],
                pixel_bounds: None,
            },
            PageMarginInput {
                page_index: 2,
                page_width: 640,
                page_height: 900,
                detections: vec![detection("image", [0.0, 0.0, 640.0, 900.0])],
                pixel_bounds: None,
            },
        ];
    ...
    fn free_aspect_crop_uses_uniform_stable_text_box_size() {
        let pages = vec![
            ...
            PageMarginInput {
                ...
                page_width: 640,
                page_height: 900,
                detections: vec![detection("doc_title", [200.0, 120.0, 440.0, 180.0])],
                pixel_bounds: None,
            },
        ];
    ...
    #[test]
    fn residual_ink_guards_page_without_expanding_document_template() {
        let layout_bounds = ContentBounds {
            min_x: 100,
            min_y: 120,
            max_x: 500,
            max_y: 720,
        };
        let residual_bounds = ContentBounds {
            min_x: 40,
            min_y: 60,
            max_x: 600,
            max_y: 820,
        ...
        let pages = vec![PageMarginInput {
            ...
            detections: vec![detection(
                "text",
                [
                    layout_bounds.min_x as f32,
                    layout_bounds.min_y as f32,
                    layout_bounds.max_x as f32,
                    layout_bounds.max_y as f32,
                ],
            )],
            pixel_bounds: Some(residual_bounds),
        }];
        ...
        let safety = analysis.pages[&0]
            .content_bounds
            .expect("page safety bounds");

        assert!(safety.contains(&layout_bounds));
        assert!(safety.contains(&residual_bounds));
        assert!(analysis.baseline_bounds.min_x > residual_bounds.min_x);
        assert!(analysis.crop_bounds.width() < safety.width());
    }
