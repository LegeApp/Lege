...
/// Process a ZIP of page images without extracting it to a temporary folder.
/// Entries are extracted and decoded in-memory by the bounded source workers.
pub fn run_zip_mode_with_config(
    zip_path: PathBuf,
    output: Option<PathBuf>,
    mut pipeline_config: PipelineConfig,
    progress_tracker: Option<ProgressTracker>,
    ...
    if !zip_path.is_file() {
        return Err(anyhow!("Path is not a ZIP file: {}", zip_path.display()));
    }
    ...
    let output_path = output.unwrap_or_else(|| {
        ...
        let stem = zip_path
            ...
            .unwrap_or("output");
        let extension = if pipeline_config.text_format() == "djvu" {
            "djvu"
        } else {
            "pdf"
        };
        parent.join(format!("{stem}_processed.{extension}"))
    });
    ...
    let adaptive = AdaptiveConcurrency::detect();
    let source = Arc::new(crate::pipeline::source::ZipImagePageSource::open(
        zip_path.clone(),
        adaptive.io_workers,
        pipeline_config.target_height(),
        pipeline_config.target_width(),
    ...
    info_println!(
        ...
        zip_path.display(),
        output_path.display(),
        crate::pipeline::source::PageSource::page_count(source.as_ref()),
        if pipeline_config.enable_layout_detection() {
            "ENABLED"
        } else {
            "disabled"
...
fn run_image_source_with_config(
    source: Arc<dyn crate::pipeline::source::PageSource>,
    output_path: PathBuf,
    mut pipeline_config: PipelineConfig,
    progress_tracker: Option<ProgressTracker>,
    ...
    let total_pages = page_range
        .as_ref()
        .map(|range| range.len())
        .unwrap_or_else(|| source.page_count());
...
pub fn run_pdf_to_images_mode(
    ...
    for page_num in pages_to_render.iter() {
        ...
        if enable_layout_detection {
            ...
            let binarized = crate::color::binarization::binarize_image_raw(
                img.as_raw(),
                width,
                height,
                &crate::color::BinarizationOptions {
            ...
            let pbm_data = crate::color::binarization::pbm::make_pbm_p4(&binarized, width, height);
