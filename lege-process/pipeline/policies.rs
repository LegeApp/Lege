...
//       PP-DocLayout stretch resize config and image building.
...
/// PP-DocLayout resize configuration — direct stretch to `target`×`target`
/// (`keep_ratio=false`, matching the model's training preprocessing). No
/// letterbox padding; `LayoutDetector::detect_rgb` applies ImageNet
/// normalization on top.
#[derive(Debug, Clone)]
pub struct PaddleResizeConfig {
    pub target: u32,
    pub border_value: u8,
}
...
impl Default for InferenceResizeSpec {
    fn default() -> Self {
        PaddleResizeConfig::default().into()
    }
}

impl From<PaddleResizeConfig> for InferenceResizeSpec {
    fn from(c: PaddleResizeConfig) -> Self {
        Self {
            target: c.target,
            // PP-DocLayout stretches (keep_ratio=false); no letterbox.
...
/// Layout detection provider using the existing InferenceHandle.
pub struct LayoutDetectionProvider {
...
#[async_trait::async_trait]
impl DetectionProvider for LayoutDetectionProvider {
    async fn run_detection(&self, page: RenderedPageData) -> InferenceResult {
        let dets = match self
            .inference_handle
...
mod tests {
    ...
    fn test_letterbox_mapping() {
        // Exercises the Letterbox coordinate math directly (the shipped
        // PP-DocLayout path uses Direct, but the Letterbox helper remains).
        let spec = InferenceResizeSpec {
            target: 1024,
