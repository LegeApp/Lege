...
fn page_text_from_hocr(page: &HocrPage) -> Option<PageText> {
    let hocr_lines = crate::hocr::parse_hocr(&page.hocr).ok()?;
...
mod tests {
    ...
    fn epub_region_filter_skips_page_furniture_before_ocr() {
        let detections = vec![
            det("header", ContentCategory::Abandon),
            det("footer", ContentCategory::Abandon),
            det("number", ContentCategory::Abandon),
            det("seal", ContentCategory::Image),
            det("image", ContentCategory::Image),
            det("text", ContentCategory::Text),
            det("table", ContentCategory::Table),
            det("footnote", ContentCategory::Text),
        ];
        ...
        assert_eq!(classes, vec!["text", "table", "footnote"]);
    }
