...
fn main() {
    ...
    // Sheet Music Edition has only three compact settings columns and does not
    // need the main GUI's wide presentation. 700px is about 30% narrower than
    // the previous 990px window while still clearing every control and popup.
    let (window_width, window_height) = (700.0, 544.0);
    let (window_min_width, window_min_height) = (700.0, 544.0);

    launch(
        LaunchConfig::new().with_window(
            WindowConfig::new(app::app)
                .with_title("Lege — Sheet Music Edition")
                .with_size(window_width, window_height)
                .with_min_size(window_min_width, window_min_height)
                .with_max_size(1980.0, 1380.0)
                .with_aspect_ratio_range(1.30, 1.86)
