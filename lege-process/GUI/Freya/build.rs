...
fn get_external_version_from_main_project() -> Option<String> {
    ...
    let version_file = manifest_dir.join("../..").join("external_version.txt");
...
fn main() {
    println!("cargo:rerun-if-changed=../../external_version.txt");
    let external_version = get_external_version_from_main_project().unwrap_or_else(|| {
        env::var("LEGE_EXTERNAL_VERSION").unwrap_or_else(|_| env!("CARGO_PKG_VERSION").to_string())
    });
