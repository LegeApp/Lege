...
//!   * Building inference images (direct resize, no letterbox)
...
/// Currently only Direct (no padding). Extend if letterbox or constrained fits are reintroduced.
...
impl Default for InferenceResizeSpec {
    fn default() -> Self { Self { target: 640, policy: ResizePolicy::Direct } }
}
...
#[inline]
pub fn scale_factors_infer_to_page(page_w: u32, page_h: u32, spec: &InferenceResizeSpec) -> (f32, f32) {
...
#[inline]
pub fn map_bbox_infer_to_page(b: [f32;4], page_w: u32, page_h: u32, spec: &InferenceResizeSpec) -> [f32;4] {
...
pub fn build_inference_image(
    ...
    match spec.policy {
        ResizePolicy::Direct => {
            ...
            #[cfg(feature = "debug-logging")]
            crate::debug_println!("RESIZE INFERENCE: Using resize params - Method: {:?}, Letterbox: {}, Border value: {}", 
                params.method, params.letterbox, params.border_value);
...
#[inline]
pub fn maybe_remap_bbox_from_infer(b: [f32;4], page_w: u32, page_h: u32, spec: &InferenceResizeSpec) -> [f32;4] {
...
#[inline]
pub fn apply_page_adjustments(b: [f32;4], _deskew: Option<&DeskewTransform>, margin: Option<&MarginCorrection>) -> [f32;4] {
    let mut result = b;
...
pub fn full_infer_bbox_to_final_page(
    b: [f32;4], page_w: u32, page_h: u32, spec: &InferenceResizeSpec,
    deskew: Option<&DeskewTransform>, margin: Option<&MarginCorrection>
) -> [f32;4] {
