...
impl DjvuOrchestrator {
    ...
    fn parse_hocr_to_words(
        ...
        for cap in word_re.captures_iter(hocr) {
            let x1: u16 = cap[1].parse().unwrap_or(0);
            let y1: u16 = cap[2].parse().unwrap_or(0);
            let x2: u16 = cap[3].parse().unwrap_or(0);
            let y2: u16 = cap[4].parse().unwrap_or(0);
            ...
            match msg {
                DjvuWriterMessage::AppendPage { page, page_index } => {
                    ...
                    while let Some(page) = page_buffer.remove(&next_expected) {
                        ...
                        if should_update {
                            progress_tracker.update(crate::progress::ProcessingStatus::DjvuBundling {
