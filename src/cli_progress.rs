...
pub async fn run_cli_three_line(rx: flume::Receiver<crate::progress::ProgressUpdate>) -> anyhow::Result<()> {
    ...
    loop {
        match rx.recv_async().await {
            Ok(crate::progress::ProgressUpdate::Status { status, .. }) => {
                ...
                match &status {
                    crate::progress::ProcessingStatus::LayoutProgress { rendered, detected, encoded, .. } => {
                        // Only update every 10 pages or when a stage meaningfully advances
                        if *rendered != last_rendered && (*rendered % 10 == 0 || *rendered > last_rendered) {
                            is_significant = true;
                        }
                        if *detected != last_detected && (*detected % 10 == 0 || *detected > last_detected) {
                            is_significant = true;
                        }
                        if *encoded != last_encoded && (*encoded % 10 == 0 || *encoded > last_encoded) {
                            is_significant = true;
                        }
                    ...
                    crate::progress::ProcessingStatus::MarginProgress { pass1_rendered, pass2_processed, .. } => {
                        // Update every 10 pages
                ...
                if is_significant && (l0.as_str(), l1.as_str(), l2.as_str()) != (last_lines.0.as_str(), last_lines.1.as_str(), last_lines.2.as_str()) {
    ...
    if let Some(duration) = completed_duration {
        println!();
        println!("{} \x1b[92mProcessing completed in {}.\x1b[0m", timestamp(), format_duration(duration));
        io::stdout().flush()?;
    }
