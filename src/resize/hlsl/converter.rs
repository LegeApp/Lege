...
impl HlslConverter {
    ...
    pub fn convert_to_nchw_f32_on_gpu(
        ...
        let buffers = self.buffers.as_ref().ok_or_else(|| {
            ResizerError::ResourceAllocationFailed("Converter buffers not initialized".into())
        })?;
        let pso = self
            .pipelines
        ...
        unsafe {
            ...
            if self.verbose {
                eprintln!(
                    "[Convert] SRV elements: {} ({} bytes total)",
                    srv_elements, input.size_in_bytes
                );
            }
            ...
            self.ctx
                .device
                .CreateShaderResourceView(&input.resource, Some(&srv_desc), cpu_srv);

            // UAV for output NCHW f32 buffer
            let uav_elements = (dst_bytes / 4) as u32;
            if self.verbose {
                eprintln!(
                    "[Convert] UAV elements: {} ({} bytes total)",
                    uav_elements, dst_bytes
                );
            }
            ...
            self.ctx
                .command_list
                .SetComputeRootConstantBufferView(0, buffers.cb_upload.GetGPUVirtualAddress());
            self.ctx
                .command_list
            ...
            // Barriers
            if self.verbose {
                eprintln!("[Convert] Barrier: input GENERIC_READ -> NON_PIXEL_SHADER_RESOURCE");
            }
            ...
            if self.verbose {
                eprintln!("[Convert] Barrier: dst COMMON -> UNORDERED_ACCESS");
            }
            self.transition_barrier(
                &buffers.gpu_dst_nchw,
            ...
            if self.verbose {
                eprintln!(
                    "[Convert] Dispatch: groups=({}, {}, {})",
                    groups_x, groups_y, batch_size as u32
                );
            }
            self.ctx
                .command_list
                .Dispatch(groups_x, groups_y, batch_size as u32);

            // Prepare for read by DML / ORT
            if self.verbose {
                eprintln!("[Convert] Barrier: dst UNORDERED_ACCESS -> GENERIC_READ");
            }
            self.transition_barrier(
                &buffers.gpu_dst_nchw,
                D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
    ...
    fn ensure_pipeline(&mut self, ctype: ConversionType) -> Result<()> {
        ...
        if shader.is_empty() {
            return Err(ResizerError::ShaderCompilationFailed(
                "Converter shader bytecode not available".into(),
            ));
        }
    ...
    pub fn convert_to_nchw_f32_then_readback(
        ...
        unsafe {
            ...
            if self.verbose {
                eprintln!(
                    "[Convert] Readback NCHW f32 completed: {} floats",
                    out.len()
                );
            }
            ...
            if std::env::var("LEGE_DUMP_HLSL_CONVERT").ok().as_deref() == Some("1") {
                ...
                if self.verbose {
                    eprintln!("[Convert] Dumped NCHW sample to {}", filename);
                }
            }
            Ok(out)
        }
